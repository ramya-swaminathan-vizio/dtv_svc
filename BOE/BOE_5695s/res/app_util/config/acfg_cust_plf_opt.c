...
-----------------------------------------------------------------------------*/
#ifndef EEP_MODEL_NAME
#define EEP_MODEL_NAME (EEP_OFFSET_PLF_OPT_11 + 1)
#endif
...
{
#ifdef APP_MODEL_NAME_IN_EEPROM
    INT32 i4_ret = 0;
...
#ifdef APP_MODEL_NAME_IN_EEPROM
    i4_ret = a_cfg_eep_raw_read(EEP_MODEL_NAME, s_str, (PLF_OPT_MODEL_NAME_LEN - 1), &ui4_read_byte);
    if (i4_ret != APP_CFGR_OK)
    {
        return i4_ret;
    }
    s_str [PLF_OPT_MODEL_NAME_LEN - 1] = '\0';
#else
    c_strncpy(s_str, "VIA", PLF_OPT_MODEL_NAME_LEN);
#endif
