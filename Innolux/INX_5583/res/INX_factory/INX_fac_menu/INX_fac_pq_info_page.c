...
static UINT32 inx_fac_pq_info_page_dolby_ver_content(UTF16_T* content)
{
    ...
    c_uc_w2s_strcpy(content, w2s_menu_item_string);
    _nav_fac_view_menu_list_enable_items(NAV_INX_FAC_PQ_INFO_PAGE_DOLBY_VER,FALSE);
    return 0;
#else
	printf("%s,%d\n",__func__,__LINE__);
	UTF16_T w2s_menu_item_string[NAV_FAC_MENU_TEXT1_LEN + 1] = {0};
...
static UINT32 inx_fac_pq_info_page_elcs_content(UTF16_T* content)
{
	...
	if(inx_fac_items_get_inx_at_ui_switch() == INX_AT_SWITCH_PD)
		_nav_fac_view_menu_list_enable_items(NAV_INX_FAC_PQ_INFO_PAGE_ELCS,FALSE);
	else
		_nav_fac_view_menu_list_enable_items(NAV_INX_TO_FAC_PQ_INFO_PAGE_ELCS,FALSE);
...
static UINT32 inx_fac_pq_info_page_color_tuner_content(UTF16_T* content)
{
...
static BOOL inx_fac_pq_info_page_color_tuner_handle(UINT32 nc_code,UINT32 key_code)
{
    if(key_code == BTN_SELECT || key_code == BTN_CURSOR_RIGHT)
	{
	    printf("%s().%d : nc_code=%d\n",__func__, __LINE__,nc_code);
	    nav_inx_fac_view_hide(INX_FAC_MENU_PQ_INFO_PAGE);
...
inx_page_entry inx_fac_pq_info_page_entry[INX_FAC_MENU_TITLE_MAX][INX_FAC_MENU_ITEM_CNT] =
{
    {//For PD Use
        ...
        {NAV_INX_FAC_PQ_INFO_PAGE_COLOR_TUNER, MLM_FAC_KEY_MSG_COLOR_TUNER, {0,0}, inx_fac_pq_info_page_color_tuner_content, inx_fac_pq_info_page_color_tuner_handle},
