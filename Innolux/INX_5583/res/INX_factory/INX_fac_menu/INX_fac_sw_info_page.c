...
static UINT32 inx_fac_sw_info_page_level_shift_dc_on_content(UTF16_T* content)
{
    UTF16_T w2s_menu_item_string[NAV_FAC_MENU_TEXT1_LEN + 1] = {0};
    CHAR flag_buf[32] = {0};
	CHAR path[128] = {0};
	c_sprintf(path,"/proc/inx_level_shift_flag");
    _nav_fac_get_tconless_dc_on_flag(path,flag_buf, sizeof(flag_buf));          
    c_memset(w2s_menu_item_string, 0, sizeof(w2s_menu_item_string));
    c_uc_ps_to_w2s(flag_buf, w2s_menu_item_string, (NAV_FAC_MENU_TEXT1_LEN + 1));
    c_uc_w2s_strcpy(content,w2s_menu_item_string);
    return 0;
}

static UINT32 inx_fac_sw_info_page_vcom_download_dc_on_content(UTF16_T* content)
{
    UTF16_T w2s_menu_item_string[NAV_FAC_MENU_TEXT1_LEN + 1] = {0};
    CHAR flag_buf[32] = {0};
	CHAR path[128] = {0};
	c_sprintf(path,"/proc/inx_vcom_dl_flag");
    _nav_fac_get_tconless_dc_on_flag(path,flag_buf, sizeof(flag_buf));           
    c_memset(w2s_menu_item_string, 0, sizeof(w2s_menu_item_string));
    c_uc_ps_to_w2s(flag_buf, w2s_menu_item_string, (NAV_FAC_MENU_TEXT1_LEN + 1));
    c_uc_w2s_strcpy(content,w2s_menu_item_string);
...
static BOOL inx_fac_sw_info_page_at_cmd_log_cp_key_handle(UINT32 nc_code,UINT32 key_code)
{
    ...
    if(key_code == BTN_CURSOR_RIGHT || key_code == BTN_SELECT){
        ...
        b_file_exist = (access(full_path, F_OK) == 0 ? TRUE : FALSE);
        if( b_file_exist == FALSE ){
            printf("%s() : At Cmd Log File not found.\n",__func__);
            return FALSE;
        }
        inx_system("cp -rf %s* %s/; sync; sync; sync;", INX_ATCMD_PREFIX, carrUsbPath);
...
inx_page_entry inx_fac_sw_info_page_entry[INX_FAC_MENU_TITLE_MAX][INX_FAC_MENU_ITEM_CNT] =
{
    {//For PD Use
        ...
        {NAV_INX_FAC_SW_INFO_PAGE_LEVEL_SHIFT_DC, MLM_FAC_KEY_MSG_LEVEL_SHIFT_DC,{0,0}, inx_fac_sw_info_page_level_shift_dc_on_content, NULL},
        {NAV_INX_FAC_SW_INFO_PAGE_VCOM_DOWNLOAD_DC, MLM_FAC_KEY_MSG_VCOM_DOWNLOAD_DC,{0,0}, inx_fac_sw_info_page_vcom_download_dc_on_content, NULL},
