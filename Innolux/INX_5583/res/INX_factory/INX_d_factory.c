...
/*-----------------------------------------------------------------------------
                    data declarations
-----------------------------------------------------------------------------*/
static struct timeval START_CLOCK={0};
static struct timeval END_CLOCK={0};
...
extern UINT8 a_cfg_custom_factory_reset_ex()
{
	...
	a_cfg_cust_unblank_screen(); // display blacklight
...
/*-----------------------------------------------------------------------------
 * Author: zlhuang
 * Name: _nav_get_bluetooth_xrt260test_mac
 * Description: get nearby XRT260TEST remote control's MAC
 * Inputs:
 * Outputs: remote control's MAC "hh:hh:hh:hh:hh:hh"
 * Returns:
 ----------------------------------------------------------------------------*/
INT32 _nav_get_bluetooth_xrt260test_mac(CHAR *mac_str)
{
    INT32 i4_result = APP_CFGR_INTERNAL_ERR;
    INT32 cmd_result = 0;
	CHAR tmp[128] = {0};
 ...
 * Returns:
 ----------------------------------------------------------------------------*/ 
#define MT5691_DEMURA_PARTITION "/dev/mmcblk0p18" //partition size=3MB, refer to partition_layout.txt
void inx_clear_demura_partition(void)
{
    INT32 cmd_result = 0;
	CHAR c_cmd_buf[512] = {0};	

	//c_sprintf(c_cmd_buf, "dd if=/dev/zero of=%s bs=1M count=3 2>&1", MT5691_DEMURA_PARTITION); => permission denied for access of /dev/zero...
	c_sprintf(c_cmd_buf, "dd if=/dev/zram0 of=%s bs=1M count=3 2>&1", MT5691_DEMURA_PARTITION); ///dev/zram0 access ok!
    DBG_LOG_PRINT(("%s() c_cmd_buf=%s\n", __func__, c_cmd_buf));
...
void _get_wifi_vid_pid_data(CHAR *s_path , CHAR *data)
{
	FILE *fp;
    CHAR tmp[64] = {0};
    UINT16 len=0;
...
/*-----------------------------------------------------------------------------
 * Autho: truedano
 * Name: a_cfg_get_video_menu_setting
 * Description: 
 * Inputs:
 * Outputs: 
 * Returns:
 ----------------------------------------------------------------------------*/
extern INT16 a_cfg_get_menu_setting(UINT16 gid,UINT16 vid)
{
    UINT16 ui2_id = CFG_MAKE_ID(gid, vid);
    INT16   i2_val;
    INT16 i4_ret = 0;
...
/*-----------------------------------------------------------------------------
 * Autho: 
 * Name: _start_time_stamp
 * Description: 
 * Inputs:
 * Outputs: 
 * Returns:
 ----------------------------------------------------------------------------*/
void _start_time_stamp(void)
{
...
/*-----------------------------------------------------------------------------
 * Autho: 
 * Name: _end_time_stamp
 * Description: 
 * Inputs:
 * Outputs: 
 * Returns:
 ----------------------------------------------------------------------------*/
double _end_time_stamp(void)
{
    UINT32 start_us=0, end_us=0;
    double cpu_time_used;

    gettimeofday(&END_CLOCK, NULL);
    start_us = START_CLOCK.tv_sec*1000000 + START_CLOCK.tv_usec;
    end_us = END_CLOCK.tv_sec*1000000 + END_CLOCK.tv_usec;
    cpu_time_used = ((double) (end_us - start_us)) / 1000000;
    return cpu_time_used;
}

/*-----------------------------------------------------------------------------
 * Author: zlhuang
 * Name: _nav_set_auto_switch_to_smartcast
 * Description: enable/disable [Vizio] auto-switch-to-smartcast function
 * Inputs: enable_flag: FALSE=disable-auto-switch; TRUE=enable-auto-switch
 ----------------------------------------------------------------------------*/
INT32 _nav_set_auto_switch_to_smartcast(BOOL enable_flag)
{
    FILE *file1;
    INT32 line_start_pos = 0;
    CHAR tmp[128] = {0};
    CHAR new_setting[64] = {0};
    CHAR scpl_json[] = "/data/app/storage/scpl/scpl.capabilities.json";
    INT32 file_write_len;
    INT32 file_write_result;

    DBG_LOG_PRINT(("%s() %d, set status=%s\n", __func__, __LINE__, enable_flag?"true":"false"));

    // to read and write the same file:
    // the length of original and updated content must be the same
    file1 = fopen(scpl_json, "r+");
    if (file1 != NULL)
    {
        while (fgets(tmp, 128, file1) != NULL)
        {
            //DBG_LOG_PRINT(("file_content(%d)='%s'\n", line_start_pos, tmp));
            //e.g. "BootToSC": true,
            if (strstr(tmp, "BootToSC") != NULL)
            {
                DBG_LOG_PRINT(("file_content(%d)='%s'\n", line_start_pos, tmp));

                // to avoid unnecessary write
                if ( (enable_flag && strstr(tmp, "true")) ||
                     (!enable_flag && strstr(tmp, "false"))
                   )
                {
                    DBG_LOG_PRINT(("%s() %d, same status, just break\n", __func__, __LINE__));
                    break;
                }

                if (enable_flag)
                    sprintf(new_setting, "    \"BootToSC\": true, \n"); //the original content just has a space before newline...
                else
                    sprintf(new_setting, "    \"BootToSC\": false,\n");

                DBG_LOG_PRINT(("%s() %d, new_setting='%s'\n", __func__, __LINE__, new_setting));
                // set file position to current line start and replace the content
                fseek(file1, line_start_pos, SEEK_SET);
                file_write_len = strlen(new_setting);
                file_write_result = fputs(new_setting, file1);
                //DBG_LOG_PRINT(("%s() %d, file_write_len=%d, file_write_result=%d\n", __func__, __LINE__, file_write_len, file_write_result));
                if (file_write_result == EOF)
                {
                    DBG_LOG_PRINT(("%s() %d, file write failed\n", __func__, __LINE__));
                    fclose(file1);
                    return INX_RET_FAIL;
                }
            ...
            // mark the next line start position
            line_start_pos = ftell(file1);
        }
	    fclose(file1);
    ...
    {
        DBG_LOG_PRINT(("%s() %d, file open failed\n", __func__, __LINE__ ));
        return INX_RET_FAIL;
    }
...
////////////////////////////////////////////////////////////////////////////////
//parse INI related functions, zl
INT32 a_cfg_cust_info_get_customer_ini_path(CHAR * ps_str)
{
    CHAR s_str[MI_INI_FILE_PATH_LEN+1] = {0};

    c_strncpy(s_str,MI_INI_CONFIG_PATH,MI_INI_FILE_PATH_LEN);
    c_strncat(s_str,MI_INI_MODEL_DIR,MI_INI_FILE_PATH_LEN);
    c_strncat(s_str,MI_INI_CUSTOMER_FILE,MI_INI_FILE_PATH_LEN);
...
/*-----------------------------------------------------------------------------
 * Autho:
 * Name: a_cfg_cust_info_get_hdmi_edid
 * Description: Get HDMI EDID BIN
 * Inputs:
 * Outputs: edid data length
 * Returns:
 ----------------------------------------------------------------------------*/
/*
hdmi_version: 1:1.4, 2:2.0, 3:2.1
hdmi_port: 1 ~ N
*/
INT32 a_cfg_cust_info_get_hdmi_edid(INT8 hdmi_version, INT8 hdmi_port, UINT8 *pui1_edid_data){
    CHAR ps_customer_ini_path[MI_INI_FILE_PATH_LEN+1]={0};
    CHAR ps_section_name[32]={0};
    CHAR ps_item_name[32]={0};
    INT32 edid_data_length;

    a_cfg_cust_info_get_customer_ini_path(ps_customer_ini_path);

    c_sprintf(ps_section_name,"HDMI_EDID_%d", hdmi_port);

    switch (hdmi_version)
    {
        case INX_EDID_1_4:
            c_strncpy(ps_item_name, "HDMI_EDID_File_1_4", sizeof(ps_item_name));
            break;
        case INX_EDID_2_0:
            c_strncpy(ps_item_name, "HDMI_EDID_File_2_0", sizeof(ps_item_name));
            break;
        case INX_EDID_2_1:
            c_strncpy(ps_item_name, "HDMI_EDID_File_2_1", sizeof(ps_item_name));
            break;
        default:
            return APP_CFGR_INTERNAL_ERR;
    }

    DBG_LOG_PRINT(("%s(), section_name=%s, item_name=%s\n", __func__, ps_section_name, ps_item_name));
    char ps_edid_bin_path[512] = {0};
    a_cfg_cust_info_get_ini_item_string(ps_customer_ini_path, ps_section_name, ps_item_name, "", ps_edid_bin_path, sizeof(ps_edid_bin_path));
    DBG_LOG_PRINT(("%s(), edid_bin_path='%s'\n", __func__, ps_edid_bin_path));
    edid_data_length = _nav_fac_get_file_size(ps_edid_bin_path);
    if((edid_data_length == -1) || 
       (edid_data_length == 0) || 
       (edid_data_length %128 != 0)){
        DBG_LOG_PRINT(("%s(), get file size error, edid_data_length = %d \n", __func__, edid_data_length));
        return APP_CFGR_CANT_INIT;
    }

    //read EDID bin file
    FILE* fstream = fopen(ps_edid_bin_path, "rb");
    int return_read=0;
    if(!fstream){
        DBG_LOG_PRINT(("%s(), unable to open EDID bin file: %s => %s\n", __func__, ps_edid_bin_path, strerror(errno)));
        return APP_CFGR_INTERNAL_ERR;
    }
    else{
        return_read = fread(pui1_edid_data, edid_data_length, 1, fstream);
        DBG_LOG_PRINT(("%s(): return_read=%d\n", __func__, return_read));
        fclose(fstream);
    }

    return edid_data_length;
}
