...
static UINT32 inx_fac_main_page_isp_info_content(UTF16_T* content)
{
    inx_fac_change_page_content(content);
    return 0;
}
...
static BOOL inx_fac_main_page_isp_info_key_handle(UINT32 nc_code,UINT32 key_code)
{
	if(key_code == BTN_SELECT || key_code == BTN_CURSOR_RIGHT)
	{
	    printf("%s().%d : nc_code=%d\n",__func__, __LINE__,nc_code);
	    nav_inx_fac_view_hide(INX_FAC_MENU_MAIN_PAGE);
	    nav_inx_fac_view_activate(INX_FAC_MENU_ISP_INFO_PAGE);
	}
	return 0;
}
...
inx_page_entry inx_fac_main_page_entry[INX_FAC_MENU_TITLE_MAX][INX_FAC_MENU_ITEM_CNT] =
{
    {//For PD Use
        ...
        {NAV_INX_FAC_MAIN_PAGE_ISP_INFO, MLM_FAC_KEY_MSG_ISP_INFO,{0,0}, inx_fac_main_page_isp_info_content, inx_fac_main_page_isp_info_key_handle},    
