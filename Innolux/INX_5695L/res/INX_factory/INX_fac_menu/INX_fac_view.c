...
#include "INX_factory/INX_fac_alsa.h"
...
typedef struct _NAV_FAC_VIEW_T
{
	...
	HANDLE_T				   h_bt_rms_osd;
	HANDLE_T				   h_bt_rms_osd_timer;
	BOOL						b_fac_active;
} NAV_FAC_VIEW_T;
...
#else
	INT32 rssi_value =_nav_get_bluetooth_rssi();
    //printf("%s, %d, rssi_value=%d\n",__func__,__LINE__, rssi_value);
    if(rssi_value == 0xFF)
...
/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_create
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
extern INT32 nav_fac_view_bt_rms_osd_create(VOID)
{
	NAV_FAC_VIEW_T*     pt_view = &t_g_nav_fac_view;
    UINT32 i4_ret = 0;
    GL_RECT_T           t_rect;
    ...
    t_clr_info.u_color_data.t_standard.t_highlight =inx_g_fa_cmenu_color_orange;
    t_clr_info.u_color_data.t_standard.t_disable   =inx_g_fa_cmenu_color_orange;
    i4_ret = c_wgl_do_cmd(pt_view->h_bt_rms_osd,
                          WGL_CMD_GL_SET_COLOR,
                          WGL_PACK(WGL_CLR_TEXT),
                          WGL_PACK(&t_clr_info ));
    INX_LOG_ON_FAIL(i4_ret);
    ...
    t_fnt_info.e_font_style = FE_FNT_STYLE_REGULAR;
    t_fnt_info.e_font_cmap  = FE_CMAP_ENC_UNICODE;
    t_fnt_info.ui1_custom_size = 100; //one byte only => max=255
    ...
    i4_ret = c_wgl_float(pt_view->h_bt_rms_osd, TRUE, FALSE);
    INX_LOG_ON_FAIL(i4_ret);
    
    i4_ret = c_wgl_set_visibility(pt_view->h_bt_rms_osd, WGL_SW_HIDE);
    INX_LOG_ON_FAIL(i4_ret);
...
/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_show
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
extern INT32 nav_fac_view_bt_rms_osd_show(CHAR* pstring)
{
    ...
    i4_ret = c_wgl_set_visibility(pt_view->h_bt_rms_osd, WGL_SW_RECURSIVE);
    INX_LOG_ON_FAIL(i4_ret);
    
    i4_ret = c_wgl_repaint(pt_view->h_bt_rms_osd, NULL, TRUE);
    INX_LOG_ON_FAIL(i4_ret);
...
/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_display
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
static VOID nav_fac_view_bt_rms_osd_display(void)
{
	CHAR message_buf[64] = {0};
	S_D7_MIC_RMS s_rms_value;
	inx_snd_mixer_execute();
	read_rms_value(&s_rms_value);
	c_sprintf(message_buf,"%d,%d,%d,%d,%d,%d,%d,%d",(UINT8)s_rms_value.rms_value[7]
													   ,(UINT8)s_rms_value.rms_value[6]
													   ,(UINT8)s_rms_value.rms_value[5]
													   ,(UINT8)s_rms_value.rms_value[4]
													   ,(UINT8)s_rms_value.rms_value[3]
													   ,(UINT8)s_rms_value.rms_value[2]
													   ,(UINT8)s_rms_value.rms_value[1]
													   ,(UINT8)s_rms_value.rms_value[0]);


    nav_fac_view_bt_rms_osd_create();
    nav_fac_view_bt_rms_osd_show(message_buf);
}

/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_hide
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
extern INT32 nav_fac_view_bt_rms_osd_hide(void)
{
    UINT32 i4_ret = 0;
    NAV_FAC_VIEW_T*     pt_view = &t_g_nav_fac_view;
...
/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_timer_create
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
extern INT32 nav_fac_view_bt_rms_osd_timer_create(void)
{
    UINT32 i4_ret = 0;
    
    i4_ret = c_timer_create(&t_g_nav_fac_view.h_bt_rms_osd_timer);
    INX_LOG_ON_FAIL(i4_ret);
...
/*-----------------------------------------------------------------------------
 * Name : _nav_fac_view_bt_rms_osd_timeout
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
static VOID _nav_fac_view_bt_rms_osd_timeout(
    HANDLE_T                    h_timer,
    VOID*                       pv_tag
    )
{
	if(b_bt_rssi_flag){
        printf("%s, %d, timer start again ...\n",__func__,__LINE__);
		nav_fac_view_bt_rms_osd_timer_start(b_rms_period);
	}
    else{
   	    printf("%s, %d, timer stop !\n",__func__,__LINE__);
    }
	
}

/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_timer_start
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
extern INT32 nav_fac_view_bt_rms_osd_timer_start(UINT32 msec)
{
    UINT32 i4_ret = 0;

	nav_fac_view_bt_rms_osd_display();

	b_rms_period = msec;
    i4_ret = c_timer_start(t_g_nav_fac_view.h_bt_rms_osd_timer,
                	       b_rms_period,
                	       X_TIMER_FLAG_ONCE,
                	       _nav_fac_view_bt_rms_osd_timeout,
                	       NULL);
    INX_LOG_ON_FAIL(i4_ret);
...
/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_timer
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
extern VOID nav_fac_view_bt_rms_osd_timer(UINT32 msec)
{
    nav_fac_view_bt_rms_osd_timer_create();
    nav_fac_view_bt_rms_osd_timer_start(msec);
}

/*-----------------------------------------------------------------------------
 * Name : nav_fac_view_bt_rms_osd_flag
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
...
extern VOID nav_fac_view_bt_rms_osd_flag(UINT8 start_flag)
{
    if (start_flag==1)
	{
	    b_bt_rssi_flag=TRUE;
	...
	{
		nav_fac_view_bt_rms_osd_hide();
    	b_bt_rssi_flag=FALSE;
	}	
}
