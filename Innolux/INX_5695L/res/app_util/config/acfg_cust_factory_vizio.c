...
extern INT32 a_cfg_fac_eep_init(VOID)  //ke_vizio_20160908 add
{
    INT32   i4_ret;
    #if 0
    UINT32  ui4_off     = 0x140;    //Soar add 20081028   Modify EEPROM init don't need change HDCP data. 
    #else
    UINT32  ui4_off     = 0x00;    //sam 121119 due to HDCP move to 0x1CA0
    #endif
...
#ifdef APP_BLUETOOTH_SUPPORT
    UINT8 aui1_faccmd_ret_val_BT[CMD_LEN_Maximum] = {0};
#endif
    UINT8 aui1_faccmd_ret_val_HDCP[320] = {0};  //ke_vizio_20121114 modify //sam 121113
...
#ifdef APP_BLUETOOTH_SUPPORT
    UINT32 ui4_read_BT;
    a_cfg_eep_raw_read(EEP_BT_MAC, aui1_faccmd_ret_val_BT, 7, &ui4_read_BT);
#endif
...
#ifdef APP_BLUETOOTH_SUPPORT
    UINT32 ui4_written_BT;
    i4_ret = a_cfg_eep_raw_write(EEP_BT_MAC, aui1_faccmd_ret_val_BT, 7, &ui4_written_BT);
#endif
...
INT32 a_cfg_cust_info_get_hdmi_edid(INT8 hdmi_version, INT8 hdmi_port, UINT8 *pui1_edid_data){
	...
	/*
	pinifile_data inidb;
	i4_ret =open_ini_file(ps_customer_ini_path, &inidb);
	printf("%s(), ret=%d\n", __func__, i4_ret);
	if(i4_ret != 0){
	...
	*/
	...
	//const char *ps_edid_bin_path = get_value(inidb, ps_section_name, ps_item_name);
    ...
    if( edid_data_length == -1 ){
        DBG_LOG_PRINT(("%s(), get file size error\n", __func__));
...
# if 1
	...
	if(i4_ret != FMR_OK){
		printf("%s(): unable to open EDID bin file: %s (error=%d)\n", __func__, ps_edid_bin_path, i4_ret);
	}
	else{
		c_fm_read(h_fm_edid_bin, edid_data_length, 1, pui1_edid_data);
		c_fm_close(h_fm_edid_bin);
	}
#endif

    //close_ini_file(inidb); // invocation timing is important for get_value() result (e.g. ps_edid_bin_path)
...
void a_cfg_cust_get_format_time(CHAR *buf)
{
    ...
    sprintf (buf, "%ld%02d%02d%02d%02d", 
...
void a_cfg_cust_get_format_time_at_cmd_log(CHAR *buf)
{
    ...
    sprintf (buf, "%ld/%02d/%02d %02d:%02d:%02d.%lld%lld%lld",
        (1900+p->tm_year),( 1+p->tm_mon), p->tm_mday,p->tm_hour, p->tm_min, p->tm_sec, msec/100%10, msec/10%10, msec%10);
    //return buf;
}
/*-----------------------------------------------------------------------------
