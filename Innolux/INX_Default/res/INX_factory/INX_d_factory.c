...
/*----------------------------------------------------------------------------
 * Name: a_cfg_custom_set_short_protection
 *
 * Description: 
 *
 * Inputs:  -  true : enable the function of short protection,false else if as.
 *
 * Outputs: -
 *
 * Returns: -
 *          
 ----------------------------------------------------------------------------*/
extern void a_cfg_custom_set_short_protection(UINT8 ui2_switch)
{
	UINT32 ui4_read = 0;
	//Write EEP_INX_AS382X_SHORT_PROTECTION_SWITCH_FLAG 0x01
	INT32 i4_ret = a_cfg_eep_raw_write(EEP_INX_AS382X_SHORT_PROTECTION_SWITCH_FLAG, &ui2_switch, 1, &ui4_read);
				
	if(i4_ret !=0)
	{
		DBG_INFO(("!!!!%s  EEP_INX_AS382X_SHORT_PROTECTION_SWITCH_FLAG write fail\n", __func__));
		return;
	}
	...
	UINT8 devid = 1;
    SIZE_T z_size       = sizeof(devid);
    SIZE_T z_size_drv   = sizeof(DRV_CUST_OPERATION_INFO_T);        
    INT32 i4_Ret        = 0;  
	c_memset(&t_op_info, 0, sizeof(DRV_CUST_OPERATION_INFO_T));
                
    t_op_info.e_op_type = DRV_CUSTOM_OPERATION_TYPE_CUST_SPEC_SET;
    t_op_info.u.t_cust_spec_set_info.e_cust_spec_type =  DRV_CUSTOM_CUST_SPEC_SWITCH_AS382X_SHORT_PROTECTION;
    t_op_info.u.t_cust_spec_set_info.pv_set_info = (VOID *)&devid;
    t_op_info.u.t_cust_spec_set_info.z_size = z_size;
    t_op_info.u.t_cust_spec_set_info.b_store = TRUE;
    ...
    i4_Ret = c_rm_set_comp(
     ...
     if (i4_Ret != RMR_OK)
     {
     	DBG_LOG_PRINT (("<Fac_menu> Set Short Protection failed !\n"));
     }          
}
/*-----------------------------------------------------------------------------
...
extern INT32 a_cfg_custom_read_file(UINT8 * fpath, UINT8 * fbuf, UINT32 fbuf_size)
{
    ...
    //check file size
    if( fsize > fbuf_size ) {
        printf("%s check file size fail ( %d %d )\n", __func__, fsize, fbuf_size);
        fclose(fp);
	return -3;
    }
...
INT32 a_cfg_cust_inx_get_wifi_module_info(WIFI_DEVICE_INFO* pt_info )
{
    ...
    else
    {
       if(!pf_out)
       {
           DBG_LOG_PRINT(("[%s %d] popen failed\n",__FUNCTION__,__LINE__));
       }
    }
    return APP_CFGR_OK;
}
...
* Description: Get wifi status information
* Input arguments: void
* Output arguments: N/A
*---------------------------------------------------------------------------*/
extern UINT8 _get_wifi_status(VOID)
{
                ...
                UINT8 wifi_sta=0;
                inx_system("export LD_LIBRARY_PATH=/linux_rootfs/lib:/basic:/basic/lib:/3rd/lib");
                inx_system("/3rd/bin/wpa_supplicant/common/wpa_cli -iwlan0 status|grep wpa_state > /tmp/wpa_status");
                
                fl = fopen("/tmp/wpa_status","r");         
                if(fl != NULL)
                {
                                if(fread(s_status,sizeof(CHAR),22,fl)!=0)
                                {
                                                if(c_strncmp(s_status,"wpa_state=COMPLETED",19) == 0)
                                                {
                                                                b_connected = TRUE;
								wifi_sta = 0x01; //joe.zy
																
                                                }
                                                else
                                                {
                                                                b_connected = FALSE;
								wifi_sta = 0x00; //joe.zy
                                                }
                                }
                                fclose(fl);
                }
                inx_system("rm /tmp/wpa_status");
                //return b_connected;
                return wifi_sta;
}
...
extern BOOL _get_usb_path(CHAR *path)
{
	...
	FILE *f1;
			...
			if(f1 != NULL)
			{
				...
				INX_AckString("[_get_usb_path] arr[0]=0x%02X\n", arr[0]);
...
UINT32 a_cfg_custom_dump_eep2usb(VOID)
{
    ...
    {
        INX_AckString("can't find usb device\n");
...
INT32 _nav_set_bluetooth_pair(UINT8 *device_mac, UINT8 mac_length)
{
	...
	c_sprintf(c_cmd_buf, "LD_LIBRARY_PATH=/data/app/bluetooth/lib /data/app/bluetooth/bin/bt_cli set_pair %s > /tmp/bt_pair_result &", c_mac_buf);
...
INT32 _nav_set_bluetooth_unpair(void)
{
	...
	cmd_result = inx_system("LD_LIBRARY_PATH=/data/app/bluetooth/lib /data/app/bluetooth/bin/bt_cli set_unpair");
   	DBG_LOG_PRINT(("%s() cmd_result=%d\n", __func__, cmd_result));
	if (cmd_result != 0)
	{
		return APP_CFGR_INTERNAL_ERR;
...
INT32 _nav_get_bluetooth_rc_mac(CHAR *mac_str)
{
    ...
    FILE * fl = popen("LD_LIBRARY_PATH=/data/app/bluetooth/lib /data/app/bluetooth/bin/bt_cli get_list | grep XRT | grep \"PAIRED:Y\"", "r");
    if (fl != NULL)
    {
...
INT32 _nav_get_bluetooth_rssi(void)
{
	...
	_nav_get_bluetooth_rc_mac(mac_str);
	//e.g. LD_LIBRARY_PATH=/data/app/bluetooth/lib /data/app/bluetooth/bin/bt_cli get_rssi A0:22:4E:00:10:B2
	c_sprintf(c_cmd_buf,"LD_LIBRARY_PATH=/data/app/bluetooth/lib /data/app/bluetooth/bin/bt_cli get_rssi %s", mac_str); 
    DBG_LOG_PRINT(("%s() c_cmd_buf=%s\n", __func__, c_cmd_buf));
	FILE * fl = popen(c_cmd_buf, "r");
    if (fl != NULL)
    {
		if (fgets(tmp, 127, fl) != NULL)
		{
			...
			if(p_str_rssi)
			{
				i4_result = c_strtoll(p_str_rssi, NULL, 10);
			}
		}
	    pclose(fl);
    }
...
INT32 _nav_get_bluetooth_host_mac(UINT8 *host_mac)
{
	...
	c_sprintf(c_cmd_buf, "LD_LIBRARY_PATH=/data/app/bluetooth/lib /data/app/bluetooth/bin/bt_cli get_mac_address"); 
    DBG_LOG_PRINT(("%s() c_cmd_buf=%s\n", __func__, c_cmd_buf));
	FILE * fl = popen(c_cmd_buf, "r");
    if (fl != NULL)
    {
		if (fgets(tmp, 127, fl) != NULL)
		{
			...
			DBG_LOG_PRINT(("p_str_host_mac=%s\n", p_str_host_mac));
			if(p_str_host_mac)
			{
