...
#define EMMC_BLOCK_DEMO	"/dev/mmcblk0p37"
...
UINT8 get_bt_rssi_status=INX_DISABLE;
UINT8 sscg_status=1;
...
static UINT32 inx_fac_inx_tools_page_get_bt_mac_content(UTF16_T* content)
{
    ...
    if (( access("/basic/features/VIZIO_FEATURE_CHIP_MT5691", F_OK) == 0 ) && (a_cfg_cust_Is_CurModel_2020() == TRUE))//MT5691 2020 H serial
    {
        //disable BT MAC when 2020 H
...
static UINT32 inx_fac_inx_tools_page_get_bt_rssi_content(UTF16_T* content)
{
    ...
    if (( access("/basic/features/VIZIO_FEATURE_CHIP_MT5691", F_OK) == 0 ) && (a_cfg_cust_Is_CurModel_2020() == TRUE))//MT5691 2020 H serial
    {
        //disable GET BT RSSI when 2020 H
        if( inx_fac_items_get_inx_at_ui_switch() == INX_AT_SWITCH_PD ){
...
static BOOL inx_fac_inx_tools_page_demo_update_handle(UINT32 nc_code,UINT32 key_code)
{
	...
	{	    
		...
		nav_fac_view_alert_osd("Copy vizio_demo.txt and *.mp4 to \"demo\" dir in usb disk.");
        nav_fac_view_alert_osd_timer(5000);
	}
