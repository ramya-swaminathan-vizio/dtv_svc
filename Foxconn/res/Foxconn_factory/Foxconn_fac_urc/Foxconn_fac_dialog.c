...
static BOOL b_Foxconn_wifi_connect_status = FALSE;
#endif
...
extern INT32 a_sleep_timer_start(UINT32 ui4_time);	//<Foxconn>James 20120321:Add auto shut-down at URC RST key

#ifdef APP_Foxconn_DFT_URC_WIFI
...
static INT32 _nav_fac_dialog_image_init(NAV_FAC_DIALOG_VIEW_T*    pt_nav_fac_dialog)
{
    ...
    i4_ret = Foxconn_fac_img_create_urc_bk_img_tpl(&pt_nav_fac_dialog->h_ui_simple_dialog_bk0);
    if(WGLR_OK != i4_ret)
    {
        return NAVR_FAIL;
    }
                    ...
                    )
{
    ...
    switch (ui4_key_code & 0x00ffffff)
    {
		/*	//mind 20160509
        ...
        case BTN_FAC_RST:
			{
    			...
    			_nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SET_TO_FM,
    																	   pw2s_string,
    																	  NULL);
    			a_cfg_set_bgm_wake_disable(FALSE);	// Tommy 20191009 
                a_amb_enable_power_key(TRUE);  //  Tommy 20191009 Ben 20190327, fix TV can't shut-down issue
        ...
        case BTN_FAC_CTC:
		{
			...
			i2_val += 1;
			if(i2_val > NAV_FAC_MENU_COLOR_TEMP_Normal){
			    i2_val = 0;
			}
			_nav_fac_color_temp_setting(i2_val);

			if(i2_val == NAV_FAC_MENU_COLOR_TEMP_WARM) //warm
			{
			    DBG_LOG_PRINT(("\n--------Color Temp to Warm--------\n"));
			...
			else if (i2_val == NAV_FAC_MENU_COLOR_TEMP_COOL) //cool
			{
			    DBG_LOG_PRINT(("\n--------Color Temp to Cool--------\n"));
			...
			else if (i2_val == NAV_FAC_MENU_COLOR_TEMP_Normal) //normal
			{
		...
		case BTN_FAC_ANTENNA:
        {
            ...
            a_cfg_av_get(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_CLR_TEMP), &i2_val);
            if(i2_val == NAV_FAC_MENU_COLOR_TEMP_WARM)
            {
	...
	case BTN_FAC_AVL_OFF:
       ...
       case BTN_FAC_SET_TO_FM:
              ...
              _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SET_TO_FM,
                                                                          ...
                                                                          NULL);
			  a_amb_enable_power_key(TRUE);  //  Tommy 20191009 Ben 20190327, fix TV can't shut-down issue
			  a_sleep_timer_start(3);	// Tommy 20191009 <TPV>James 20120321:Add auto shut-down at URC RST key
		break;
		#if 0
		...
		case BTN_FAC_LSR:
		{
            ...
            #ifdef Foxconn_2K19_DFM // Ben 20181221
                UINT8 ui1_status;
                UINT32 ui4_wirtten;
                ui1_status = 0x00;
    ...
    case BTN_FAC_WIFI_SSID:  //ke_vizio_20170523
    {
        ...
        i4_ret = Foxconn_FACTORY_WIFI_AUTODETECT();
        if (i4_ret != NWR_OK)
        {
    ...
    case BTN_FAC_RESERVE1:  //ke_vizio_20170523
    {
        ...
        DBG_LOG_PRINT(("%s EEP_WIFI_status: %d\n", __FUNCTION__, Foxconn_fac_get_WIFI_status()));

        pw2s_string = (TRUE == Foxconn_fac_get_WIFI_status() ? L"WIFI status : 01": L"WIFI Status : 00");
                                            ...
                                            wgl_widget_proc_fct pf_wdgt_proc)
{
    ...
    if(pt_this->e_dialog_id != e_dialog_id)
    {
        c_memset(&t_nav_fac_dialog_init, 0, sizeof(NAV_FAC_DIALOG_SHOW_INIT_T));	//Tommy 20191203 Fixed
 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name : a_Foxconn_fac_dialog_register
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
INT32 a_Foxconn_fac_dialog_register(VOID)
{
    NAV_COMP_T      t_comp;
    INT32           i4_ret;
...
VOID Foxconn_custom_rcu_key_pre_handler(
                    NAV_MODEL_T*                pt_this,
                    NAV_EVN_T*                  pt_event
                    )
{
...
BOOL Foxconn_custom_system_key_cb(UINT32     ui4_evt_code)
{
    BOOL               b_process = TRUE;
...
INT32 Foxconn_FACTORY_WIFI_AUTODETECT(VOID)
{
    ...
    Foxconn_fac_set_WIFI_status(FALSE);  // Reset the connect status

    a_cfg_custom_get_odm_index(&ui1_odm_index);

    if(ui1_odm_index != 2){
        DBG_LOG_PRINT(("Foxconn_FACTORY_WIFI_AUTODETECT ODM ID : %d\n", ui1_odm_index));
    ...
    switch (e_nfy_id)
    {
        case NW_NFY_ID_DHCP_SUCCESS_DHCPv4:
        {
            ...
            if(ui1_status == 0xAA)
            {
                ...
                {
                    DBG_LOG_PRINT(("%s Line: %d, Foxconn_FACTORY_WIFI_AUTODETECT\n", __FUNCTION__, __LINE__));
                    Foxconn_FACTORY_WIFI_AUTODETECT();
                }
                else
                {
                    Foxconn_fac_set_WIFI_status(TRUE);
                }
            }
            break;
        }
        ...
        {
            Foxconn_fac_set_WIFI_status(FALSE);
            break;
        }
 ...
 *
 ----------------------------------------------------------------------------*/
VOID Foxconn_fac_set_WIFI_status(BOOL b_connect)
{
    b_Foxconn_wifi_connect_status = b_connect;
}
/*----------------------------------------------------------------------------
 * Name: _auto_detect_notify_callback
 *
 ...
 *
 ----------------------------------------------------------------------------*/
BOOL Foxconn_fac_get_WIFI_status(VOID)
{
    return b_Foxconn_wifi_connect_status;
}

#endif /* end of APP_Foxconn_DFT_WIFI_AUTO_DETECTION */
