...
#include "Foxconn_factory/Foxconn_fac_mlm.h"
...
#ifdef Foxconn_FAC_USB_AUTO_UPDATE  //ke_vizio_20130218
#include "c_os.h"
...
#include "Foxconn_factory/Foxconn_fac_ch_table/Foxconn_fac_load_ch.h"
...
#if	0 //ndef EEP_COLOR_TUNER_VALUE_BEGIN
#include "Foxconn_color_tuner.h"
#include "Foxconn_color_temp.h"
#else
...
#ifdef FAC_PANEL_NITS // Tommy 20191009 Ben 20180629
...
static UINT16 aui2_item_fac_menu[MAX_ELEM_OF_FAC_MENU] = {
...
#ifdef APP_BLUETOOTH_SUPPORT
#ifdef FAC_BLUETOOTH_SUPPORT // Tommy 20191009 Ben 20180126
                                    MLM_FAC_KEY_MSG_BT_MAC,
#endif
#endif
                                    MLM_FAC_KEY_MSG_SN,         //mind 20160812
                                    MLM_FAC_KEY_MSG_ULPK_ID,
#ifdef HDCP_CHECK		//Tommy 20191009
                               ...
                               #endif
                               #ifdef Foxconn_FAC_INSTALL_ULPK
                                    ...
                                    #endif
#endif
                               #ifdef Foxconn_FAC_WINDOW_PATTERN  //ke_vizio_20150522 add
                                    MLM_FAC_KEY_MSG_WINDOW_PATTERN,
                               #endif
#ifdef FAC_BACKLIGHT_CHECK //Add FXC_WT@@2018504 From Ben 20180403
                                    MLM_FAC_KEY_MSG_BACKLIGHT_CHECK,
#endif
#ifdef FAC_WB_DATA   // Tommy 20191009 Ben 20180410
									...
									MLM_FAC_KEY_MSG_AUD_SRS_BYPASS,			//Tommy 20200722
#endif
#ifdef Foxconn_HDMI_CERT						//Tommy 20200722
									MLM_FAC_KEY_MSG_EARC_MUTE,
									MLM_FAC_KEY_MSG_EARC_WORD_LENGTH,
									MLM_FAC_KEY_MSG_EARC_LAYOUT,
#endif
                                    MLM_FAC_KEY_MSG_BACK_LIT_TIME,			//Tommy 20200722
									MLM_FAC_KEY_MSG_CPU_TEMPERATURE,		//Tommy 20200722
#ifdef FAC_DUMP_CH_INFO														//Tommy 20200722
									MLM_FAC_KEY_MSG_DUMP_CH_INFO,
#endif
									MLM_FAC_KEY_MSG_WRITE_HDCP_TEST_KEY,	//Tommy 20200722
									MLM_FAC_KEY_MSG_COPY_QDA_LOG,			//Tommy 20200722
									MLM_FAC_KEY_MSG_COPY_EMBEDDED_DEMO,		//Tommy 20200722
                                    };
...
extern VOID a_cfg_custom_reset_GAMMA_key(VOID);//colby@191220
/*-----------------------------------------------------------------------------
...
#ifdef Foxconn_FAC_INSTALL_ULPK
static INT32 Foxconn_fac_install_security_key(VOID);
#endif

static INT32 Foxconn_get_PQ_AQ_version(UINT32 ui4_model_index,UINT16 ui2_model_group,CHAR * s_format,CHAR * s_ver);
...
#if 0	//ndef EEP_COLOR_TUNER_VALUE_BEGIN
    Foxconn_fac_color_tuner_menu_hide(FALSE);
    Foxconn_fac_color_temp_menu_hide(FALSE);
#else
    Foxconn_fac_color_tuner_menu_hide_v2(FALSE);
    Foxconn_fac_color_temp_menu_hide_v2(FALSE);
#endif
                    ...
                    )
{
    ...
    switch(ui4_key_code) {
	 ...
	 case BTN_PSUDO_AUTO_FAIL:
        ...
        case BTN_EXIT:
        case BTN_RETURN:	//  Tommy 20191009 
            {
                UINT32 ui4_fac_val;
                     ...
                     )
{
	...
	#ifdef FAC_BT_TESTBLE // Tommy 20191009 Ben 20170810
                    ...
                    )
{
    ...
    switch (ui2_idx)
    {
...
#if defined(Foxconn_FAC_EEPROM_VER)
            UINT32  ui4_read;
        ...
        case NAV_FAC_MENU_PQ_VER:  //ke_vizio_20130606 add
        {
            ...
            t_get_info.e_cust_spec_type = DRV_CUSTOM_CUST_SPEC_TYPE_PQ_VER;
            ...
            //Printf("\n[Foxconn_FAC] PQ model idx: %d\n", ui4_model_idx); //ke_test
            //Printf("\n[Foxconn_FAC] PQ group: %d\n", ui2_model_group); //ke_test
            
            c_sprintf(s_ver,"Not Support!!!");
			
			Foxconn_get_PQ_AQ_version(ui4_model_idx, ui2_model_group, "PQ", s_ver);
			//Printf("\n[Foxconn_FAC] PQ ver: %s\n", s_ver);
#endif
        ...
        case NAV_FAC_MENU_AQ_VER:  //ke_vizio_20130606 add
        {
            ...
            t_get_info.e_cust_spec_type = DRV_CUSTOM_CUST_SPEC_TYPE_AQ_VER;
            t_get_info.pv_get_info = (VOID *)&s_ver;
            t_get_info.pz_size = &z_size;
            i4_ret = a_cfg_get_drv_comp(DRV_CUSTOM_OPERATION_TYPE_CUST_SPEC_GET,(VOID *)&t_get_info);
            if(i4_ret != 0)
            ...
            a_cfg_custom_get_model_index(&ui4_model_idx);
            a_cfg_custom_get_model_group(&ui2_model_group);
            //Printf("\n[Foxconn_FAC] AQ model idx: %d\n", ui4_model_idx); //ke_test
            //Printf("\n[Foxconn_FAC] AQ group: %d\n", ui2_model_group); //ke_test
    
            c_sprintf(s_ver,"Not Support!!!");

            Foxconn_get_PQ_AQ_version(ui4_model_idx, ui2_model_group, "AQ", s_ver);
            //Printf("\n[Foxconn_FAC] AQ ver: %s\n", s_ver);
#endif
...
//            UINT32  ui4_write_byte;
        ...
        case NAV_FAC_MENU_ULPK_ID:
			{//Form TPV
				...
				i4_ret = a_uli_get_factory_ulpk_v4(&ui1_ulpk_key_temp[0], &ui4_ulpk_len, &ui4_ulpk_uid_temp);
...
#ifdef HDCP_CHECK		//Tommy 20191009
        case NAV_FAC_HDCP_CHECK: // Tommy 20191009 
        {
        ...
        case NAV_FAC_MENU_AUTO_COLOR:
            ...
            break;
#endif
        case NAV_FAC_MENU_COLOR_TEMP: //Tommy Factory OSD reflash 20191214
#if 1
            if(i4_val == NAV_FAC_MENU_COLOR_TEMP_WARM)
            {
				c_uc_w2s_strcpy(w2s_menu_item_string,
						  MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_WARM));	
            }
            else if (i4_val == NAV_FAC_MENU_COLOR_TEMP_COOL)
            {
				c_uc_w2s_strcpy(w2s_menu_item_string,
						  MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_COOL));	
            }
            else if (i4_val == NAV_FAC_MENU_COLOR_TEMP_Normal)
            {
				c_uc_w2s_strcpy(w2s_menu_item_string,
						  MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_NORMAL));	
            }
			else
			{
				c_uc_w2s_strcpy(w2s_menu_item_string,
						  MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_USER));	
			}
#else
            c_uc_w2s_strcpy(w2s_menu_item_string,
                      MLM_FAC_TEXT(nav_get_mlm_lang_id(), (MLM_FAC_KEY_MSG_COOL + i4_val)));   
#endif
            break;
#if 0	//kyo_20111221
		...
		case NAV_FAC_MENU_TESTBLE:
#endif
#ifdef FAC_WB_DATA  // Tommy 20191009 Ben 20180411
        ...
        case NAV_FAC_MENU_PQ_BYPASS:    // Tommy 20191009 Ben 20180411
#endif
       ...
       case NAV_FAC_MENU_HDMI_PORT_NUM_SETTING:
        ...
        #ifdef Foxconn_FAC_WINDOW_PATTERN  //ke_vizio_20150522
        case NAV_FAC_MENU_WINDOW_PATTERN:
        {
            c_uc_w2s_strcpy(w2s_menu_item_string, 
        ...
        case NAV_FAC_MENU_HDCP_DEF_WRITE:	//Tommy 20200722
		case NAV_COPY_QDA_LOG:				//Tommy 20200722
		case NAV_COPY_EMBEDDED_DEMO:		//Tommy 20200722
		{
				c_uc_w2s_strcpy(w2s_menu_item_string, MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_PRESS));
				break;
		}
    ...
    switch(ui4_msg)
    {
        case WGL_MSG_KEY_DOWN:
            ...
            case BTN_CURSOR_RIGHT:
            {
                ...
                case NAV_FAC_MENU_RESET:
                	{
						CHAR s_cmd[512] = {0};		//Tommy 20210115
						...
						printf("[NAV_FAC_MENU_RESET] command is [%s] \n", s_cmd);
	                    ...
	                    i4_ret =system("sync;sync;sync");		//colby@191220
...
#endif
				case NAV_FAC_MENU_HDCP_DEF_WRITE:
				{
...
//						DBG_LOG_PRINT(("!!!NO USB DEVICE\n"));
					...
					if (i4_ret == FMR_OK)
					{
						c_sprintf(s_command_line,"cp /cache/uli/qda.log %s/",s_mount_point);
						
						i4_ret = system(s_command_line);

						if (i4_ret == NAVR_OK)
						{
							i4_ret = system("sync");
							Printf("Copy qda.log Done \n");
						}
						else
							Printf("Copy qda.log Fail \n");
					}
					else
						Printf("!!!NO qda.log find \n");
...
//						DBG_LOG_PRINT(("!!!NO USB DEVICE\n"));
					...
					c_sprintf(s_filename,"%s/demo/retail_demo/vizio_demo.txt",s_mount_point);
					...
					if (i4_ret == FMR_OK)
					{
						...
						Printf("Remove Old Embedded Video .... \n");
						i4_ret = system("rm -Rf /demo/*");
						c_sprintf(s_filename,"cp -r %s/demo/* /demo",s_mount_point);
//						Printf("Copy Command line : %s \n", s_filename);

						Printf("Copy Embedded Video Start.... \n");
						i4_ret = system(s_filename);
						if (i4_ret != NAVR_OK)
							Printf("Copy Embedded Video Fail..... \n");
						else
							Printf("Copy Embedded Video Done... \n");
							
					}
					else
						Printf("!!!NO Embedded Video find \n");
                ...
                case NAV_FAC_MENU_RESET:
					...
					a_amb_reboot(); // Ben 20160906,   for [TF1016VIZUSMTK03-1]
            ...
            case BTN_SELECT:
                #ifdef FAC_WB_USE_SCE_PATTERN // Denny 2010/01/22 add
                {
                    ...
                    {
#if 0	//ndef EEP_COLOR_TUNER_VALUE_BEGIN
                        Foxconn_fac_color_tuner_menu_show(FALSE);
#else
                        Foxconn_fac_color_tuner_menu_show_v2(FALSE);
#endif
                    ...
                    {
#if 0	//ndef EEP_COLOR_TUNER_VALUE_BEGIN
                        Foxconn_fac_color_temp_menu_show(FALSE);
#else
                        Foxconn_fac_color_temp_menu_show_v2(FALSE);
#endif
                        c_wgl_set_visibility(pt_view->h_fac_main_frame, WGL_SW_HIDE);
                        c_wgl_repaint(pt_view->h_fac_canvas, NULL, TRUE);
                    }
                ...
                #ifdef APP_Foxconn_HDCP_SIF_TRIGGER  //ke_vizio_20170927
                {
                ...
                #ifdef Foxconn_FAC_INSTALL_ULPK
                {
                    ...
                    {
                        i4_ret = Foxconn_fac_install_security_key();
                        if(i4_ret == 0){
                            i4_ret = c_wgl_do_cmd(pt_view->h_fac_list_box,
                                                   WGL_CMD_LB_SET_ITEM_TEXT,
                                                   WGL_PACK_2(ui2_hlt_idx, 2),
...
#if 0	//ndef EEP_COLOR_TUNER_VALUE_BEGIN
        Foxconn_fac_color_tuner_view_init(pt_view->h_fac_canvas, 0, 0);
        Foxconn_fac_color_temp_view_init(pt_view->h_fac_canvas, 0, 0);
#else
        Foxconn_fac_color_tuner_view_init_v2(pt_view->h_fac_canvas, 0, 0);
        Foxconn_fac_color_temp_view_init_v2(pt_view->h_fac_canvas, 0, 0);
#endif
...
#endif /* end Foxconn_FAC_USB_AUTO_UPDATE */
...
/*-----------------------------------------------------------------------------
 * Name : Foxconn_fac_menu_set_focus
 * Description: 
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
INT32 Foxconn_fac_menu_set_focus(UINT16 ui2_element, BOOL b_repaint)
{
    NAV_FAC_VIEW_T* pt_view = &t_g_nav_fac_view;
    INT32       i4_ret = 0;
...
#ifdef Foxconn_FAC_INSTALL_ULPK  //ke_vizio_20171003
/*-----------------------------------------------------------------------------
 * Name : Foxconn_fac_read_file
 * Description: 
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
static INT32 Foxconn_fac_read_file(CHAR* ps_filename_path, UINT8 *ui1_data, UINT32 ui4_size)
{
    INT32     i4_ret = FMR_OK;
    HANDLE_T h_fm_dev;
    UINT32    ui4_r_byte = 0;
...
static INT32 Foxconn_fac_install_security_key(VOID)
{
    ...
    UINT8 ui1_HDCP_1X[320] = {0};		// Tommy 20191009 
    UINT8 ui1_HDCP_2X[880] = {0};		// Tommy 20191009 
#endif
#ifdef MST_HDCP
#else
    BOOL b_valid = FALSE;
    ...
    i4_ret = Foxconn_fac_read_file(s_filename, ui1_ulpk, (sizeof(ui1_ulpk)/sizeof(ui1_ulpk[0])));
    if(i4_ret != FMR_OK){
        return NAVR_FAIL;
    }
    #if 0
    ...
    i4_ret = Foxconn_fac_read_file(s_filename, ui1_potk, (sizeof(ui1_potk)/sizeof(ui1_potk[0])));
    if(i4_ret != FMR_OK){
        return NAVR_FAIL;
    }
#if 0
    ...
    i4_ret = Foxconn_fac_read_file(s_filename, ui1_HDCP_1X, sizeof(ui1_HDCP_1X));
    if(i4_ret != FMR_OK){
        return NAVR_FAIL;
    }

    c_sprintf(s_filename,"%s/KEY/%s",s_mount_point, "HDCP2_X.bin");
    i4_ret = Foxconn_fac_read_file(s_filename, ui1_HDCP_2X, sizeof(ui1_HDCP_2X));
    if(i4_ret != FMR_OK){
        return NAVR_FAIL;
    }
#endif
...
#else	//ifndef MST_HDCP		//Tommy 20191009
...
#if 0	//Tommy 20191216 TPV remove
...
#endif /* end of Foxconn_FAC_INSTALL_ULPK */

//Tommy 20191009 vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
static INT32 Foxconn_get_PQ_AQ_version(UINT32 ui4_model_index, UINT16 ui2_model_group, CHAR *s_format, CHAR *s_ver)
{
	UINT8		ui1_ver[33]			= {0};
	UINT8		ui1_PQ_ver[16]		= {0};
    ...
    if(c_memcmp(s_format, "AQ", sizeof(s_format)) == 0){
#if 1
        i4_ret = c_sprintf((char *)s_filename, "/config/aq/AQ_ver.txt");
#else
        i4_ret = c_sprintf((char *)s_filename, "%s%d%s%d%s", 
					"/vendor/tvconfig/config/foxconn/ModelGroup",
                  (ui2_model_group == 0x304? 304:314),
					"/ModelIndex", 
					ui4_model_index, 
					"/aq/AQ_ver.txt");
#endif                  
        ...
        {
            if(fgets((char *)ui1_ver, sizeof(ui1_ver),pFile)==NULL)
                i4_ret = -1;
    ...
    else if(c_memcmp(s_format, "PQ", sizeof(s_format)) == 0)
    {
        //PQ ver
#if 1
        i4_ret = c_sprintf((char *)s_filename, "/config/pq/PQ_ver.txt");
#else
        i4_ret = c_sprintf((char *)s_filename, "%s%d%s%d%s", 
				"/vendor/tvconfig/config/foxconn/ModelGroup",
				(ui2_model_group == 0x304? 304:314),
        ...
        {
            if(fgets((char *)ui1_PQ_ver, sizeof(ui1_PQ_ver),pFile)==NULL) //colby@191220
                i4_ret = -1;
        ...
        //HSY.bin ver
#if 0	//Tommy Wait for release 1
		{
			UINT8		ui1_idx = 0;	//Tommy Wait for release 1
			ui1_idx = DRV_CUSTOM_PQ_TYPE_HSY;
			a_cfg_cust_drv_get_pq_ver_by_bin_type(ui1_idx, ui1_HSY_ver, sizeof(ui1_HSY_ver));
		}
#else
        i4_ret = c_sprintf((char *)s_filename, "/config/pq/HSY.bin");
        DBG_LOG_PRINT(("HSY ver Path: %s \n", s_filename));
        pFile = fopen(s_filename,"r");
        if(pFile==NULL){
            DBG_LOG_PRINT(("fopen Can't open : %s\n", s_filename));
            i4_ret = -1;
        ...
        {
			UINT8	ui1_Temp[32] = {0};			//Tommy 20200612
			UINT8	i  = 0, j = 0;
            if(fgets((char *)ui1_Temp, sizeof(ui1_Temp),pFile)==NULL) //colby@191220
                i4_ret = -1;
			
			if ((ui1_Temp[0] == 0x76) || (ui1_Temp[0] == 0x56))
				i = 0;
			else
				i = 19;

			for (j = 0; j < 10; j++)
				ui1_HSY_ver[j] = ui1_Temp[j+i];
			
            fclose(pFile);
        }
#endif
		DBG_LOG_PRINT(("HSY ver : %s\n", ui1_HSY_ver));
        i4_ret = c_sprintf((char *)ui1_ver, "GAMMA:%s; HSY:%s", ui1_PQ_ver, ui1_HSY_ver);
    ...
    memcpy(s_ver, ui1_ver, sizeof(ui1_ver));
    return i4_ret;
