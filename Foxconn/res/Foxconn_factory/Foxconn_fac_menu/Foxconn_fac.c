...
#endif
BOOL b_nav_default_FYT_ch_loading = FALSE;  //FXC_Tommy 20180328
BOOL b_nav_default_FBC_ch_loading = FALSE;//Soar Add 20081013 for default channel //FXC_Tommy 20180328
BOOL b_nav_default_NK_ch_loading = FALSE;//Soar Add 20081013 for default channel //FXC_Tommy 20180328

#ifdef FAC_WB_USE_SCE_PATTERN // Denny 2010/01/22 add
BOOL   b_nav_fac_wb_intrnl_pat_on_off = FALSE;
UINT16 ui2_nav_fac_wb_intrnal_pat_val = 768; //colby@200110 range 0 ~ 1023
#endif
...
#ifdef FAC_WB_DATA  // Tommy 20191009 Ben 20180413
BOOL b_nav_fac_wb_data = FALSE;
#endif

#ifdef FAC_BACKLIGHT_CHECK // Add FXC_WT@@2018504 From Ben 20180413 
...
#ifdef PQ_BYPASS
BOOL b_nav_fac_pq_bypass_on_off = FALSE;    // Tommy 20191009 Ben 20180411
#endif
#define Foxconn_DEFAULT_FBC_CH 1  // 1 :FBC ; 0 : FYT //FXC_Tommy 20180328
...
#ifdef Foxconn_FAC_WINDOW_PATTERN //ke_vizio_20150522 add
static CLI_EXEC_T at_Foxconn_fac_cmd_tbl[];
...
static VOID _nav_fac_Foxconn_eep_update(VOID * pv_tag1,VOID * pv_tag2,VOID * pv_tag3);
#endif
/*-----------------------------------------------------------------------------
...
#else
			if (ui2_idx == 0) // bypass = off
				i4_ret = acfg_cust_audio_enable_surround_mode(acfg_cust_audio_is_surround_mode_enable());
			if (ui2_idx == 1) // bypass = on
				i4_ret = acfg_cust_audio_enable_surround_mode(SURROUND_SOUND_BYPASS);
#endif
...
#ifdef PQ_BYPASS
    if (b_nav_fac_pq_bypass_on_off == TRUE)  // Tommy 20191009 Ben 20180417
    {
        a_cfg_custom_Set_Factory_PQbypass_OnOff(TRUE);
    }
#endif    
#ifdef FAC_BACKLIGHT_CHECK // Add FXC_WT@@2018504 From Ben 20180413
...
#if 0	//def FAC_WB_DATA  // Tommy 20191009 Ben 20180411
...
#if 0	//def FAC_PANEL_NITS // Tommy 20191009 Ben 20180629
                    ...
                    )
{
    ...
    {  
        ...
        {
            #ifdef APP_Foxconn_DFT_DC_ON_RESET
            UINT8 ui1_mode = 0;
...
BOOL _nav_fac_check_key_stack(VOID)
{
    ...
    c_uc_ps_to_w2s(Foxconn_FAC_DEFAULT_CH, w2s_pwd_slave, NAV_FAC_KEY_STACK_NUM);
    c_uc_ps_to_w2s(Foxconn_FAC_DEFAULT_CH_POL, w2s_pwd_third, NAV_FAC_KEY_STACK_NUM);
...
// Mill 20090312 Erase Partition 5 END
    ...
    /* Master password is always available in case of forgetting the password. The master password is 1150.*/
    UINT32 ui4_key_pwd1[NAV_FAC_KEY_STACK_NUM] = Foxconn_FAC_DEFAULT_CH;
    UINT32 ui4_key_pwd2[NAV_FAC_KEY_STACK_NUM] = Foxconn_FAC_DEFAULT_CH_POL;
   
    for(ui1_idx = 0; ui1_idx < (NAV_FAC_KEY_STACK_NUM); ui1_idx++)
    {
        if(pt_fac->at_key_stack[ui1_idx] != ui4_key_pwd1[ui1_idx])
        {
#ifdef Foxconn_DEFAULT_FBC_CH  //FXC_Tommy 20180328
        	b_nav_default_FBC_ch_loading = FALSE;
#else
            b_nav_default_FYT_ch_loading = FALSE;
#endif
    ...
    for(ui1_idx = 0; ui1_idx < (NAV_FAC_KEY_STACK_NUM); ui1_idx++)
    {
        ...
        {
            b_nav_default_NK_ch_loading = FALSE;  //FXC_Tommy 20180328
        	
            break;
        }
        b_nav_default_NK_ch_loading = TRUE;    //FXC_Tommy 20180328
    }

    #ifdef Foxconn_DEFAULT_FBC_CH   //FXC_Tommy 20180328
    if(b_nav_default_FBC_ch_loading == TRUE ||  b_nav_default_NK_ch_loading == TRUE)
    #else
    if(b_nav_default_FYT_ch_loading == TRUE ||  b_nav_default_NK_ch_loading == TRUE)
    #endif
    {
        return TRUE;
    }
...
BOOL _nav_fac_check_key_stack2(VOID)
{
    ...
    /* Master password is always available in case of forgetting the password. The master password is 1150.*/
    UINT32 ui4_key_pwd[NAV_FAC_KEY_STACK2_NUM] = Foxconn_FAC_PASSWORD_DEF;
...
BOOL _nav_fac_check_key_stack3(VOID)
{
    ...
    UINT32 ui4_key_pwd[NAV_FAC_KEY_STACK3_NUM] = Foxconn_FAC_PASSWORD_DEF_EEPROM;
    ...
    if (pt_fac->ui2_key_num == NAV_FAC_KEY_STACK_NUM)
    {
        if (_nav_fac_check_key_stack())
        {
            if ((c_strcmp (s_active_app, MENU_NAME) == 0))
            {
                ...
                a_amb_pause_app(MENU_NAME);
                pt_fac->ui2_key_num = 0;
//FXC_Tommy 20180328 vvvv
                if(b_nav_default_FYT_ch_loading == TRUE)
                {
                    acfg_Foxconn_load_default_channel("PERSET_CHANNEL_FYT");
                    b_nav_default_FYT_ch_loading = FALSE;
                }
                else if(b_nav_default_FBC_ch_loading == TRUE)
                {
                    acfg_Foxconn_load_default_channel("PERSET_CHANNEL_FBC");
                    b_nav_default_FBC_ch_loading = FALSE;
                }
                else if(b_nav_default_NK_ch_loading == TRUE)
                {
                    acfg_Foxconn_load_default_channel("PERSET_CHANNEL_NK");
                    b_nav_default_NK_ch_loading = FALSE;
                }
//FXC_Tommy 20180328 ^^^^
            }
        }
                    ...
                    INT16*             pui2_max)
{
    ...
    switch (ui2_idx)
    {
...
#endif  
#ifdef PQ_BYPASS
        case NAV_FAC_MENU_PQ_BYPASS:	//Tommy 20191009 
#endif        
        ...
        case NAV_FAC_MENU_TUNER_TYPE:
            i2_min = 0;
            i2_max = 4;//1;	//sam 120606 add tuner index
            break;
#endif
...
//@jh            INT32   i4_ret = 0;
            ...
            i2_min = 1;	//Tommy 20191009 
            //i2_max = sizeof(_ar_Fac_Panel_ID_Map)/sizeof(PANEL_ID_T) -1;
            i2_max= (INT16)i4_max;	//Tommy 20191009 
    ...
    INT32*              pi4_val)
{
    ...
    {   
		...
		case NAV_FAC_MENU_TUNER_TYPE:
		{
			i4_ret = a_cfg_cust_fac_get_tuner_type(&i2_val);
			if (i4_ret != APP_CFGR_OK)
				return TVR_FAIL;
			break;
		}
        ...
        case NAV_FAC_MENU_PANEL_TYPE:
#if 1	// Tommy 20191009 
            a_cfg_custom_get_model_index((UINT32 *)&i2_val);
#else
        ...
        case NAV_FAC_MENU_COLOR_TEMP:	//Tommy20191204
            i4_ret = a_cfg_av_get(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_CLR_TEMP), &i2_val);

			Printf("Tommy NAV_FAC_MENU_COLOR_TEMP Get = %d \n", i2_val);
            switch (i2_val)
            {
                case ACFG_CUST_CLR_TEMP_WARM:
                {
                    i2_val = NAV_FAC_MENU_COLOR_TEMP_WARM;
                }
                break;

                case ACFG_CUST_CLR_TEMP_COOL:
                {
                    i2_val = NAV_FAC_MENU_COLOR_TEMP_COOL;
                }
                break;

                case ACFG_CUST_CLR_TEMP_NORMAL:
                {
                    i2_val = NAV_FAC_MENU_COLOR_TEMP_Normal;
                }
                break;

//				case ACFG_CUST_CLR_TEMP_CUSTOM:
//				{
//				    i2_val = NAV_FAC_MENU_COLOR_TEMP_USER;
//				}
//				break;
                default:
                    i2_val = NAV_FAC_MENU_COLOR_TEMP_WARM;
        ...
        case NAV_FAC_MENU_FLIP:
        {
            ...
            nav_fac_c_rm_get_vid_comp((DRV_CUSTOM_VID_TYPE_T)(DRV_CUSTOM_OPERATION_TYPE_FLIP_ON),
                                          e_fac_vid_inp,(VOID *)&ui1_temp, &z_size);
            i2_val = (INT16)ui1_temp;
            break;
        }
        case NAV_FAC_MENU_MIRROR:
        {
            ...
            SIZE_T      z_size = sizeof(ui1_temp);
            e_fac_vid_inp = DTV_CUSTOM_VID_INP_FLAG_NONE;
            nav_fac_c_rm_get_vid_comp((DRV_CUSTOM_VID_TYPE_T)(DRV_CUSTOM_OPERATION_TYPE_MIRROR_ON),
                                          e_fac_vid_inp, (VOID *)&ui1_temp, &z_size);
            i2_val = (INT16)ui1_temp;
            break;
        }
        ...
        case NAV_FAC_MENU_FAC_WB_DATA:
        {
            #ifdef Foxconn_2K19_GAMMA //colby@200115 switch gamma sub-item flag by eeprom         
            a_cfg_fac_data_eeprom_get_fac_gamma(&b_nav_fac_wb_data);                      
            i2_val = b_nav_fac_wb_data;
            //DBG_LOG_PRINT(("\n ------------ get NAV_FAC_MENU_FAC_WB_DATA=%d-------------\n",i2_val));            
            #else /* switch panel.ini or eeprom gamma */
...
DBG_LOG_PRINT(("!!!!@djh %s,  NAV_FAC_MENU_PANEL_TYPE:\n", __func__));            
            ...
            //Re-write Fac part. Refer acfg_Foxconn_fac_panel.c.
        ...
        #if 0   // Tommy 20191009 Ben 20190904
            i4_ret = a_cfg_cust_fac_set_panel_type(i4_val);
            if (i4_ret != APP_CFGR_OK){
                DBG_LOG_PRINT(("!!!!%s : a_cfg_cust_fac_set_panel_type fail\n", __func__));
                return i4_ret;
        ...
        #endif
#if 0	//Foxconn Tommy 201800607 Init color temp
        ...
        case NAV_FAC_MENU_COLOR_TEMP:
        {
			INT16 i2_gain = 0;  //Foxconn Tommy 20180409
        	...
        	Printf("Tommy NAV_FAC_MENU_COLOR_TEMP Set = %d \n",i4_val);

  			//ke_vizio_20140909 add
            switch (i4_val)	
            {
                case NAV_FAC_MENU_COLOR_TEMP_WARM:
                {
                    i4_val = ACFG_CUST_CLR_TEMP_WARM;	//Tommy 20191204
                }
                break;
                case NAV_FAC_MENU_COLOR_TEMP_COOL:
                {
                    i4_val = ACFG_CUST_CLR_TEMP_COOL;	//Tommy 20191204
                }
                break;
                case NAV_FAC_MENU_COLOR_TEMP_Normal:
                {
                    i4_val = ACFG_CUST_CLR_TEMP_NORMAL;	//Tommy 20191204
                }
                break;
//				case NAV_FAC_MENU_COLOR_TEMP_USER:
//				{
//					i4_val = ACFG_CUST_CLR_TEMP_CUSTOM;
//				}
//				break;
                default:
                    i4_val = NAV_FAC_MENU_COLOR_TEMP_WARM;
                    break;
            }
//            i4_ret = a_cfg_av_set(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_CLR_TEMP+(ui2_idx-NAV_FAC_MENU_COLOR_TEMP)), i4_val);
			i4_ret = a_cfg_av_set(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_CLR_TEMP), i4_val);   // NT@FXC, 20130408, 1->0
...
#endif        
#ifdef FAC_WB_DATA  // Tommy 20191009 Ben 20180411
        case NAV_FAC_MENU_FAC_WB_DATA: //colby@200115
        {
            #ifdef Foxconn_2K19_GAMMA //colby@200115 switch gamma sub-item flag by eeprom
            b_nav_fac_wb_data = i4_val;
            a_cfg_fac_data_eeprom_set_fac_gamma(i4_val);
            a_cfg_gamma_enable_warm(b_nav_fac_wb_data);           
            #else
            b_nav_fac_wb_data = i4_val;
...
#ifdef PQ_BYPASS  // Tommy 20191009 Ben 20180411
       case NAV_FAC_MENU_PQ_BYPASS:    // Tommy 20191009 Ben 20180411
        {
            b_nav_fac_pq_bypass_on_off = i4_val;
            a_cfg_custom_Set_Factory_PQbypass_OnOff(i4_val);
            break;
        }
#endif
...
INT32 nav_fac_set_flip_mirror_enable(BOOL b_filp_enable, BOOL b_mirror_enable)
{
    ...
    i4_ret = nav_fac_c_rm_set_vid_comp(DRV_CUSTOM_OPERATION_TYPE_FLIP_ON,(VOID *)&ui1_val, sizeof(ui1_val), TRUE);
    if(i4_ret != 0)
    {
        return i4_ret;
    }
    ...
    i4_ret = nav_fac_c_rm_set_vid_comp(DRV_CUSTOM_OPERATION_TYPE_MIRROR_ON,(VOID *)&ui1_val, sizeof(ui1_val), TRUE);
    if(i4_ret != 0)
    {
        return i4_ret;
    }
...
INT32 nav_get_cur_model_info(VIZIO_MOEDL_INFO_T *pvt_info)
{
    ...
    #if 0 //defined(FAC_MENU_MODEL_NAME_SELECT) && defined(APP_READ_MODEL_NAME_FROM_EEPROM)
    if(pvt_info == NULL)
    {
        return NAVR_INV_ARG;
    }
    ...
    /*
        { MODEL_ID_E551VA,        "XXXX",     55},  // 0

      */
    UINT16 ui2_id = 0;
    nav_fac_model_name_get_current_model_name_id(&ui2_id);

    UINT16 ui2_id_by_panel = 0;
    nav_fac_get_current_model_id_by_panel(&ui2_id_by_panel);


    if(ui2_id_by_panel != ui2_id)
    {
        /* Wirte the right panel model */
        nav_fac_model_name_write_default_model_name();
    }

    switch(ui2_id_by_panel)
    {
        case MODEL_ID_E551VA:
        {
            pvt_info->e_model_series = VIZIO_E_SERIES;
            pvt_info->e_RC_type = RC_XRV1TV;
            pvt_info->e_hdmi_port = IO_table_0;	
            pvt_info->b_3D_support = TRUE;
            pvt_info->b_flip_mirror = TRUE;
            pvt_info->e_tuner = UNKNOWN_TUNER;
            break;
        }
        ...
        default:
        {
            ...
            pvt_info->e_RC_type = RC_XRV1TV;
            pvt_info->e_hdmi_port = IO_table_0;
            pvt_info->b_3D_support = FALSE;
            pvt_info->b_flip_mirror = FALSE;
            pvt_info->e_tuner = UNKNOWN_TUNER;
            break;
        }
    }
    ...
    if(c_strncmp(s_data_r, s_model_name,16)!= 0)
    {
        /* Wirte the right panel model */
        ...
        i4_ret = a_cfg_eep_raw_write(EEP_MODEL_NAME,
                                 ...
                                 &ui4_write_byte);
        if(i4_ret != 0){
            DBG_LOG_PRINT(("\n!!!! %s : Line : %d : raw_write name fail\n", __FUNCTION__, __LINE__));
        }
    }

    UINT16 ui2_id_by_panel_id = 0;
    ui2_id_by_panel_id = a_cfg_custom_get_cur_panelID();
    
    switch(ui2_id_by_panel_id)
    {
        case PANEL_AUO_42_HW1_V0:
        {
            pvt_info->e_model_series = VIZIO_E_SERIES;
            pvt_info->e_RC_type = RC_XRV1TV;
            pvt_info->e_hdmi_port = IO_table_0;	
...
#ifdef Foxconn_FAC_USB_AUTO_UPDATE  //ke_vizio_20141107 modify
BOOL b_auto_EEPROM_dump_all = FALSE;
/*-----------------------------------------------------------------------------
* Name:
*      _nav_fac_Foxconn_eep_update
* Description:
*
* Input arguments:
*      pv_tag
 ...
 *      None
 *---------------------------------------------------------------------------*/
static VOID _nav_fac_Foxconn_eep_update(
                    VOID*               pv_tag1,
                    VOID*               pv_tag2,
                    VOID*               pv_tag3)
{
...
INT32 nav_fac_border_window(BOOL b_onOff, UINT32 u4L_width, UINT32 u4R_width, UINT32 u4U_height, UINT32 u4D_height)
{
    ...
    /* SCE pattern */
    nav_fac_sce_pat_set_level(1023); //colby@200115
