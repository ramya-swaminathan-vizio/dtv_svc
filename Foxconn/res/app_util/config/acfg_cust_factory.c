...
//#ifdef APP_Foxconn_FAC
    i4_ret = acfg_custom_Foxconn_fac_pre_init();   
...
INT32 acfg_cust_factory_post_init(VOID)
{
    INT32	i4_ret;
//#ifdef APP_Foxconn_FAC
	UINT8	eep_buf[EEP_FACTORY_DATA_MAX -1] = {0};
	UINT8	eep_buf_Len = EEP_FACTORY_DATA_MAX;
	
	i4_ret = a_cfg_custom_EEPROM_load(eep_buf, &eep_buf_Len);
    ACFG_LOG_ON_FAIL(i4_ret);  

    i4_ret = acfg_custom_Foxconn_fac_post_init(); 
    ACFG_LOG_ON_FAIL(i4_ret);  
//#endif
    ...
    switch(ui2_cfg_grp)
    {
        case APP_CFG_GRPID_FACTORY:
        {
            ...
            switch (i2_clr_temp)
            {
                case 0:
                {
                    i2_clr_temp = ACFG_CUST_CLR_TEMP_WARM;	//ACFG_CUST_CLR_TEMP_COOL;
                }
                break;
                case 1:
                {
                    i2_clr_temp = ACFG_CUST_CLR_TEMP_COOL;	//ACFG_CUST_CLR_TEMP_NORMAL;
                }
                break;
                case 2:
                {
                    i2_clr_temp = ACFG_CUST_CLR_TEMP_NORMAL;	//ACFG_CUST_CLR_TEMP_COMPUTER;
                }
                break;
//Tommy 20210129 
//				case 3:
...
//				break;
                default:
				{
					i2_clr_temp = ACFG_CUST_CLR_TEMP_WARM;	//Tommy 20210129 
            ...
            switch(ui2_cfg_rec)
            {
                ...
                case APP_CFG_RECID_VID_FAC_GAIN_B:
                {
                    ...
                    else
                    {
                        ...
                        ui4_offset = ui4_dynamic_address + ui1_Foxconn_color_eep_offset[i2_clr_temp][ui1_play_content] * COLOR_TUNER_FAC_PIECE_NUM + i1_idx;
...
//                    INT8 i1_idx = 0;
                    ...
                    ui4_offset = ui4_dynamic_address + ui1_Foxconn_color_eep_offset[i2_clr_temp][ui1_play_content] * COLOR_TUNER_FAC_PIECE_NUM + ui4_color_tuner_idx;
                ...
                case APP_CFG_RECID_CUST_FAC_11_POINT_BLUE:
                {
                    ...
                    for(ui1_idx = 0; ui1_idx < COLOR_TUNER_FAC_20POINT_NUM; ui1_idx++)
                    {
                        ...
                        //Printf("\033[01;31m [Foxconn_FAC] \033[m i2_array[%d]: %d\n", ui1_idx, i2_array[ui1_idx]);
                    }
...
INT32	a_cfg_custom_reset_factory_colortemp(VOID)
{
	...
	for (ui1_input=0; ui1_input<ACFG_CUST_INPUT_GRP_MAX; ui1_input++)
    {
    	...
    	{
    		acfg_get_int16_by_idx(IDX_CLR_FAC_GAIN_R, (UINT16)ui1_clr_temp, &i2_value);
		...
		for(ui1_clr_temp=0; ui1_clr_temp<(UINT8)ACFG_CLR_TEMP_NUM; ui1_clr_temp++)
    	{    	
        	acfg_get_int16_by_idx(IDX_CLR_FAC_GAIN_G, (UINT16)ui1_clr_temp, &i2_value);
		...
		for(ui1_clr_temp=0; ui1_clr_temp<(UINT8)ACFG_CLR_TEMP_NUM; ui1_clr_temp++)
    	{
        	acfg_get_int16_by_idx(IDX_CLR_FAC_GAIN_B, (UINT16)ui1_clr_temp, &i2_value);
		...
		for(ui1_clr_temp=0; ui1_clr_temp<(UINT8)ACFG_CLR_TEMP_NUM; ui1_clr_temp++)
    	{
        	acfg_get_int16_by_idx(IDX_CLR_FAC_OFFSET_R, (UINT16)ui1_clr_temp, &i2_value);
		...
		for(ui1_clr_temp=0; ui1_clr_temp<(UINT8)ACFG_CLR_TEMP_NUM; ui1_clr_temp++)
    	{
        	acfg_get_int16_by_idx(IDX_CLR_FAC_OFFSET_G, (UINT16)ui1_clr_temp, &i2_value);
		...
		for(ui1_clr_temp=0; ui1_clr_temp<(UINT8)ACFG_CLR_TEMP_NUM; ui1_clr_temp++)
    	{
        	acfg_get_int16_by_idx(IDX_CLR_FAC_OFFSET_B, (UINT16)ui1_clr_temp, &i2_value);
...
/*-----------------------------------------------------------------------------
 * Name: a_cfg_cust_get_cpu_tempature_info
 * Description:
 * Inputs:
 * Outputs:
 * Returns:
 *-----------------------------------------------------------------------------*/
INT32 a_cfg_cust_get_cpu_tempature(INT16 *i2_temp)
{
    FILE*   pf_out = NULL;
    CHAR    buf[512] = {0};
    INT16   i2_num = 0;
    ...
    if((pf_out = popen("cat /proc/mstar_dvfs/temperature_info | grep -r \"CPU_0_temp:\" | awk -F':' '{print $2}'","r"))!= NULL)
    {
        while (fgets(buf, 255, pf_out) != NULL)
        DBG_LOG_PRINT(("[TPV_FAC]%s", buf));
...
INT32 a_cfg_custom_get_fac_reserv_FFA (UINT8*  pui1_val, SIZE_T z_size)
{
	UINT32 ui4_read;
	INT32  i4_ret;
	
    
    i4_ret = a_cfg_eep_raw_read(EEP_OFFSET_FAC_FFA, pui1_val, z_size, &ui4_read);
...
INT32 a_cfg_custom_set_fac_reserv_FFA (UINT8*  pui1_val, SIZE_T z_size)
{
	UINT32 ui4_written;
	INT32  i4_ret;
	
	 i4_ret =  a_cfg_eep_raw_write(EEP_OFFSET_FAC_FFA, pui1_val, z_size, &ui4_written);
