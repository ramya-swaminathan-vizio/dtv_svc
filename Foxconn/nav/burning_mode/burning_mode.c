...
#endif
// yuan 2012 0606 
#define BURNING_COUNTER_TIMER_DELAY      (3000)  /* in ms */ 
/*-----------------------------------------------------------------------------
...
// yuan 2012 0606 vvvvv
static UINT32 ui4_g_aging_time = 0;     // yuan 2012 0606
static UINT32 ui4_g_aging_time_total = 0;
static UINT32 ui4_g_time_base = 0; 
// yuan 2012 0606 ^^^^^
/*-----------------------------------------------------------------------------
...
// yuan 2012 0606 vvvvv
#if 1   //nicholas 
static VOID _burning_mode_counter_nfy(
                    HANDLE_T                    h_timer,
                    VOID*                       pv_tag
                    );
static VOID _burning_mode_counter_hdlr(
                    VOID*                       pv_tag,
                    VOID*                       pv_tag2,
                    VOID*                       pv_tag3
                    );
#endif    
static VOID _burning_mode_stop_counter(
                    BURNING_MODE_T*              pt_this
                    );
// static INT32 _burning_mode_start_counter(BURNING_MODE_T* pt_this);   nicholas @ 2012 1220

extern INT32 a_cfg_custom_get_total_aging_time (UINT32*  pui4_aging_time);
extern INT32 a_cfg_custom_set_total_aging_time (UINT32   ui4_aging_time);
extern INT32 a_cfg_custom_get_fac_aging_mode (UINT16*  pui2_val, SIZE_T z_size);
// yuan 2012 0606 ^^^^^
...
UINT32  a_burning_mode_get_aging_time(VOID)
{
    return ui4_g_aging_time;
}

VOID  a_burning_mode_set_aging_time(UINT32 ui4_time)
{
    ui4_g_aging_time = ui4_time;
}

UINT32  a_burning_mode_get_aging_time_total(VOID)
{
    return ui4_g_aging_time_total;
}

VOID a_burnning_mode_clear_aging_time(VOID)
{
    ui4_g_aging_time = 0;
    a_cfg_custom_set_total_aging_time( 0 );
    ui4_g_aging_time_total = 0;
}

INT32 a_burning_mode_stop_timer(VOID)
{
    BURNING_MODE_T*    pt_this = & t_g_burning_mode;

     _burning_mode_stop_counter(pt_this);
     return 1; 
}
...
// yuan 2012 0606 vvvvv
    pt_this->h_timer_counter  = NULL_HANDLE;
    pt_this->b_counter_on     = FALSE;

    a_cfg_custom_get_total_aging_time( &ui4_g_aging_time_total );
// yuan 2012 0606 ^^^^^    
...
// yuan 2012 0606 vvvvv
        i4_ret = c_timer_create (& pt_this->h_timer_counter);
...
// yuan 2012 0606 vvvvv
    if (c_handle_valid (pt_this->h_timer_counter) == TRUE)
    {
        c_timer_delete (pt_this->h_timer_counter);
    }
    pt_this->h_timer_counter     = NULL_HANDLE;
// yuan 2012 0606 ^^^^^
                    ...
                    )
{
    ...
    {    
        /* Start counter */
        i4_ret = c_timer_start (pt_this->h_timer_counter,
                            BURNING_COUNTER_TIMER_DELAY,
                            X_TIMER_FLAG_REPEAT,
                            _burning_mode_counter_nfy,
                            pt_this);
//        BURNING_CHK_FAIL (i4_ret, i4_ret);
        pt_this->b_counter_on = TRUE;  
    }
    // yuan 2012 0606 ^^^^^
                    ...
                    )
{
    ...
    if (c_handle_valid (pt_this->h_timer_counter) == FALSE)
    {
        return;
    }
...
{
    UINT32             ui4_time_gap = 0;
    
    ui4_time_gap = ((c_os_get_sys_tick() * c_os_get_sys_tick_period())/1000 - ui4_g_time_base);
    ui4_g_aging_time += ui4_time_gap;
    ui4_g_aging_time_total += ui4_time_gap;
    ui4_g_time_base = c_os_get_sys_tick() * c_os_get_sys_tick_period()/1000;

    a_cfg_custom_set_total_aging_time( ui4_g_aging_time_total );    
}
static VOID _burning_mode_counter_nfy(
                    HANDLE_T                    h_timer,
                    VOID*                       pv_tag
...
{
    /* Executed in timer's thread context. */
    nav_request_context_switch (_burning_mode_counter_hdlr, (VOID*) h_timer, NULL, NULL);    
}
#endif
