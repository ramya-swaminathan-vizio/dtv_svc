...
static WZD_ANIM_COND_CB_DATA_T at_anim_cb_get_focus_data;
static WZD_ANIM_COND_CB_DATA_T at_anim_cb_lose_focus_data;
static BOOL ab_anim_end[WZD_SEL_BOX_BTN_COUNT] = {0};
...
static INT32 _wzd_view_sel_box_frm_move_by_direction(
                                WZD_SEL_BOX_FRM_MOVE_T e_direction,
                                BOOL b_anim);
...
static INT32 _wzd_view_set_frm_bg_img(
    HANDLE_T            h_widget,
                    ...
                    )
{
    ...
    case WGL_MSG_KEY_DOWN:
        ...
        DBG_INFO(("[WZD][FOOTER] --- KEY value: %d  @LINE: %d.\n", (UINT32)pv_param1, __LINE__));
...
/*-----------------------------------------------------------------------------
 * Name: wzd_view_main_frm_show_bg_only
 *
 * Description: The function shows the main frame background image only.
 *
 * Inputs:  -
 *
 * Outputs: -
 ...
 *          Any other values       Failed.
 ----------------------------------------------------------------------------*/
INT32 wzd_view_main_frm_show_bg_only (VOID)
{
    c_wgl_set_visibility (t_g_view.h_main_frm, WGL_SW_HIDE_RECURSIVE);
    c_wgl_set_visibility (t_g_view.h_main_frm, WGL_SW_NORMAL);

    c_wgl_repaint (t_g_view.h_main_frm, NULL, TRUE);
...
INT32 wzd_view_reset_base_frm_rect (VOID)
{
    GL_RECT_T           t_rect;
    ...
    c_wgl_move(t_g_view.h_base_frm, &t_rect, WGL_NO_AUTO_REPAINT);
...
INT32 wzd_view_init (VOID)
{
    ...
    /* reset the struct variable of WZD_ANIM_COND_CB_DATA_T */
    c_memset(&at_anim_cb_get_focus_data, 0, sizeof(WZD_ANIM_COND_CB_DATA_T));
    c_memset(&at_anim_cb_lose_focus_data, 0, sizeof(WZD_ANIM_COND_CB_DATA_T));
...
INT32 _wzd_dlg_update_btn_navigation (WZD_DLG_CONTROL_T*  pt_dlg_ctrl)
{
    ...
    if( pt_dlg_ctrl->ui4_style_mask & WZD_DLG_STYLE_MASK_2_BTN )
    {
        ...
        if( WZDR_OK != i4_ret )
        {
            DBG_INFO(( "[WZD][DLG] WGL_CMD_GL_SET_IMAGE fail, i4_ret =%d\n", i4_ret ));
            return i4_ret;
        }
                                ...
                                VOID*       pv_param2)
{
    ...
    if (ui4_msg == WGL_MSG_NOTIFY)
    {
        if (pt_wgl_nfy_data->ui4_nc_code == WGL_NC_GL_GET_FOCUS) /* some widget gets focus */
        {
            ...
            if( WZDR_OK != i4_ret )
            {
                DBG_INFO(( "[WZD][DLG][_wzd_dlg_frm_nfy] _wzd_dlg_get_attach_data() fail, i4_ret =%d\n", i4_ret ));
                return i4_ret;
            }

            if( NULL == pt_dlg_ctrl )
            {
                DBG_INFO(( "[WZD][DLG][_wzd_dlg_frm_nfy] pt_dlg_ctrl = NULL\n" ));
                return i4_ret;
            }
                    ...
                    BOOL                b_repaint)
{
    ...
    {
        DBG_INFO(( "[WZD][DLG][Show] invalid button index = %d\n", (UINT32)e_btn_index ));
        return WZDR_FAIL;
    }
...
/*----------------------------------------------------------------------------
 * Name: _wzd_view_set_frm_bg_img
 *
 * Description: This API set widget background image
 *
 * Inputs:  -
 *
 * Outputs: -
...
/*-----------------------------------------------------------------------------
 * Name: _wzd_view_all_widget_anim_finish
 * Description: This API get all widget anim is finish
 * Inputs: -
 * Outputs: -
 * Returns: TRUE:               All widget anim is finish.
 *          Any other values    Widget anim is NOT finish.
 ----------------------------------------------------------------------------*/ 
static BOOL _wzd_view_all_widget_anim_finish (VOID)
{
    /* check get focus widget and lose focus widget */
    if ((at_anim_cb_get_focus_data.b_used) || (at_anim_cb_lose_focus_data.b_used))
    {
        return FALSE;
    }
    else
...
/*-----------------------------------------------------------------------------
 * Name: _wzd_view_scale_cond_cb_handler
 * Description: 
 * Inputs: -
 * Outputs: -
 * Returns: -
 ----------------------------------------------------------------------------*/ 
static VOID _wzd_view_scale_cond_cb_handler(
    VOID*               pv_data,
    SIZE_T              z_data_size)
{
    GL_RECT_T           		t_rect = {0};
    HANDLE_T 					h_sb_icon_1st = NULL_HANDLE;
    WZD_ANIM_COND_CB_DATA_T*    pt_anim_cb_data = NULL;
    ...
    do
    {
        ...
        if ((NULL == pv_data) ||
            (z_data_size != sizeof(WZD_ANIM_COND_CB_DATA_T)))
        {
            break;
        }

        pt_anim_cb_data = (WZD_ANIM_COND_CB_DATA_T*)pv_data;
        ...
        DBG_INFO(("[WZD]ANIM end:0x%x, param:%d, index:%d\n",
                    pt_anim_cb_data->h_anim_handle, 
                    param, 
                    pt_anim_cb_data->ui4_index));

        /* Stop animation */
        if (param == WZD_CALLBACK_PARAM_SCALE_DOWN)
        {
            _wzd_view_set_frm_bg_img(pt_anim_cb_data->h_anim_handle, h_g_wzd_img_select_box);

            /* send lose focus msg */
            if (NULL != t_g_view.t_sb_info.pf_data_chg_nfy)
            {
                t_g_view.t_sb_info.pf_data_chg_nfy(
                            WZD_SEL_BOX_MSG_ITEM_LOSE_FOCUSED, 
                            (VOID*)(pt_anim_cb_data->ui4_index),
                            NULL);
            }
        ...
        {
            wzd_view_sel_box_set_order(pt_anim_cb_data->h_anim_handle);

            c_wgl_set_alpha(pt_anim_cb_data->h_anim_handle, 255);

            /* update position */
            if (t_g_view.t_sb_info.b_anim_affect_child)
            {
				h_sb_icon_1st = wzd_view_sel_box_get_icon_1_handle(pt_anim_cb_data->ui4_index);
        		SET_RECT_BY_SIZE(&t_rect,
        						(340 * WZD_UI_RATIO),
        						(200 * WZD_UI_RATIO),
        						WZD_SEL_BOX_BTN_ICON_1_W,
        						WZD_SEL_BOX_BTN_ICON_1_H);
        		c_wgl_move(h_sb_icon_1st,
        						&t_rect,
        						WGL_NO_AUTO_REPAINT);
            }

            _wzd_view_set_frm_bg_img(pt_anim_cb_data->h_anim_handle, h_g_wzd_img_select_box_highlight); 
        }
        else if (WZD_CALLBACK_PARAM_LEFT == param)
        {
        ...
        else if (WZD_CALLBACK_PARAM_RIGHT == param)
        {
        ...
        /* clean data */
        if (pt_anim_cb_data->h_anim_handle == at_anim_cb_get_focus_data.h_anim_handle)
        {
            at_anim_cb_get_focus_data.h_anim_handle = NULL_HANDLE;
            at_anim_cb_get_focus_data.e_param = WZD_CALLBACK_PARAM_NOTHING;
            at_anim_cb_get_focus_data.ui4_index = 0;
            at_anim_cb_get_focus_data.b_used = FALSE;
        }
        else if(pt_anim_cb_data->h_anim_handle == at_anim_cb_lose_focus_data.h_anim_handle)
        {
            at_anim_cb_lose_focus_data.h_anim_handle = NULL_HANDLE;
            at_anim_cb_lose_focus_data.e_param = WZD_CALLBACK_PARAM_NOTHING;
            at_anim_cb_lose_focus_data.ui4_index = 0;
            at_anim_cb_lose_focus_data.b_used = FALSE;
        }
...
/*-----------------------------------------------------------------------------
 * Name: _wzd_view_scale_cb_fct
 * Description: scale anim callback function
 * Inputs: -
 * Outputs: -
 * Returns: 0                   Successful.
 *          Any other values    Failed.
 ----------------------------------------------------------------------------*/ 
static INT32 _wzd_view_scale_cb_fct(
    HANDLE_T            h_widget,
...
{
    #if 0
    DBG_INFO(("[WZD]ui2_anim_type=%d, ui2_end_type=%d\n", ui2_anim_type,
                t_anim_cond_inf.ui2_end_type));
    #endif
    ...
    {
        DBG_INFO(("[WZD][SB]ANIM_TOTAL_END Line@%d.\n", __LINE__));

        /* Async invoke callback handler function */        
        wzd_async_invoke(
                _wzd_view_scale_cond_cb_handler, 
                pv_tag, 
                sizeof(WZD_ANIM_COND_CB_DATA_T), 
                FALSE);
    }
...
/*-----------------------------------------------------------------------------
 * Name: _wzd_view_move_cond_cb_handler
 * Description: 
 * Inputs: -
 * Outputs: -
 * Returns: -
 ----------------------------------------------------------------------------*/ 
static VOID _wzd_view_move_cond_cb_handler(
    VOID*               pv_data,
    SIZE_T              z_data_size)
{
    UINT8			    ui1_i = 0;
    ...
    do
    {
        /* Check input arguments */
        if ((NULL == pv_data) ||
            (z_data_size != sizeof(HANDLE_T)))
        {
            break;
        }

        h_widget = *(HANDLE_T*)pv_data;
	    ...
	    {
	        if (t_g_view.ah_sb_btn[ui1_i] == h_widget)
            {   
                ab_anim_end[ui1_i] = TRUE;
                break;
            }
        }
        c_wgl_set_alpha(h_widget, 255);

        a_wzd_anim_end(h_widget);
        
    } while(0);
}

/*-----------------------------------------------------------------------------
 * Name: _wzd_view_move_cb_fct
 * Description: move anim callback function
 * Inputs: -
 * Outputs: -
 * Returns: 0                   Successful.
 *          Any other values    Failed.
 ----------------------------------------------------------------------------*/ 
static INT32 _wzd_view_move_cb_fct(
    HANDLE_T            h_widget,
    UINT16              ui2_anim_type,
    WGL_ANIM_COND_INF_T t_anim_cond_inf,
    VOID*               pv_tag)
{
    ...
    if (_ANIM_TOTAL_END(&t_anim_cond_inf))
    {
        /* Async invoke callback handler function */        
        wzd_async_invoke(
                _wzd_view_move_cond_cb_handler, 
                (VOID*)(&h_widget), 
                sizeof(HANDLE_T), 
                FALSE);
    }
 ...
 *          Any other values    Failed.
 ----------------------------------------------------------------------------*/
static INT32 _wzd_view_nfy_sel_box_frm (
                    HANDLE_T                    h_widget,
                    UINT32                      ui4_msg,
                    VOID*                       pv_param1,
                    VOID*                       pv_param2
                    )
{
    ...
    WZD_SEL_BOX_BTN_INDEX_T  e_crnt_index = WZD_SEL_BOX_BTN_COUNT;
	WZD_SEL_BOX_BTN_INDEX_T  e_next_index = WZD_SEL_BOX_BTN_COUNT;
    WZD_ANIM_COND_CB_DATA_T* pt_data = NULL;

    switch(ui4_msg)
    {
    case WGL_MSG_KEY_DOWN:

        DBG_INFO(("[WZD][SB] --- KEY value: %d  @LINE: %d.\n", (UINT32)pv_param1, __LINE__));

        /* if sel box show animation is NOT finish, just return */
        if (FALSE == wzd_view_sel_box_show_finish())
        {
            return WZDR_OK;
        }
        /* if sel box scale animation is NOT finish, just return */
        if (FALSE == _wzd_view_all_widget_anim_finish())
        {
            return WZDR_OK;
        }
        ...
        e_crnt_index = (WZD_SEL_BOX_BTN_INDEX_T)(t_g_view.t_sb_ctrl.ui4_index_current - t_g_view.t_sb_ctrl.ui4_index_offset);

		/* deal with LEFT & RIGHT key first */
		if (BTN_CURSOR_LEFT == (UINT32)pv_param1)
        {
			i4_ret = _wzd_view_sel_box_get_previous_btn(e_crnt_index, &e_next_index);
	        if (WZDR_FAIL == i4_ret)
	        {
	        	return WZDR_FAIL;
	        }
			...
			{
				if (e_next_index == WZD_SEL_BOX_BTN_03)
				{
					/* reset sel box frame */
					_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_RIGHT, TRUE);
				}
 				else
 				{
					/* do nothing */
			...
			{
				if (e_next_index == WZD_SEL_BOX_BTN_04)
				{
					/* reset sel box frame */
					_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_CENTER, TRUE);
				}
 				else if (e_next_index < WZD_SEL_BOX_BTN_04)
				{
					/* keep focus middle box */
					if (t_g_view.t_sb_ctrl.ui4_index_current > WZD_SEL_BOX_BTN_04)
					{
						b_need_shift = TRUE;
						e_next_index = WZD_SEL_BOX_BTN_04;
					}
            ...
            if (e_crnt_index < e_next_index)
            {
                t_g_view.t_sb_ctrl.ui4_index_current = e_next_index;
            }
            else
            {
			...
			if (b_need_shift)
			{
				t_g_view.t_sb_ctrl.ui4_index_offset = t_g_view.t_sb_ctrl.ui4_index_current - e_crnt_index;

                /* set scale down animation callback data */
                pt_data = &at_anim_cb_get_focus_data;
                if (NULL == pt_data)
                {
                    return WZDR_OK;
                }
                c_memset(pt_data, 0, sizeof(WZD_ANIM_COND_CB_DATA_T));
                pt_data->b_used = TRUE;
                pt_data->h_anim_handle = t_g_view.ah_sb_btn[e_next_index];
                pt_data->ui4_index = t_g_view.t_sb_ctrl.ui4_index_current;
                pt_data->e_param = WZD_CALLBACK_PARAM_LEFT;

                /* set scale down callback */
                a_wzd_anim_set_cb(
                           t_g_view.ah_sb_btn[e_next_index],
                           _wzd_view_scale_cb_fct,
                           (void *)pt_data);

                /* start scale down and up anim */
                wzd_anim_scale_down_up(t_g_view.ah_sb_btn[e_next_index]);
			}
			else
			{
			    /* update box widget order */
			    wzd_view_sel_box_flip_order(t_g_view.ah_sb_btn[e_crnt_index], TRUE);

                /* set scale down animation callback data */
                pt_data = &at_anim_cb_lose_focus_data;
                if (NULL == pt_data)
                {
                    return WZDR_OK;
                }
                c_memset(pt_data, 0, sizeof(WZD_ANIM_COND_CB_DATA_T));
                pt_data->b_used = TRUE;
                pt_data->h_anim_handle = t_g_view.ah_sb_btn[e_crnt_index];
                pt_data->ui4_index = (e_crnt_index < e_next_index) ? e_crnt_index : (t_g_view.t_sb_ctrl.ui4_index_current + 1);
                pt_data->e_param = WZD_CALLBACK_PARAM_SCALE_DOWN;

                /* set scale down callback */
                a_wzd_anim_set_cb(
                           t_g_view.ah_sb_btn[e_crnt_index],
                           _wzd_view_scale_cb_fct,
                           (void *)pt_data);
                ...
                /* update background before scale anim start */
				_wzd_view_set_frm_bg_img(t_g_view.ah_sb_btn[e_next_index], h_g_wzd_img_select_box_highlight_small);

                /* start scale down anim */
                wzd_anim_fade_out_scale_down(t_g_view.ah_sb_btn[e_crnt_index]);

                /* start scale up anim */
                wzd_anim_scale_up(t_g_view.ah_sb_btn[e_next_index]);
		...
		{
			i4_ret = _wzd_view_sel_box_get_next_btn(e_crnt_index, &e_next_index);
	        if (WZDR_FAIL == i4_ret)
	        {
	        	return WZDR_FAIL;
	        }
			...
			else if (t_g_view.t_sb_info.ui4_index_total > WZD_SEL_BOX_BTN_COUNT-1)
			{
 				...
 				else if (e_next_index > WZD_SEL_BOX_BTN_04)
				{
					...
					{
						/* reset sel box frame */
						_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_LEFT, TRUE);
					}
			...
			if (b_need_shift)
			{
                ...
                pt_data->h_anim_handle = t_g_view.ah_sb_btn[e_next_index];
                pt_data->ui4_index = t_g_view.t_sb_ctrl.ui4_index_current;
                pt_data->e_param = WZD_CALLBACK_PARAM_RIGHT;
			...
			else
			{
                ...
                if (e_crnt_index > e_next_index)
                {
			        wzd_view_sel_box_flip_order(t_g_view.ah_sb_btn[e_next_index], TRUE);
                }
                else
                {
                    wzd_view_sel_box_flip_order(t_g_view.ah_sb_btn[e_crnt_index], FALSE);
                }
...
static INT32 _wzd_view_create_sel_box_buttons (VOID)
{
    ...
    t_g_view.t_sb_info.b_anim_affect_child = FALSE;

	t_g_view.t_sb_ctrl.ui4_index_current = 0;
	t_g_view.t_sb_ctrl.ui4_index_offset	= 0;

    /* reset anim flag status */
    for (ui1_i = 0 ; ui1_i < WZD_SEL_BOX_BTN_COUNT ; ui1_i ++)
	{
        ab_anim_end[ui1_i] = TRUE;
    }
                                ...
                                WZD_SEL_BOX_BTN_INDEX_T* pe_next_btn_index)
{
    ...
    /* 1st round */
    ui1_j = e_sel_box_index + 1;
    while (ui1_j < WZD_SEL_BOX_BTN_COUNT)
    {
        if (TRUE == t_g_view.t_sb_ctrl.ab_enabled[ui1_j])
        {
            *pe_next_btn_index = (WZD_SEL_BOX_BTN_INDEX_T)ui1_j;
            return WZDR_OK;
        }
        ui1_j++;
    }
...
{	
    ...
    /* 1st round */
    i1_j  = e_sel_box_index - 1;
    while (i1_j >= WZD_SEL_BOX_BTN_01)
    {
        if (TRUE == t_g_view.t_sb_ctrl.ab_enabled[i1_j])
        {
            *pe_pre_btn_index = (WZD_SEL_BOX_BTN_INDEX_T)i1_j;
            return WZDR_OK;
        }
        i1_j--;
    }
    ...
    /* 2nd round */
    i1_j = WZD_SEL_BOX_BTN_COUNT - 1;
    while (i1_j > e_sel_box_index)
    {
        if (TRUE == t_g_view.t_sb_ctrl.ab_enabled[i1_j])
        {
            *pe_pre_btn_index = (WZD_SEL_BOX_BTN_INDEX_T)i1_j;
 ...
 *			Any other values	   Failed.
 ----------------------------------------------------------------------------*/
static INT32 _wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_T e_direction, BOOL b_anim)
{
	static INT32   	i4_left_ori = 0;
    INT32   		i4_left;
    GL_RECT_T   	t_rect;
    BOOL            b_is_sel_frm_visible = FALSE;
    UINT8           ui1_repaint = WGL_NO_AUTO_REPAINT;

    ANIM_DATA_MOVE_T                t_anim_data;

    c_wgl_get_visibility(t_g_view.h_sb_frm, &b_is_sel_frm_visible);
    if (b_is_sel_frm_visible)
    {
        ui1_repaint = WGL_SYNC_AUTO_REPAINT;
    }
    ...
    if (i4_left != WZD_SEL_BOX_FRM_X)
    {
        i4_left_ori = WZD_SEL_BOX_FRM_X;
    }
    ...
    if (b_anim)
    {
    	...
    	t_anim_data.ui2_total_steps = 3;
        t_anim_data.ui2_frame_interval = 50;

        t_anim_data.i2_start_x = i4_left_ori;
        t_anim_data.i2_start_y = WZD_SEL_BOX_FRM_Y;
        t_anim_data.i2_end_x   = i4_left;
        t_anim_data.i2_end_y   = WZD_SEL_BOX_FRM_Y;

        a_wzd_anim_move(t_g_view.h_sb_frm, &t_anim_data, TRUE);
        a_wzd_anim_end(t_g_view.h_sb_frm);
    }
    else
    {
        c_memset(&t_rect, 0, sizeof(GL_RECT_T));
 ...
 *
 * Inputs:  e_type     WZD_SEL_BOX_SHOW_TYPE_NORMAL - Show select box frame.
 *                     WZD_SEL_BOX_SHOW_TYPE_ANIM - Popup select box with anim.
 *                     WZD_SEL_BOX_SHOW_TYPE_HIDE - Hide select box frame.
 *
 * Outputs: -
 *
 * Returns: 0                      Successful.
 *          Any other values       Failed.
 ----------------------------------------------------------------------------*/
INT32 wzd_view_sel_box_show (WZD_SEL_BOX_SHOW_TYPE_T e_type)
{
    UINT8			ui1_i = 0;
	UINT32			ui4_total;
    UINT16          ui2_steps = 6;
    UINT32          ui4_start_tick = 0;
    INT16           i2_start_gap = 50;
    ANIM_DATA_MIX_T t_anim_data;
	INT32   		i4_left;		/* left margins */
	GL_RECT_T   	t_rect;

    DBG_INFO(("[WZD]wzd_view_sel_box_show type = %d, total = %d\n", 
					(UINT32)e_type, 
					t_g_view.t_sb_info.ui4_index_total));
    
    ui4_total = t_g_view.t_sb_info.ui4_index_total;

    if (WZD_SEL_BOX_SHOW_TYPE_HIDE == e_type ||
        0 == ui4_total)
    {
        RET_ON_ERR (c_wgl_set_visibility (t_g_view.h_sb_frm, WGL_SW_HIDE));
        return WZDR_OK;
	...
	if (ui4_total < WZD_SEL_BOX_BTN_COUNT - 1)
	{
    ...
    if (WZD_SEL_BOX_SHOW_TYPE_NORMAL == e_type)
    {
        if (ui4_total < WZD_SEL_BOX_BTN_COUNT - 1)
    	{
    		/* set enable */
    ...
    else if (WZD_SEL_BOX_SHOW_TYPE_ANIM == e_type)
    {
        c_memset(&t_anim_data, 0, sizeof(ANIM_DATA_MIX_T));

        SET_RECT_BY_SIZE (&t_rect,
            		WZD_SEL_BOX_FRM_W - WZD_SEL_BOX_BTN_W,
            		WZD_SEL_BOX_BTN_Y,
            		WZD_SEL_BOX_BTN_W,
            		WZD_SEL_BOX_BTN_H);
    	...
    	for (ui1_i = 0 ; ui1_i < WZD_SEL_BOX_BTN_COUNT ; ui1_i ++)
    	{
            ...
            {
                SHOW_ON_ERR (c_wgl_set_visibility(t_g_view.ah_sb_btn[ui1_i], WGL_SW_HIDE));
            }
        }

        if (ui4_total >= WZD_SEL_BOX_BTN_COUNT)
    	{
    	    ui4_total = WZD_SEL_BOX_BTN_COUNT - 1;

            /* sel box do NOT need anim */
    		SET_RECT_BY_SIZE (&t_rect,
        			(i4_left + (WZD_SEL_BOX_BTN_W + WZD_SEL_BOX_BTN_GAP) * WZD_SEL_BOX_BTN_07),
        			WZD_SEL_BOX_BTN_Y,
        			WZD_SEL_BOX_BTN_W,
        			WZD_SEL_BOX_BTN_H);
    	...
    	for (ui1_i = 0 ; ui1_i < ui4_total ; ui1_i ++)
    	{
            ...
            {
                t_anim_data.ui2_frame_interval = 50;
                t_anim_data.ui4_start_tick = ui4_start_tick;

                t_anim_data.ui2_move_steps = ui2_steps;
                t_anim_data.i2_start_x = (i2_start_gap * ui1_i) + (WZD_SEL_BOX_FRM_W - WZD_SEL_BOX_BTN_W)/2;
                t_anim_data.i2_start_y = WZD_SEL_BOX_BTN_Y;
                t_anim_data.i2_end_x   = (i4_left + (WZD_SEL_BOX_BTN_W + WZD_SEL_BOX_BTN_GAP) * ui1_i);
                t_anim_data.i2_end_y   = WZD_SEL_BOX_BTN_Y;

                t_anim_data.ui2_scale_steps = ui2_steps;
                t_anim_data.ui8_start_w = 350;
                t_anim_data.ui8_start_h = 325;
                t_anim_data.ui8_end_w   = WZD_SEL_BOX_BTN_W;
                t_anim_data.ui8_end_h   = WZD_SEL_BOX_BTN_H;

                t_anim_data.ui2_alpha_steps = ui2_steps;
                t_anim_data.ui1_start_alpha = 0;
                t_anim_data.ui1_end_alpha = 255;

                /* TODO: adjust value to make anim more smooth */
                //ui2_steps = ui2_steps + 1;
                if (ui1_i <= 4)
                {
                    ui4_start_tick = ui4_start_tick + 2;
                }
                else
                {
                    ui4_start_tick = ui4_start_tick + 1;
                }
                    

                ab_anim_end[ui1_i] = FALSE;
                a_wzd_anim_set_cb(t_g_view.ah_sb_btn[ui1_i], _wzd_view_move_cb_fct, NULL);
                a_wzd_anim_mix(t_g_view.ah_sb_btn[ui1_i], &t_anim_data, TRUE);
            }
    	}
    }
...
/*-----------------------------------------------------------------------------
 * Name: wzd_view_sel_box_show_finish
 *
 * Description: This API to get status of select box frame anim.
 *
 * Inputs:  -
 *
 * Outputs: -
 *
 * Returns: TRUE                Animation finish.
 *          Any other values    Animation not finish.
 ----------------------------------------------------------------------------*/
BOOL wzd_view_sel_box_show_finish (VOID)
{
    UINT8			ui1_i = 0;

    for (ui1_i = 0 ; ui1_i < WZD_SEL_BOX_BTN_COUNT ; ui1_i ++)
    {
        if (FALSE == ab_anim_end[ui1_i])
        {
            return FALSE;
        }
    }
...
static INT32 _wzd_view_sel_box_update_btn_pos(VOID)
{
	...
	{
		RET_ON_ERR (_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_CENTER, FALSE));
	...
	{
		RET_ON_ERR (_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_RIGHT, FALSE));
...
/*-----------------------------------------------------------------------------
 * Name: _wzd_view_sel_box_update_btn_pos_with_focus_idx
 *
 * Description: The function update button postion with focus index.
 *
 * Inputs:	
 * Inputs:	
 *
 ...
 *			Any other values	   Failed.
 ----------------------------------------------------------------------------*/
static INT32 _wzd_view_sel_box_update_btn_pos_with_focus_idx(UINT32 ui4_idx)
{
	UINT32			ui4_total;
	...
	if (ui4_total < WZD_SEL_BOX_BTN_COUNT - 1)
	{
		...
		_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_CENTER, FALSE);
	}
	else if (ui4_total == WZD_SEL_BOX_BTN_COUNT - 1)
	{
	    /* just 6 box */
	    ...
	    if (ui4_idx < (UINT32)WZD_SEL_BOX_BTN_04)
		{
			_wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_RIGHT, FALSE);
		}
        else
        {
            _wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_CENTER, FALSE);
	...
	else
	{
        ...
        else if (ui4_idx > t_g_view.t_sb_info.ui4_index_total - 4)
        {
            t_g_view.t_sb_ctrl.ui4_index_offset = t_g_view.t_sb_info.ui4_index_total - (UINT32)WZD_SEL_BOX_BTN_COUNT;

            _wzd_view_sel_box_frm_move_by_direction(WZD_SEL_BOX_FRM_MOVE_LEFT, FALSE);
        }
        else
        {
            t_g_view.t_sb_ctrl.ui4_index_offset = ui4_idx - (UINT32)WZD_SEL_BOX_BTN_04;
...
INT32 wzd_view_sel_box_set_info_data (WZD_VIEW_SEL_BOX_INFO_T*	pt_info_data)
{
    ...
    t_g_view.t_sb_info.b_anim_affect_child = pt_info_data->b_anim_affect_child;
	...
	for( ui1_i = 0 ; ui1_i < WZD_SEL_BOX_BTN_COUNT ; ui1_i ++ )
	{
        ...
        RET_ON_ERR (_wzd_view_set_frm_bg_img(t_g_view.ah_sb_btn[ui1_i], h_g_wzd_img_select_box));
	}
 ...
 *          Any other values       Failed.
 ----------------------------------------------------------------------------*/
INT32 wzd_view_sel_box_set_focus (UINT32 ui4_idx, BOOL b_repaint)
{
    INT32   					i4_ret = WZDR_OK;
    BOOL                        b_is_sel_frm_visible = FALSE;
    WZD_SEL_BOX_BTN_INDEX_T 	e_sb_idx = WZD_SEL_BOX_BTN_01;
    WZD_ANIM_COND_CB_DATA_T*    pt_data = NULL;

    /* if sel box scale animation is NOT finish, just return */
    if (!_wzd_view_all_widget_anim_finish())
    {
        DBG_INFO(("\n####[WZD][SB][set focus@line:%d,just return]\n",__LINE__));
        return WZDR_OK;
    }

    /* update current index */
    t_g_view.t_sb_ctrl.ui4_index_current = ui4_idx;

    /* update button postion with focus index */
    _wzd_view_sel_box_update_btn_pos_with_focus_idx(ui4_idx);
    ...
    if (b_is_sel_frm_visible)
    {
        _wzd_view_set_frm_bg_img(t_g_view.ah_sb_btn[e_sb_idx], h_g_wzd_img_select_box_highlight_small);

        /* set animation callback data */
        pt_data = &at_anim_cb_get_focus_data;
        if (NULL == pt_data)
        {
            return WZDR_OK;
        ...
        c_memset(pt_data, 0, sizeof(WZD_ANIM_COND_CB_DATA_T));
        pt_data->b_used = TRUE;
        pt_data->h_anim_handle = t_g_view.ah_sb_btn[e_sb_idx];
        pt_data->ui4_index = t_g_view.t_sb_ctrl.ui4_index_current;
        pt_data->e_param = WZD_CALLBACK_PARAM_SCALE_UP;

        /* set animation callback func */
        a_wzd_anim_set_cb(
                   t_g_view.ah_sb_btn[e_sb_idx],
                   _wzd_view_scale_cb_fct,
                   (void *)pt_data);

        /* start scale up anim */
        wzd_anim_scale_up(t_g_view.ah_sb_btn[e_sb_idx]);
    }
    else
    {
        /* wgl repaint float repaint bug */
        wzd_view_sel_box_set_order(t_g_view.ah_sb_btn[e_sb_idx]);

        _wzd_view_set_frm_bg_img(t_g_view.ah_sb_btn[e_sb_idx], h_g_wzd_img_select_box_highlight);
        if (b_repaint)
        {
            RET_ON_ERR (c_wgl_repaint(t_g_view.ah_sb_btn[e_sb_idx], NULL, TRUE));
        }
    }
...
/*-----------------------------------------------------------------------------
 * Name: wzd_view_sel_box_lose_focus
 *
 * Description: The function set focus to select box buttons.
 *
 * Inputs:  ui4_idx                         Which select box button.
 ...
 *          Any other values       Failed.
 ----------------------------------------------------------------------------*/
INT32 wzd_view_sel_box_lose_focus (UINT32 ui4_idx, BOOL b_repaint)
{
    BOOL                        b_is_sel_frm_visible = FALSE;
    WZD_SEL_BOX_BTN_INDEX_T 	e_sb_idx = WZD_SEL_BOX_BTN_01;
    WZD_ANIM_COND_CB_DATA_T*    pt_data = NULL;
    ...
    if (!_wzd_view_all_widget_anim_finish())
    {
        DBG_INFO(("\n####[WZD][SB][lose focus@line:%d,just return]\n",__LINE__));
        return WZDR_OK;
    }
    ...
    if (b_is_sel_frm_visible)
    {
        ...
        /* start scale down anim */
        wzd_anim_fade_out_scale_down(t_g_view.ah_sb_btn[e_sb_idx]);
    }
    else
    {
        _wzd_view_set_frm_bg_img(t_g_view.ah_sb_btn[e_sb_idx], h_g_wzd_img_select_box);

        /* sent item focused message */
        if (NULL != t_g_view.t_sb_info.pf_data_chg_nfy)
        {
...
/*-----------------------------------------------------------------------------
 * Name: wzd_view_sel_box_set_order
 *
 * Description: The function set order of handle of select box.
 *
 * Inputs:  h_handle               select box.
 *
 * Outputs: -
 *
 * Returns: 0                      Successful.
 *          Any other values       Failed.
 ----------------------------------------------------------------------------*/
INT32 wzd_view_sel_box_set_order (HANDLE_T h_handle)
{
    INT32   		    i4_ret = WZDR_OK;
    UINT8               ui1_i = 0;
	HANDLE_T            ah_handle[WZD_SEL_BOX_BTN_COUNT] = {NULL_HANDLE};
    HANDLE_T            h_handle_tmp = NULL_HANDLE;
    INT32               i4_number = WZD_SEL_BOX_BTN_COUNT;
    ...
    i4_ret = c_wgl_get_children_order(t_g_view.h_sb_frm, ah_handle, &i4_number);
    RET_ON_ERR (i4_ret);

#if 0
    UINT8               ui1_j = 0;

    /* find h_handle in handle list */
    for (ui1_i = 0 ; ui1_i < WZD_SEL_BOX_BTN_COUNT ; ui1_i ++)
    {
        for (ui1_j = 0 ; ui1_j < i4_number ; ui1_j ++)
        {
            if (t_g_view.ah_sb_btn[ui1_i] == ah_handle[ui1_j])
            {
                break;
            }
        }
        DBG_INFO(("[WZD] t_g_view.ah_sb_btn[%d] in ah_handle[%d].\n", ui1_i, ui1_j));
    }
#endif

    /* do NOT need adjust order */
    if (i4_number <= 1 || i4_number != WZD_SEL_BOX_BTN_COUNT)
    {
        return WZDR_OK;
    }
    if (ah_handle[i4_number-1] == h_handle)
    {
        return WZDR_OK;
    }

    /* find h_handle in handle list */
    for (ui1_i = 0 ; ui1_i < i4_number ; ui1_i ++)
    {
        if (ah_handle[ui1_i] == h_handle)
        {
            break;
        }
    }
    ...
    /* DBG_INFO(("[WZD] set order index = %d, number = %d.\n", ui1_i, i4_number)); */

    /* switch order with last one */
    h_handle_tmp = ah_handle[i4_number-1];
    ah_handle[i4_number-1] = ah_handle[ui1_i];
    ah_handle[ui1_i] = h_handle_tmp;

    i4_ret = c_wgl_set_children_order(t_g_view.h_sb_frm, ah_handle, i4_number);
    RET_ON_ERR (i4_ret);
...
/*-----------------------------------------------------------------------------
 * Name: wzd_view_sel_box_flip_order
 *
 * Description: The function flip order of handle of select box.
 *
 * Inputs:  h_handle               select box.
 *
 * Outputs: -
 ...
 *          Any other values       Failed.
 ----------------------------------------------------------------------------*/
INT32 wzd_view_sel_box_flip_order (HANDLE_T h_handle, BOOL b_reverse)
{
    INT32   		    i4_ret = WZDR_OK;
    UINT8               ui1_i = 0;
    UINT8               ui1_order = 0;
	HANDLE_T            ah_handle[WZD_SEL_BOX_BTN_COUNT];
    HANDLE_T            h_handle_tmp = NULL_HANDLE;
    INT32               i4_number = WZD_SEL_BOX_BTN_COUNT;

    /* reset handle list and find h_handle order in handle list */
    ui1_order = i4_number;
    for (ui1_i = 0 ; ui1_i < i4_number ; ui1_i ++)
    {
        if (b_reverse)
        {
            ah_handle[ui1_i] = t_g_view.ah_sb_btn[ui1_i];
        }
        else
        {
            ah_handle[ui1_i] = t_g_view.ah_sb_btn[i4_number - 1 - ui1_i];
        }
    ...
    /* DBG_INFO(("[WZD] reverse = %d, handle order = %d.\n", b_reverse, ui1_order)); */

    /* switch order with last one */
    if (ui1_order < i4_number)
    {
        h_handle_tmp = ah_handle[i4_number-1];
        ah_handle[i4_number-1] = ah_handle[ui1_order];
        ah_handle[ui1_order] = h_handle_tmp;
    }

    i4_ret = c_wgl_set_children_order(t_g_view.h_sb_frm, ah_handle, i4_number);
    RET_ON_ERR (i4_ret);

//#ifdef WZD_ANIM_DEBUG
#if 0
    UINT8               ui1_j = 0;
    ...
    for (ui1_i = 0 ; ui1_i < WZD_SEL_BOX_BTN_COUNT ; ui1_i ++)
    {
        ...
        DBG_INFO(("[WZD] wzd_view_sel_box_flip_order t_g_view.ah_sb_btn[%d] in ah_handle[%d].\n", ui1_i, ui1_j));
    }
#endif
