...
#define WZD_NW_PAGE_SHOW_TYPE_NULL      0   /* no move anim */
#define WZD_NW_PAGE_SHOW_TYPE_RTL       1   /* right to left move anim */
#define WZD_NW_PAGE_SHOW_TYPE_LTR       2   /* left to right move anim */
...
static UINT32                   ui4_g_curr_ap_idx = 0;

/* 0: default; 1: accept tos; -1: reject tos */
static INT8                 i1_tos_is_accept = 0;

/* FALSE: default; TRUE: enter reg info page and select next btn(store info) */
static BOOL                 b_reg_info_is_store = FALSE;

static BOOL                 b_nw_ready_to_show = FALSE;

static UINT8                ui1_page_show_type = WZD_NW_PAGE_SHOW_TYPE_NULL;
/*-----------------------------------------------------------------------------
                    function prototypes
 ----------------------------------------------------------------------------*/
...
static INT32 _wzd_page_nw_setup_sel_box_update_data (const UINT32 ui4_btn_idx);
...
static VOID _wzd_nw_notify_handler(VOID* pv_data, SIZE_T  z_data_size)
{
    ...
    {
        DBG_INFO( ("[WZD][NW] --- DHCP started successfully and get IPV4. %d\n",__LINE__));
    ...
    {
        DBG_INFO( ("[WZD][NW] --- DHCP started successfully but get link local IP. %d\n",__LINE__));
    ...
    case NW_NFY_ID_ETHERNET_UNPLUG:
        DBG_INFO( ("[WZD][NW] --- ETHERNET PLUGIN or UNPLUG. %d\n",__LINE__));
        _wzd_nw_conn_lan_or_wifi();
        break;
    case NW_NFY_ID_INTERN_CT_RESULT:
    {
        DBG_INFO( ("[WZD][NW] --- Internet connection test result. %d\n",__LINE__));
    ...
    {
        DBG_INFO( ("[WZD][NW] --- WLAN ASSOCIATE . %d\n",__LINE__));
    ...
    {
        DBG_INFO( ("[WZD][NW] --- WLAN START INIT. %d\n",__LINE__));
    ...
    {
        DBG_INFO( ("[WZD][NW] --- WLAN scan result notification. %d\n",__LINE__));
...
static VOID _wzd_nw_notify_callback(VOID* pv_tag, NW_NFY_ID_T e_nfy_id, VOID* pv_nfy_param)
{
    ...
    /* current network notify is NOT stable */
    if (NWR_FAIL == a_get_nw_init_status())
    {
        return;
    }
...
static INT32 _wzd_nw_wifi_scan_timeout (VOID)
{
    ...
    if (!t_wzd_nw.b_is_busying)
    {
        DBG_INFO( ("[WZD][WARNING] wlan scan be quit. %d\n", __LINE__));
...
#if 0
    DBG_INFO( ("[WZD][NW] --- %s, WLAN disassociate.\n", __FUNCTION__));
    ...
    ui1_page_show_type = WZD_NW_PAGE_SHOW_TYPE_RTL;
...
{
    DBG_INFO(  ("[WZD][NW] --- wzd nw wifi scan result. \n"));
    ...
    ui4_g_curr_ap_idx = 0;
    ui1_page_show_type = WZD_NW_PAGE_SHOW_TYPE_NULL;
...
static INT32 _wzd_nw_wifi_assoc_timeout (VOID)
{
    ...
    DBG_INFO( ("[WZD][NW] --- %s %d, WIFI ASSOC TIMEOUT.\n", __FUNCTION__,__LINE__));
    ...
    {
        DBG_INFO( ("[WZD][NW] --- %s, WIFI not ready.\n", __FUNCTION__));
    }
...
{
    DBG_INFO( ("[WZD][NW] --- %s %d, ASSOC RESULT.\n", __FUNCTION__,__LINE__));
...
static INT32 _wzd_nw_wifi_assoc_result (VOID)
{
    ...
    DBG_INFO( ("[WZD][NW] --- ASSOC RESULT SSID: %s.\n", w2s_ssid));
            ...
            if( BTN_SELECT == (UINT32)pv_param1 )
            {
                ...
                if (WZD_DLG_BTN_TYPE_01 == (WZD_DLG_BTN_TYPE_T)(UINT32)pv_param2)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ui1_page_show_type = WZD_NW_PAGE_SHOW_TYPE_LTR;
...
static INT32 _wzd_nw_assoc_dlg_create (VOID)
{
    ...
    DBG_INFO( ("[WZD][NW] --- ASSOC FAILED SSID: %s.\n", w2s_ssid));
    ...
    {
        DBG_INFO( ("[WZD][NW] --- current state: SSID_INPUT! : %s.\n", w2s_ssid));
    ...
    else
    {
        ...
        if(pt_associate == NULL)
        {
            DBG_INFO( ("[WZD][NW] --- %s %d, WIFI ASSOC DATA is NULL.\n", __FUNCTION__,__LINE__));
            return WZDR_FAIL;
        }
 ...
 *          FALSE: the E_mail addr str is not valid.
 ----------------------------------------------------------------------------*/
static BOOL _wzd_nw_reg_info_is_valid_email_addr(CHAR* s_str, UINT8
ui1_str_len)
{
    if ( NULL == s_str)
    {
 ...
 *              including: E-mail, first name, last nme, phone num and zip
code.
 *
...
static BOOL _wzd_nw_reg_info_is_valid(CHAR* s_str, UINT8 ui1_str_len)
{
    ...
    switch (t_wzd_nw.t_info_reg.e_current_item)
    {
        ...
        {
            b_is_valid = _wzd_nw_reg_info_is_valid_email_addr(s_str,c_strlen(
s_str));
            break;
        }
        ...
        {
            b_is_valid = _wzd_nw_reg_info_is_valid_phone_num(s_str,c_strlen(
s_str));
            break;
        }
        case WZD_NW_REG_INFO_ZIP_CODE:
        {
            b_is_valid = _wzd_nw_reg_info_is_valid_zip_code(s_str,c_strlen(
s_str));
            break;
        }
        default:
        ...
        case WZD_DLG_MSG_KEY_INPUT:
        {
            ...
            switch (ui4_keycode)
            {
                case BTN_SELECT:
                {
                    ...
                    i4_ret = c_wgl_set_focus(t_wzd_nw.t_info_reg.ah_editbox[
t_wzd_nw.t_info_reg.e_current_item], TRUE);
                    SHOW_ON_ERR(i4_ret);
 ...
 *
 * Description:create and show warning dialog for registration info data
verification
 *
 * Input arguments: current item: E_mial, first_name, last_name, phone_num,
zip_code
 *
 * Output arguments: -
 *
...
*----------------------------------------------------------------------------*/
static INT32 _wzd_nw_reg_info_warning_dlg_create (WZD_NW_INFO_REG_FOCUS_E
e_current_item)
{
    WZD_DLG_INIT_T          t_dlg_init;
    HANDLE_T                h_layout_frm = NULL_HANDLE;
    ...
    pw2s_dlg_text = WZD_TEXT(MLM_WZD_KEY_NETWORK_REG_INFO_EMAIL_DLG_TXT +
e_current_item );
    ...
    t_wzd_nw.t_info_reg.b_warning_dlg_is_show[t_wzd_nw.t_info_reg.
e_current_item] = TRUE;
...
static BOOL _wzd_nw_is_ssid_scan_success(VOID)
{
    ...
    for (ui1_idx = 0; ui1_idx < ui1_max_ap_num; ui1_idx ++)
    {
        ...
        DBG_INFO( ("[WZD][NW] --- %s %d, ssid scan success.\n", __FUNCTION__,__LINE__));
...
static INT32 _wzd_nw_ssid_wifi_assoc(VOID)
{
    ...
    {
        DBG_INFO( ("[WZD][NW] --- %s %d, start SSID WLAN associate.\n", __FUNCTION__,__LINE__));
        i4_ret = a_nw_custom_wlan_disassociate();
        RET_ON_ERR(i4_ret);
...
static INT32 _wzd_nw_wifi_conn(VOID)
{
    ...
    if(WZDR_OK != i4_ret)
    {
        DBG_INFO(  ("[WZD][NW] --- wifi can not start scanning... \n"));
...
static INT32 _wzd_nw_conn_is_connected(BOOL* pb_is_connected)
{
    ...
    {
        b_connected = FALSE;
        DBG_INFO( ("<WZD Network> --- %s, %d. Network connection fail! ---", __FUNCTION__, __LINE__));
    }*/
...
/*----------------------------------------------------------------------------
 * Name: _wzd_nw_conn_show_begin_page
 *
 * Description:
 *
 * Inputs:
 ...
 *
 ----------------------------------------------------------------------------*/
static INT32 _wzd_nw_conn_show_begin_page(VOID)
{
    INT32           i4_ret;
    BOOL            b_wired_plug = FALSE;
    ...
    if (NWR_FAIL == a_get_nw_init_status())
    {
        i4_ret = _wzd_nw_go(NW_SETUP_STATE_WIRELESS_SCANNING, FALSE);
        RET_ON_ERR(i4_ret);
    ...
    if (WZDR_OK != i4_ret)
    {
        i4_ret = _wzd_nw_go(NW_SETUP_STATE_NONE_CONN, FALSE);
        RET_ON_ERR(i4_ret);

        return WZDR_OK;
    }

    if (TRUE == b_wired_plug)
    {
        DBG_INFO( ("[WZD][NW] PLUG --- %s,%d.\n", __FUNCTION__, __LINE__));
        _wzd_nw_go(NW_SETUP_STATE_BEGIN, FALSE);
    }
    else
    {
        DBG_INFO( ("[WZD][NW] Un-Plug --- %s,%d.\n", __FUNCTION__, __LINE__));
        _wzd_nw_go(NW_SETUP_STATE_WIRELESS_SCANNING, FALSE);
    }
...
static INT32 _wzd_nw_pwd_wifi_assoc(VOID)
{
    ...
    if (b_valid)
    {
        ...
        DBG_INFO( ("[WZD][NW] --- %s %d, start WLAN associate.\n", __FUNCTION__,__LINE__));
        i4_ret = a_nw_custom_wlan_disassociate();
        RET_ON_ERR(i4_ret);
...
static INT32 _wzd_nw_store_ap_list_data(VOID)
{
    ...
    DBG_INFO( ("[WZD][NW] --- total AP num: %d. visiable AP num: %d.\n",
                ui1_max_ap_num,
                ui1_visiable_ap_num));
...
static INT32 _wzd_nw_reg_info_json_merge(CHAR **pstr_in, CHAR **pstr_out)
{
    ...
    if(WZDR_OK != i4_ret)
    {
        DBG_INFO( ("\n%s(%d)#### a_uli_get_serial_number FAIL return[%d]\n",__FUNCTION__,__LINE__,i4_ret));
    }
    else
#endif
    {
 ...
 *
 * Description: set nw reg info(email/first name/last name/phone num/zip code) to /3rd_rw/wzd_reg_info file
 *
 * Inputs:
 *
 * Outputs:
...
static INT32 _wzd_nw_info_reg_store_user_info(VOID)
{
    ...
    if (ULIR_OK != i4_ret)
    {
        DBG_INFO( ("<WZD Network> ---  failed, return_value=%d\n", i4_ret));
        return WZDR_FAIL;
    }
#endif
...
static INT32 _wzd_nw_wps_associate_cancel(VOID)
{
    ...
    DBG_INFO( ("[WZD][NW] --- Cancel WPS. %s %d \n", __FUNCTION__,__LINE__));
...
static INT32 _wzd_nw_info_next_btn_select_proc_fct()
{
    ...
    /*set reg info to /3rd_rw/wzd_reg_info file for data connection*/
...
static INT32 _wzd_nw_info_reg_set_nav(VOID)
{
    ...
    i4_ret =
    c_wgl_set_navigation(t_wzd_nw.t_info_reg.ah_editbox[WZD_NW_REG_INFO_LAST_NAME], 1, at_key_lnk);
    RET_ON_ERR(i4_ret);
                                ...
                                VOID*       pv_param2)
{
    ...
    if (ui4_msg == WGL_MSG_NOTIFY)
    {
        ...
        {
            DBG_INFO( ("[WZD][EB] --- CONTENT CHANGED. %s %d \n", __FUNCTION__,__LINE__));
...
/*----------------------------------------------------------------------------
 * Name: _wzd_page_nw_setup_anim_finish_nfy_cb
 *
 * Description: Continue scan ap or ethernet connection after anim finish
 *
 * Inputs:  -
 *
 * Outputs: -
 ...
 *
 ----------------------------------------------------------------------------*/
static VOID _wzd_page_nw_setup_anim_finish_nfy_cb(VOID)
{
    INT32           i4_ret;
    UINT8           ui1_count = 0;
    BOOL            b_wired_plug = FALSE;
    ...
    if (NW_SETUP_STATE_PROTOCOL == e_g_nw_setup_state
        || NW_SETUP_STATE_INFO_REG == e_g_nw_setup_state)
    {
        return;
    }

    DBG_INFO( ("[WZD][NW] --- Step by step = %s %d.\n", __FUNCTION__,__LINE__));
    ...
    else
    {
        if (NW_SETUP_STATE_WIRELESS_SCANNING != e_g_nw_setup_state)
        {
            DBG_INFO( ("\n[WZD][NW] --- NOT in scanning state, just STOP. LINE:%d.\n", __LINE__));
            return;
        }

        ui1_count = 0;

        /* check the wlan driver init, waiting... */
        while (!a_nw_custom_nw_wifi_start() && (ui1_count < 5))
        {
            DBG_INFO( ("\n[WZD][NW] --- waiting wlan start... LINE:%d.\n", __LINE__));
            c_thread_delay(1000);
            ui1_count ++;
        }
        if (!a_nw_custom_nw_wifi_start())
...
static INT32 _wzd_nw_go(NW_SETUP_STATE e_nw_setup_state, BOOL b_repaint)
{
    ...
    {
        DBG_INFO( ("[WZD][NW] --- FUNC: %s LINE: %d, state = %d.\n", __FUNCTION__,__LINE__, e_nw_setup_state));
    ...
    case NW_SETUP_STATE_WIRELESS_SCANNING:
    {
        ...
        i4_ret = wzd_view_sel_box_show(WZD_SEL_BOX_SHOW_TYPE_NORMAL);
        RET_ON_ERR(i4_ret);
    ...
    case NW_SETUP_STATE_WIRELESS_SCAN_RESULT:
    {
        i4_ret = wzd_view_sel_box_show(WZD_SEL_BOX_SHOW_TYPE_HIDE);
        RET_ON_ERR(i4_ret);
        ...
        ui4_g_pre_focused_sb_idx = ui4_g_curr_ap_idx;

        i4_ret = wzd_view_sel_box_set_focus(ui4_g_curr_ap_idx, FALSE);
        RET_ON_ERR(i4_ret);

        if (ui4_g_curr_ap_idx >= 3)
        {
            if (ui4_g_curr_ap_idx < t_wzd_nw.t_choice_connect.ui1_visiable_ap_num - 4)
            {
                i4_ret = _wzd_page_nw_setup_sel_box_update_data(ui4_g_curr_ap_idx);
                RET_ON_ERR(i4_ret);
            }
            else if (ui4_g_curr_ap_idx >= t_wzd_nw.t_choice_connect.ui1_visiable_ap_num - 4)
            {
                i4_ret = _wzd_page_nw_setup_sel_box_update_data(t_wzd_nw.t_choice_connect.ui1_visiable_ap_num - 4);
                RET_ON_ERR(i4_ret);
            }
        }

        if (NW_SETUP_STATE_WIRELESS_SCANNING == e_g_nw_setup_pre_state)
        {
            i4_ret = wzd_view_sel_box_show(WZD_SEL_BOX_SHOW_TYPE_ANIM);
            RET_ON_ERR(i4_ret);
        }
        else
        {
    ...
    i4_ret = wzd_anim_start_move_page(e_move_show_type);
    RET_ON_ERR(i4_ret);
...
static INT32 _wzd_page_nw_setup_show(UINT32 ui4_sub_page_id)
{
        ...
        ui2_status == WZD_STATE_START_SETUP)
    {
        ...
        */
        ...
        {
            RET_ON_ERR(_wzd_nw_conn_show_begin_page());
        }
    }
    else
    {
...
/*----------------------------------------------------------------------------
 * Name: _wzd_page_nw_setup_sel_box_update_data
 *
 * Description:
 *
 * Inputs:  ui4_btn_idx     Select box button index
...
static INT32 _wzd_page_nw_setup_sel_box_btn_select (const UINT32 ui4_btn_idx)
{
    ...
    else if (NW_SETUP_STATE_WIRELESS_SCAN_RESULT == e_g_nw_setup_state)
    {
        ...
        if (FALSE == _wzd_nw_is_lock_ap(pt_associate->e_AuthMode,pt_associate->e_AuthCipher))
        {
            ...
            /* set sel box lose focus maunaly */
            ui4_g_pre_focused_sb_idx = ui4_btn_idx;
    ...
    else if (NW_SETUP_STATE_WIRELESS_ENCRYPTION_SELECT == e_g_nw_setup_state)
    {
        if (ui4_btn_idx == 0)
        {
            DBG_INFO( ("[WZD][NW] --- %s %d, Select NONE.\n", __FUNCTION__,__LINE__));
        }
        else
        {
            DBG_INFO( ("[WZD][NW] --- %s %d, Select other ENCRYPTION.\n", __FUNCTION__,__LINE__));
        }
    }
    else
    {
...
static INT32 _wzd_page_nw_setup_sel_box_hl_theme (const UINT32 ui4_btn_idx)
{
    HANDLE_T                    h_sb_txt = NULL_HANDLE;
    HANDLE_T                    h_sb_icon_1st = NULL_HANDLE;
    BOOL                        b_is_widget_visible = FALSE;
    GL_RECT_T                   t_rect = {0};
    WGL_COLOR_INFO_T            t_color_txt;
    WGL_FONT_INFO_T             t_fnt_info;
    WZD_SEL_BOX_BTN_INDEX_T     e_sb_idx = WZD_SEL_BOX_BTN_COUNT;
    ...
    else if (NW_SETUP_STATE_WIRELESS_SCAN_RESULT == e_g_nw_setup_state)
    {
        ...
        */
        ...
        RET_ON_ERR (c_wgl_get_visibility(h_sb_icon_1st, &b_is_widget_visible));
        if (!b_is_widget_visible)
        {
            /* move position */
            SET_RECT_BY_SIZE(&t_rect,
                            (340 * WZD_UI_RATIO),
                ...
                case BTN_CURSOR_DOWN:
                    {
                        ...
                        ui4_g_curr_ap_idx = (UINT32)pv_param2;
...
static INT32 _wzd_page_nw_setup_create_sel_box (VOID)
{
    ...
    else if (NW_SETUP_STATE_WIRELESS_SCAN_RESULT == e_g_nw_setup_state)
    {
        ...
        t_sel_box_info.b_anim_affect_child = TRUE;
    }
    else if (NW_SETUP_STATE_WIRELESS_ENCRYPTION_SELECT == e_g_nw_setup_state)
    {
                ...
                WZD_PAGE_INFO_DATA_T*   pt_info_data)
{
    ...
    pt_info_data->pf_anim_finish_nfy_fct   = _wzd_page_nw_setup_anim_finish_nfy_cb;
