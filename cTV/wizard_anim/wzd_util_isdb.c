...
#include "u_sb_isdb_eng.h"

/*-----------------------------------------------------------------------------
                    macros, defines, typedefs, enums
 ----------------------------------------------------------------------------*/                
#define _GET_PHY_CH_NUM(v) (((v)>>16) & 0xFF)
...
#define ISDB_TRSTRL_SVL_ID              ATSC_TRSTRL_SVL_ID
#define ISDB_TRSTRL_SVL_NAME            ATSC_TRSTRL_SVL_NAME
#define ISDB_TRSTRL_SVL_FILE            ATSC_TRSTRL_SVL_FILE
#define ISDB_TRSTRL_TUNER_NAME          SN_TUNER_GRP_0

#ifndef BRAZIL_MAX_ANA_CABLE_IDX
#define BRAZIL_MAX_ANA_CABLE_IDX 125
#endif

#ifndef ANALOG_MAX_CABLE_INDEX
#define ANALOG_MAX_CABLE_INDEX BRAZIL_MAX_ANA_CABLE_IDX
#endif
...
typedef struct
{
    ...
    /* Svl Builder of the ISDB AIR */
    BOOL                b_isdb_air_scanning;
    HANDLE_T            h_isdb_air;
                          ...
                          VOID*             pv_nfy_tag)
{
    if (t_g_util.ui2_status == WZD_SB_STATUS_SCANNING)
    {
...
#else
            t_sb_nfy.i4_progress   = (t_scan_info_dtv.i4_phy_ch_num * 100 / t_scan_info_dtv.i4_phy_ch_ttl);
#endif
        ...
        else if (ui4_nfy_reason & SB_REASON_COMPLETE)
        {
            ...
            if (!wzd_util_is_scanning(t_g_util.ui2_scanning_type))
            {
...
static VOID _reset_scanning_flag (HANDLE_T   h_builder)
{
    ...
    else if (h_builder == t_g_util.h_isdb_air)
    {
        t_g_util.b_isdb_air_scanning = FALSE;
    }
    else
    {
        /* do nothing */
...
static INT32 _wzd_util_sb_init (VOID)
{
    ...
    /* Open SVL Builder of the ISDB AIR */
    if (t_g_util.h_isdb_air == NULL_HANDLE)
    {
        SB_ISDB_OPEN_DATA_T t_sb_isdb_open_data;
        c_memset(&t_sb_isdb_open_data, 0, sizeof(SB_ISDB_OPEN_DATA_T));
        t_sb_isdb_open_data.ui2_svl_id    = ISDB_TRSTRL_SVL_ID;   
        t_sb_isdb_open_data.ps_svl_name   = ISDB_TRSTRL_SVL_NAME;  
        t_sb_isdb_open_data.ps_file_name  = ISDB_TRSTRL_SVL_FILE; 
        t_sb_isdb_open_data.ps_tuner_name = ISDB_TRSTRL_TUNER_NAME;
        t_sb_isdb_open_data.b_use_orig =TRUE;
        RET_ON_ERR (c_sb_open_builder(SB_TRSTRL_ENG_NAME, 
                                      & t_sb_isdb_open_data, 
                                      NULL, _sb_nfy_fct, &t_g_util.h_isdb_air, & e_cond));
    }
...
{
    /*  [NOTICE] 
        ISDB IDTV: Scan digital channels firstly and then analog channels 
    */
    INT32                   i4_rc;
    ISO_3166_COUNT_T        s_country_code;
    SB_ISDB_SCAN_DATA_T      t_isdb_scan_data;
    SB_COND_T               e_cond;
#ifdef APP_ISDB_ANALOG_SUPPORT
    WZD_UTIL_NTSC_SCAN_DATA_T           t_ntsc_scan_data;
#endif
    ...
    if (ui2_scanning_type == WZD_SB_SCANNING_AIR)
    {
        ...
        /* Start scan the DTV */
        DBG_INFO(("<WZD> scan start: DTV ISDB AIR Svl Builder. %s\r\n", s_country_code));
        c_memset(& t_isdb_scan_data, 0, sizeof(SB_ISDB_SCAN_DATA_T));

        t_isdb_scan_data.e_scan_type   = SB_ISDB_SCAN_TYPE_FULL_MODE;
        c_strncpy(t_isdb_scan_data.t_country_code, s_country_code, ISO_3166_COUNT_LEN);

        i4_rc = c_sb_start_scan(t_g_util.h_isdb_air, 
                                & t_isdb_scan_data, 
                                & t_scan_info_dtv, 
                                _scan_nfy_fct, 
                                & e_cond);        

        t_g_util.b_isdb_air_scanning = (i4_rc == SBR_OK) ? TRUE : FALSE;

        if (!t_g_util.b_isdb_air_scanning)
        {
            t_g_util.ui2_status = WZD_SB_STATUS_ERROR;
        ...
        c_memset(& t_ntsc_scan_data, 0, sizeof(WZD_UTIL_NTSC_SCAN_DATA_T));
        t_ntsc_scan_data.ui1_freq_plan = WZD_UTIL_NTSC_AIR_FREQ_PLAN;
        t_ntsc_scan_data.e_scan_type   = WZD_UTIL_NTSC_SCAN_TYPE;
    ...
    else
    {
        ...
        t_ntsc_scan_data.ui2_start_idx = SB_NTSC_CAB_SCAN_FIRST_CHANNEL; 
        t_ntsc_scan_data.ui2_end_idx   = ANALOG_MAX_CABLE_INDEX;
...
INT32 wzd_util_scan_cancel (UINT16   ui2_scanning_type)
{
    if (ui2_scanning_type == WZD_SB_SCANNING_AIR)
    {
        ...
        if (t_g_util.b_isdb_air_scanning)
        {
            ...
            c_sb_cancel_scan(t_g_util.h_isdb_air);
            t_g_util.b_isdb_air_scanning = FALSE;
        }
...
BOOL wzd_util_is_scanning (UINT16   ui2_scanning_type)
{
    if (ui2_scanning_type == WZD_SB_SCANNING_AIR)
    {
        if (t_g_util.b_ntsc_air_scanning || t_g_util.b_isdb_air_scanning)
        {
            return TRUE;
        }
    }
...
INT32 wzd_util_init (VOID)
{
    ...
    t_g_util.h_isdb_air = NULL_HANDLE;
