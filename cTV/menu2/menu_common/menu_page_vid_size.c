...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
typedef struct _MENU_PIC_SIZE_PAGE_DATA_T
{
    HANDLE_T h_root_frm;
    HANDLE_T h_main_frm;
    ...
    HANDLE_T h_ok_text;
    UINT32   ui4_nfy_id;
}MENU_PIC_SIZE_PAGE_DATA_T;

#define VID_SIZE_BTN_X ((SCREEN_W-VID_SIZE_BTN_W)/2)
#define VID_SIZE_BTN_Y ((SCREEN_H-VID_SIZE_BTN_H)/2)

#define VID_SIZE_BTN_W (200)
#define VID_SIZE_BTN_H (200)


#define VID_SIZE_HELP_X (100)
#define VID_SIZE_HELP_Y (VID_SIZE_SUB_HELP_Y - VID_SIZE_HELP_H)
#define VID_SIZE_HELP_W (SCREEN_W - VID_SIZE_HELP_X * 2)
#define VID_SIZE_HELP_H (100)

#define VID_SIZE_SUB_HELP_X (100)
#define VID_SIZE_SUB_HELP_Y (SCREEN_H - VID_SIZE_SUB_HELP_H - 100)
#define VID_SIZE_SUB_HELP_W (SCREEN_W - VID_SIZE_SUB_HELP_X * 2)
#define VID_SIZE_SUB_HELP_H (30)
...
/*-----------------------------------------------------------------------------
                    data declarations
-----------------------------------------------------------------------------*/
MENU_PAGE_FCT_TBL_T  t_g_menu_page_vid_size;
static MENU_PIC_SIZE_PAGE_DATA_T t_g_pic_size_page_data;
/*-----------------------------------------------------------------------------
...
static  INT32 _vid_update_pic_size_info(UINT8 u1_input_id,INT32 i4_value,BOOL b_direction_v)
{
    INT32   i4_crt_save = 0;
    SIZE_T  size;
    ...
    switch(u1_input_id)
    {
        case INPUT_COMP:
        {
            ...
            if(b_direction_v)
                VID_INPUT_SRC_PIC_SIZE_V_SET(i4_crt_save,i4_value);
            else
                VID_INPUT_SRC_PIC_SIZE_H_SET(i4_crt_save,i4_value);
...
INT32 vid_update_pic_size_info(UINT8 u1_input_id, INT32 i4_value, BOOL b_direction_v)
{
    return _vid_update_pic_size_info(u1_input_id, i4_value, b_direction_v);
}


INT32 vid_update_current_input_src_pic_size_info()
{
    ...
    switch(ui1_input)
    {
        ...
        {
            MENU_CHK_FAIL(acfg_get(IDX_CUSTOM_COMP_VGA_SETTING,&i4_crt_save,&size));
            VID_INPUT_SRC_PIC_SIZE_V_GET(i4_crt_save,i2_v_value);
            VID_INPUT_SRC_PIC_SIZE_H_GET(i4_crt_save,i2_h_value);
            break;
        }
        case INPUT_HDMI1:
        {
        ...
        default:
        {
             ...
             {
                ui2_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_CUSTOM_H_SIZE);
				i2_h_value = 0;
				a_cfg_av_get(ui2_id, &i2_h_value);
                a_cfg_av_set(ui2_id, i2_h_value);
                a_cfg_av_update(ui2_id);
                ui2_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_CUSTOM_V_SIZE);
				i2_v_value = 0;
    ...
    DBG_LOG_PRINT(("[MENU][%s %d] current save is 0x%x size = %d 11=> %x\n",__FUNCTION__,__LINE__,i4_crt_save,size));
    ...
    else
    {
         ui2_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_CUSTOM_H_SIZE);
         a_cfg_av_set(ui2_id, i2_h_value);
         DBG_LOG_PRINT(("[MENU][%s %d] APP_CFG_GRPID_VIDEO APP_CFG_RECID_VID_CUSTOM_H_SIZE,h => %d\n",__FUNCTION__,__LINE__,i2_h_value));
         ui2_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_CUSTOM_V_SIZE);
         a_cfg_av_set(ui2_id, i2_v_value);
         DBG_LOG_PRINT(("[MENU][%s %d] APP_CFG_GRPID_VIDEO APP_CFG_RECID_VID_CUSTOM_V_SIZE,v => %d\n",__FUNCTION__,__LINE__,i2_v_value));
     }
    a_cfg_av_update(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_OVERSCAN));
    a_cfg_update_viewport();
    return MENUR_OK;
...
/*-----------------------------------------------------------------------------
                    function implementations
 ----------------------------------------------------------------------------*/
static INT32 _vid_size_sub_help_create(
    HANDLE_T    h_parent,
    HANDLE_T*   ph_widget)
{
    INT32           i4_ret;
    ...
    GL_COLOR_T  t_color_gray = {255, {128}, {128}, {128}};
    ...
    i4_ret = c_wgl_create_widget(h_parent,
                                 ...
                                 NULL,
                                 255,
                                 (VOID*)(WGL_STL_TEXT_MULTILINE | WGL_STL_TEXT_MAX_512 | WGL_STL_GL_NO_IMG_UI),
                                 NULL,
                                 ph_widget);
    if (i4_ret < 0)
    {
    ...
    t_clr_info.u_color_data.t_standard.t_enable       = t_color_gray;
    t_clr_info.u_color_data.t_standard.t_highlight    = t_color_gray;
    t_clr_info.u_color_data.t_standard.t_disable      = t_color_gray;
    i4_ret = c_wgl_do_cmd(*ph_widget,
...
static INT32 _vid_size_help_create(HANDLE_T    h_parent,
                                          HANDLE_T*   ph_widget)
{
    GL_RECT_T                   t_rect;
    ...
    SET_RECT_BY_SIZE(&t_rect,
                      (INT32)VID_SIZE_HELP_X,
                      (INT32)VID_SIZE_HELP_Y,
                      (INT32)VID_SIZE_HELP_W,
                      (INT32)VID_SIZE_HELP_H);
...
static UTF16_T* _vid_size_w2s_sch(UTF16_T*        pw2s_text,
...
static INT32 _vid_size_set_user_info(UTF16_T* pw2s_text,
                                              ...
                                              WGL_HIMG_TPL_T h_img_5)
{
    ...
    MENU_PIC_SIZE_PAGE_DATA_T* pt_this = &t_g_pic_size_page_data;
    UINT16                 ui2_sec_id = 0;
    ...
    pw2s_next = _vid_size_w2s_sch(pw2s_temp, w2s_pic_flag);
    while (pw2s_next)
    {
...
static INT32 _vid_size_set_user_info_new_parag(UTF16_T* pw2s_text,
                                              WGL_HIMG_TPL_T h_img_1,
                                              WGL_HIMG_TPL_T h_img_2,
                                              WGL_HIMG_TPL_T h_img_3,
                                              WGL_HIMG_TPL_T h_img_4,
...
static INT32 _vid_size_center_ok_text(HANDLE_T h_parent, HANDLE_T* ph_widget)
{
    INT32           i4_ret;
    GL_RECT_T       t_rect;
    WGL_INSET_T     t_inset;
...
static INT32 _vid_size_btn_create(HANDLE_T h_parent,
...
static INT32 _vid_size_set_adjust_btn_bk_image(WGL_HIMG_TPL_T h_image, BOOL b_repaint)
{
    INT32               i4_ret = MENUR_OK;
    MENU_PIC_SIZE_PAGE_DATA_T* pt_this = &t_g_pic_size_page_data;
    WGL_IMG_INFO_T      t_img_info = {0};
...
static VOID _vid_process_pic_size(UINT32 ui4_keycode,INT16* pu2_current)
{
    if(pu2_current != NULL &&
       (BTN_CURSOR_LEFT  == ui4_keycode ||
        BTN_CURSOR_RIGHT == ui4_keycode ||
        BTN_CURSOR_UP    == ui4_keycode ||
        BTN_CURSOR_DOWN  == ui4_keycode))
    {
        DBG_LOG_PRINT(("[MENU][%s %d] %s current value = %d\n",
            __FUNCTION__,__LINE__,
            BTN_CURSOR_LEFT  == ui4_keycode ? "BTN_CURSOR_LEFT" :
            BTN_CURSOR_RIGHT == ui4_keycode ? "BTN_CURSOR_RIGHT" :
            BTN_CURSOR_UP    == ui4_keycode ? "BTN_CURSOR_UP" : "BTN_CURSOR_DOWN",
            *pu2_current));
        if(*pu2_current > 50)
           *pu2_current = 50;
        else if(*pu2_current < -50)
            *pu2_current = -50;
        ...
        switch(ui4_keycode)
        {
           ...
           {
                if(-50 <= *pu2_current && *pu2_current <= 50)
                    *pu2_current -= 5;
                break;
           }
           case BTN_CURSOR_UP:
           ...
           {
               if (*pu2_current <= 50 && *pu2_current >= -50)
                  *pu2_current += 5;
               break;
           }
        }
        if(*pu2_current > 50)
           *pu2_current = 50;
            ...
            *pu2_current = -50;
        DBG_LOG_PRINT(("[MENU][%s %d] current value = %d\n",
             __FUNCTION__,__LINE__,*pu2_current));
    }
}

static INT32 _vid_size_btn_proc_fct(HANDLE_T   h_widget,
                                      UINT32     ui4_msg,
                                      VOID*      param1,
                                      VOID*      param2)
{
    ...
    switch (ui4_msg)
    {
        case WGL_MSG_KEY_DOWN:
        {
            ...
            {
                UINT16 ui2_pisition_h_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_POS_H);
                UINT16 ui2_pisition_v_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_POS_V);
                INT16 i2_hpoisition = 0;
                INT16 i2_vpoisition = 0;
                if (t_isl_rec.e_src_type == INP_SRC_TYPE_AV
                    && t_isl_rec.t_avc_info.e_video_type == DEV_AVC_VGA)
                {
                    ui2_pisition_v_id = CFG_MAKE_ID(APP_CFG_GRPID_VGA, APP_CFG_RECID_VGA_POS_V);
                    ui2_pisition_h_id = CFG_MAKE_ID(APP_CFG_GRPID_VGA, APP_CFG_RECID_VGA_POS_H);
                }
                a_cfg_av_get(ui2_pisition_v_id, &i2_vpoisition);
                a_cfg_av_get(ui2_pisition_h_id, &i2_hpoisition);
                DBG_LOG_PRINT(("%s_%d i2_vpoisition:%d i2_hpoisition;%d \n",__FUNCTION__,__LINE__,i2_vpoisition,i2_hpoisition));
                if(i2_vpoisition != 0)
                {
                    a_cfg_av_set(ui2_pisition_v_id, 0);
					vid_update_position_info(ui1_input,0,TRUE);
                    a_cfg_av_update(ui2_pisition_v_id);
                }
                if(i2_hpoisition != 0)
                {
                    a_cfg_av_set(ui2_pisition_h_id, 0);
					vid_update_position_info(ui1_input,0,FALSE);
                    a_cfg_av_update(ui2_pisition_h_id);
                }
            }
            switch (ui4_keycode)
            {
                ...
                {
                    INT16 i2_hsize = 0;
                    UINT32 ui2_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_CUSTOM_H_SIZE);

                    MENU_LOG_ON_FAIL(_vid_size_set_adjust_btn_bk_image(h_g_pic_size_remote_horizontal_add_v2, TRUE));

                    if (t_isl_rec.e_src_type              == INP_SRC_TYPE_AV &&
                        t_isl_rec.t_avc_info.e_video_type == DEV_AVC_VGA)
                    {
                        MENU_LOG_ON_FAIL(acfg_get(IDX_CUSTOM_VGA_H_SIZE, &i2_hsize, &z_size));
                        _vid_process_pic_size(ui4_keycode,&i2_hsize);
                        MENU_LOG_ON_FAIL(acfg_set(IDX_CUSTOM_VGA_H_SIZE, &i2_hsize, sizeof(INT16)));
                    }
                    else
                    {
                        MENU_LOG_ON_FAIL(a_cfg_av_get(ui2_id, &i2_hsize));
                        _vid_process_pic_size(ui4_keycode,&i2_hsize);
                        MENU_LOG_ON_FAIL(a_cfg_av_set(ui2_id, i2_hsize));
                    }
                    MENU_LOG_ON_FAIL(a_cfg_av_update(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_OVERSCAN)));
                    MENU_LOG_ON_FAIL(a_cfg_update_viewport());
                    _vid_update_pic_size_info(ui1_input,i2_hsize,FALSE);
                }
                    break;
                case BTN_CURSOR_DOWN:
                case BTN_CURSOR_UP:
                {
                    INT16 i2_vsize = 0;
                    UINT32 ui2_id = CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_CUSTOM_V_SIZE);

                    MENU_LOG_ON_FAIL( _vid_size_set_adjust_btn_bk_image(h_g_pic_size_remote_vertical_add_v2, TRUE));

                    if (t_isl_rec.e_src_type              == INP_SRC_TYPE_AV &&
                        t_isl_rec.t_avc_info.e_video_type == DEV_AVC_VGA)
                    {
                        MENU_LOG_ON_FAIL(acfg_get(IDX_CUSTOM_VGA_V_SIZE, &i2_vsize, &z_size));
                       _vid_process_pic_size(ui4_keycode,&i2_vsize);
                        MENU_LOG_ON_FAIL(acfg_set(IDX_CUSTOM_VGA_V_SIZE, &i2_vsize, sizeof(INT16)));
                    }
                    else
                    {
                        a_cfg_av_get(ui2_id, &i2_vsize);
                       _vid_process_pic_size(ui4_keycode,&i2_vsize);
                        MENU_LOG_ON_FAIL(a_cfg_av_set(ui2_id, i2_vsize));
                    }
                    MENU_LOG_ON_FAIL(a_cfg_av_update(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO, APP_CFG_RECID_VID_OVERSCAN)));
                    MENU_LOG_ON_FAIL(a_cfg_update_viewport());
                    _vid_update_pic_size_info(ui1_input,i2_vsize,TRUE);
                }
                    break;
...
static INT32 _vid_size_w2s_str_replace(
                UTF16_T*  w2s_dst,
                SIZE_T    t_dst_len,
                UTF16_T*  w2s_src[],
                UINT32    ui4_src_sz
...
static INT32 _vid_size_page_create(UINT32 ui4_page_id,
                             VOID*  pv_create_data)
{
    MENU_PIC_SIZE_PAGE_DATA_T* pt_this = &t_g_pic_size_page_data;
    INT32 i4_ret = MENUR_OK;

    c_memset(pt_this, 0, sizeof(MENU_PIC_SIZE_PAGE_DATA_T));
    ...
    i4_ret = _vid_size_btn_create(pt_this->h_root_frm,
                                           _vid_size_btn_proc_fct,
                                           MENU_TEXT(MLM_MENU_KEY_ADJUST_PIC_POS),
                                           &pt_this->h_btn_adjust);
    MENU_LOG_ON_FAIL(i4_ret);

    i4_ret = _vid_size_center_ok_text(pt_this->h_btn_adjust, &pt_this->h_ok_text);
    MENU_LOG_ON_FAIL(i4_ret);

    i4_ret = _vid_size_help_create(pt_this->h_root_frm, &pt_this->h_help);
    MENU_LOG_ON_FAIL(i4_ret);

    _vid_size_sub_help_create(pt_this->h_root_frm, &pt_this->h_sub_help);
    MENU_LOG_ON_FAIL(i4_ret);
...
static INT32 _vid_size_page_destroy(UINT32 ui4_page_id)
{
    return MENUR_OK;
}

static VOID _vid_size_page_repaint_tips(VOID)
{
    INT32       i4_ret = MENUR_OK;

    i4_ret = c_wgl_repaint(t_g_pic_size_page_data.h_help, NULL, TRUE);
    MENU_LOG_ON_FAIL(i4_ret);

    i4_ret = c_wgl_repaint(t_g_pic_size_page_data.h_sub_help, NULL, TRUE);
    MENU_LOG_ON_FAIL(i4_ret);

    i4_ret = c_wgl_repaint(t_g_pic_size_page_data.h_btn_adjust, NULL, TRUE);
    MENU_LOG_ON_FAIL(i4_ret);

}

static INT32 _vid_size_page_show(UINT32 ui4_page_id)
{
    MENU_PIC_SIZE_PAGE_DATA_T* pt_this = &t_g_pic_size_page_data;
    INT32 i4_ret = MENUR_OK;
    ...
    i4_ret = _vid_size_set_user_info(MENU_TEXT(MLM_MENU_KEY_HELP_TEXT_HV_POSITION_1),
                            h_g_arrow_up,
                            h_g_arrow_right,
                            h_g_arrow_bottom,
                            h_g_arrow_left,
    ...
    if (1 == ui2_lang)
    {
        ...
        i4_ret = c_wgl_move(pt_this->h_help,
                    &t_rect,
                    WGL_NO_AUTO_REPAINT);
        MENU_LOG_ON_FAIL(i4_ret);

        i4_ret = _vid_size_set_user_info_new_parag(
                                MENU_TEXT(MLM_MENU_KEY_HELP_TEXT_HV_POSITION_1_FRENCH_SUB),
                                h_g_ok_key_select,
                                NULL_HANDLE,
                                NULL_HANDLE,
                                NULL_HANDLE,
    ...
    i4_ret = c_wgl_do_cmd(pt_this->h_sub_help,
                        WGL_CMD_TEXT_SET_TEXT,
                        WGL_PACK(MENU_TEXT(MLM_MENU_KEY_HELP_TEXT_HV_POSITION_2)),
                        WGL_PACK(0xff));
    MENU_LOG_ON_FAIL(i4_ret);
    ...
    do {
        ...
        UTF16_T * w2s_src[] = {L"up", L"right", L"down", L"left", L"ok"};
        c_uc_w2s_strncpy(buffer, MENU_TEXT(MLM_MENU_KEY_HELP_TEXT_HV_POSITION_1), 256);
        c_uc_w2s_strncat(buffer, MENU_TEXT(MLM_MENU_KEY_HELP_TEXT_HV_POSITION_2), 256-c_uc_w2s_strlen(buffer));
        _vid_size_w2s_str_replace(buffer, 256, w2s_src, 5);
    ...
    i4_ret = c_wgl_set_visibility(pt_this->h_sub_help,
                                  WGL_SW_NORMAL);
    MENU_LOG_ON_FAIL(i4_ret);

    _vid_size_set_adjust_btn_bk_image(h_g_pic_size_pos_remote_arrows_v2, FALSE);
    ...
    _vid_size_page_repaint_tips();


    return i4_ret;
}

static INT32 _vid_size_page_hide(UINT32 ui4_page_id)
{
    MENU_PIC_SIZE_PAGE_DATA_T* pt_this = &t_g_pic_size_page_data;
    INT32 i4_ret = MENUR_OK;
...
static INT32 _vid_size_page_get_focus(UINT32 ui4_page_id,
...
static INT32 _vid_size_page_lose_focus(UINT32 ui4_page_id)
{
    return MENUR_OK;
}

static INT32 _vid_size_page_update(UINT32 ui4_page_id,
...
{
    t_g_menu_page_vid_size.pf_menu_page_create =    _vid_size_page_create;
    t_g_menu_page_vid_size.pf_menu_page_destroy=    _vid_size_page_destroy;
    t_g_menu_page_vid_size.pf_menu_page_show=       _vid_size_page_show;
    t_g_menu_page_vid_size.pf_menu_page_hide=       _vid_size_page_hide;
    t_g_menu_page_vid_size.pf_menu_page_get_focus=  _vid_size_page_get_focus;
    t_g_menu_page_vid_size.pf_menu_page_lose_focus= _vid_size_page_lose_focus;
    t_g_menu_page_vid_size.pf_menu_page_update=     _vid_size_page_update;
