 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: menu_page_auto_phase.c,v $
 * $Revision: #1 $
 * $Date: 2015/05/29 $
...
/*-----------------------------------------------------------------------------
                        customizable macros
 ----------------------------------------------------------------------------*/
#define COMMON_AUTO_PHASE_TIMEOUT         30000
#define COMMON_AUTO_PHASE_ANIMATION_STEP  4
...
/*-----------------------------------------------------------------------------
                    data declarations
-----------------------------------------------------------------------------*/
MENU_PAGE_FCT_TBL_T  t_g_menu_common_page_auto_phase;
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static INT32 _auto_phase_create(VOID)
{
    ...
    SIZE_T  z_data_size)
{
    ...
    z_size = sizeof(ui2_val);

    //Fix Coverity ID : 1143056 Unchecked return value
    ...
    DBG_INFO(("phase = %d", ui2_val));

    if(*pb_success)
    {
        a_cfg_av_sync(CFG_MAKE_ID(APP_CFG_GRPID_VIDEO,
                                  APP_CFG_RECID_VID_YPBPR_PHASE));
        menu_nav_back();
    }
    else
    {
    ...
    SIZE_T  z_data_size)
{
    ...
    if(ui4_curr_time - ui4_start_time > COMMON_AUTO_PHASE_TIMEOUT)
    {
        BOOL    b_success = FALSE;

        DBG_ERROR(("<MENU>AUTO: timeout!!\n"));
        menu_async_invoke(_do_auto_done, &b_success, sizeof(BOOL), TRUE);
    }
    else
    {
        ...
        if(COMMON_AUTO_PHASE_ANIMATION_STEP > 0)
        {
            ui1_ani_step = (UINT8)((ui4_curr_time - ui4_start_time)/ 1000 % COMMON_AUTO_PHASE_ANIMATION_STEP);
        }
        else
        {
            ui1_ani_step = 0;
        ...
        c_uc_w2s_strncpy(w2s_msg,
                         MENU_TEXT(MLM_MENU_KEY_VID_AUTO_PHASE_MSG),
                         sizeof(w2s_msg) - COMMON_AUTO_PHASE_ANIMATION_STEP);
                         /* preserve space to store the last 4 char "***\0" */
        z_len = c_uc_w2s_strlen(MENU_TEXT(MLM_MENU_KEY_VID_AUTO_PHASE_MSG));

        for(ui2_i = 0 ;
            ui2_i < COMMON_AUTO_PHASE_ANIMATION_STEP;
            ui2_i ++)
        {
        ...
        a_ui_simple_dialog_set_text_align(h_simple_dialog,WGL_AS_LEFT_CENTER);
...
INT32 menu_common_page_auto_phase_init(VOID)
{
    INT32 i4_ret;
    t_g_menu_common_page_auto_phase.pf_menu_page_create =    _on_page_create;
    t_g_menu_common_page_auto_phase.pf_menu_page_destroy=    _on_page_destroy;
    t_g_menu_common_page_auto_phase.pf_menu_page_show=       _on_page_show;
    t_g_menu_common_page_auto_phase.pf_menu_page_hide=       _on_page_hide;
    t_g_menu_common_page_auto_phase.pf_menu_page_get_focus=  _on_page_get_focus;
    t_g_menu_common_page_auto_phase.pf_menu_page_lose_focus= _on_page_lose_focus;
    t_g_menu_common_page_auto_phase.pf_menu_page_update=     _on_page_update;
...
BOOL menu_common_page_auto_phase_is_enabled(VOID)
{
    INT32   i4_ret;
    ISL_REC_T   t_isl_rec;
...
#endif /* COMMON_AUTO_PHASE_ENABLE */
