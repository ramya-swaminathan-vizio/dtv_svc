 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * $RCSfile: menu_page_chg_pwd.c,v $
 * $Revision: #1 $
 * $Date: 2015/05/29 $
 * $Author: brianpc.huang $
 * $CCRevision: /main/DTV_X_HQ_int/DTV_X_ATSC/11 $
 * $SWAuthor: Richard Lu $
 * $MD5HEX: 3d2388cb49c194d240a3f7adb14167c5 $
 *
 * Description:
 *---------------------------------------------------------------------------*/
                        ...
                        )
{
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} PID:%d, TID:%d\n",
    ...
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Error: Invalid u8ClearButtonIdx:%d !!\n", __FUNCTION__,__LINE__, u8ClearButtonIdx));
        return;
    }
    ...
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Error: Invalid ui1_pwd_stat:%d !!\n", __FUNCTION__,__LINE__, ui1_pwd_stat));
        return;
    }

    DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} repaint button[%d]\n", __FUNCTION__,__LINE__, u8ClearButtonIdx));
    ...
    //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} call c_wgl_repaint(h_cnt_frm:0x%X)\n", __FUNCTION__,__LINE__, (unsigned int)h_cnt_frm)); );
    //i4_ret = c_wgl_repaint(h_cnt_frm, NULL, TRUE);
    i4_ret = c_wgl_repaint(ph_btn[u8ClearButtonIdx], NULL, TRUE);
 ...
 * Returns:
 ----------------------------------------------------------------------------*/
VOID a_set_5351_pin_state(UINT32 ui4_pin_state)
{
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d}Set ui4_reset_pin_password_flag=%d \n",__FUNCTION__,__LINE__, ui4_pin_state  )); );

	ui4_reset_pin_password_flag = ui4_pin_state;
}

/*----------------------------------------------------------------------------
 * Name: a_get_5351_pin_state
 *
 * Description:
 *
 * Inputs:
 ...
 * Returns:
 ----------------------------------------------------------------------------*/
UINT32 a_get_5351_pin_state(VOID)
{
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d}Get ui4_reset_pin_password_flag => %d \n",__FUNCTION__,__LINE__, ui4_reset_pin_password_flag  )); );

	return ui4_reset_pin_password_flag;
}
...
static VOID _digit_obscure_timer_restart(UINT8 u8ClearButtonIdx)
{
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} u8ClearButtonIdx:%d \n",__FUNCTION__,__LINE__, u8ClearButtonIdx )); );
                               ...
                               UTF16_T *pw2s_txt)
{
    ...
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Error: Invalid e_button:%d !!\n", __FUNCTION__,__LINE__, e_button));
    ...
    if (NULL == pw2s_txt)
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Error: NULL == pw2s_txt !!\n", __FUNCTION__,__LINE__));
        return MENUR_INV_ARG;
    }
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] %s(ui1_pwd_stat:%d, e_button:%d, txt:%c)\n",__FUNCTION__, ui1_pwd_stat, e_button, cTmp )); );

    //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s} h_btn:0x%X\n", __FUNCTION__, (unsigned int)h_btn )); );
...
{
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] %s(u8Idx:%d, cCh:%d)\n", __FUNCTION__, u8Idx, cCh )); );
    ...
    if( cCh == 0 )
        cTmp = 'X';
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d} Set ac_pwd[0][%d]={%c} \n",__FUNCTION__, __LINE__, u8Idx, cTmp )); );
...
{
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] %s(ui1_input:%d)  e_current_button:%d\n",__FUNCTION__, ui1_input, e_current_button )); );
    ...
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d}Error: Invalid e_current_button:%d \n",__FUNCTION__,__LINE__, e_current_button));
        return MENUR_INV_ARG;
    }
...
static INT32 _button_update_info(BOOL b_pushed, UINT8 u8ButtonIdx_old, UINT8 u8ButtonIdx_new)
{
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s(b_pushed:%d, u8ButtonIdx_old:%d, u8ButtonIdx_new:%d ) e_current_button:%d\n",
                        __FUNCTION__, b_pushed, u8ButtonIdx_old, u8ButtonIdx_new, e_current_button)); );

    //_digit_obscure_timer_restart();
    _digit_obscure_timer_stop();

    //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} ui1_pwd_stat:%d\n",__FUNCTION__,__LINE__, ui1_pwd_stat)); );

    if (CHG_PWD_STAT_NPWD == ui1_pwd_stat)
    {
        //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} ui1_pwd_stat:%d => CHG_PWD_STAT_NPWD\n",__FUNCTION__,__LINE__, ui1_pwd_stat)); );
        ph_btn = ah_btn_npwd;
    }
    else
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d}Error: ui1_pwd_stat:%d => !CHG_PWD_STAT_NPWD\n",__FUNCTION__,__LINE__, ui1_pwd_stat));
        return MENUR_INV_STATE;
    }

    //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} e_current_button:%d\n",__FUNCTION__,__LINE__, e_current_button)); );

    // Check button idx ...
    if( (u8ButtonIdx_new > BUTTON_LAST_ENTRY) )
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d}Error: Invalid u8ButtonIdx_new:%d\n",__FUNCTION__,__LINE__, u8ButtonIdx_new));
        return MENUR_INV_STATE;
    }

    if( (u8ButtonIdx_old >= BUTTON_LAST_ENTRY) )
    {
        DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d}Error: Invalid u8ButtonIdx_old:%d\n",__FUNCTION__,__LINE__, u8ButtonIdx_old));
        return MENUR_INV_STATE;
    }
    ...
    {
        //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} u8ButtonIdx:%d, h:0x%X \n",__FUNCTION__,__LINE__, u8ButtonIdx, (unsigned int)ph_btn[u8ButtonIdx] )); );

        //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} u8ButtonIdx:%d, ch:%d \n",__FUNCTION__,__LINE__, u8ButtonIdx, ac_pwd[ui1_pwd_stat][u8ButtonIdx] )); );
        ...
        //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} bDraw:%d \n",__FUNCTION__,__LINE__, bDraw )); );
        ...
        if( bDraw )
        {
            ...
            DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Draw buttron[%d] to '%c', Btn:0x%X\n",
  ...
  #if 0
    // Patch for UI not draw button 3,4
    if( b_pushed &&(u8ButtonIdx_old==3) ) //b_pushed:1, u8ButtonIdx_old:3, u8ButtonIdx_new:4
    {
        DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Patch: Repaint for Button 3,4, h_cnt_frm:0x%X \n",
            __FUNCTION__,__LINE__, (unsigned int)h_cnt_frm )); );
    ...
    if (u8ButtonIdx_new < BUTTON_LAST_ENTRY)
    {
        DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} Draw buttron[%d] \n",__FUNCTION__,__LINE__, u8ButtonIdx_new)); );
        c_wgl_do_cmd(ph_btn[u8ButtonIdx_new],
                    WGL_CMD_BTN_SET_PUSHED,
                    WGL_PACK(FALSE),
                    NULL);

        _button_set_title(ui1_pwd_stat, u8ButtonIdx_new, L"");
    }

    if (e_current_button >= BUTTON_2)
    {
        s_txt[0] = ac_pwd[ui1_pwd_stat][e_current_button-1];
        s_txt[1] = 0;
        ...
        _button_set_title(ui1_pwd_stat, e_current_button-1, w2s_ch);
    }

    if (e_current_button >= BUTTON_3)
    {
        c_wgl_do_cmd(ph_btn[e_current_button - 2],
                    WGL_CMD_BTN_SET_PUSHED,
                    WGL_PACK(TRUE),
                    NULL);
...
static BOOL _check_new_pwd(VOID)
{
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} ac_pwd[0][]={0x%X,0x%X,0x%X,0x%X} \n",
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d} i4_ret:%d, b_5351_pass:%d ==> return %d\n",__FUNCTION__,__LINE__, i4_ret, b_5351_pass, b_ret  )); );

    return b_ret;
}

static INT32 _go_to_next_page(VOID)
{
    if (ui4_page_id_pass == ui4_g_page_clear_memory_dialog)
    {
        menu_set_page_hide(FALSE);
        menu_nav_back();
        menu_pm_repaint_sync();
        menu_nav_go(ui4_g_page_clear_memory_dialog, NULL);
    }
    else
    {
        menu_nav_back_and_go(ui4_page_id_pass,NULL);
    }
                    ...
                    )
{
    ...
    switch (ui4_msg)
    {
        ...
        case WGL_MSG_KEY_REPEAT:
        {
            ...
            case BTN_CURSOR_LEFT:
                DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} h_widget:0x%X, ui4_msg:0x%X, ui4_keycode:0x%X\n",__FUNCTION__,__LINE__, h_widget, ui4_msg, ui4_keycode)); );
            ...
            case BTN_KB_9:
            {
                DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d} h_widget:0x%X, ui4_msg:0x%X, ui4_keycode:0x%X\n",__FUNCTION__,__LINE__, h_widget, ui4_msg, ui4_keycode));
                ...
                else if(e_current_button == BUTTON_4)
                {
                    ...
                    if (_check_new_pwd())
                    {
                        DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d} PW ok\n",__FUNCTION__,__LINE__  )); );
                    ...
                    {
                        DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d} PW NG\n",__FUNCTION__,__LINE__  )); );
...
static INT32 _page_reset(VOID)
{
    ...
    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN]{%s,%d} \n",__FUNCTION__,__LINE__)); );
                                ...
                                MENU_PAGE_HINT_T* pt_hint)
{
    ...
    if (pt_hint != NULL && MENU_PAGE_HT_CUSTOM == pt_hint->e_hint_type)
    {
        ui4_page_id_pass = (UINT32)pt_hint->u.pv_custom_hint;
    }
...
{
    t_g_menu_common_page_input_sys_pin.pf_menu_page_create =    _on_page_create;
    t_g_menu_common_page_input_sys_pin.pf_menu_page_destroy=    _on_page_destroy;
    t_g_menu_common_page_input_sys_pin.pf_menu_page_show=       _on_page_show;
    t_g_menu_common_page_input_sys_pin.pf_menu_page_hide=       _on_page_hide;
    t_g_menu_common_page_input_sys_pin.pf_menu_page_get_focus=  _on_page_get_focus;
    t_g_menu_common_page_input_sys_pin.pf_menu_page_lose_focus= _on_page_lose_focus;
    t_g_menu_common_page_input_sys_pin.pf_menu_page_update=     _on_page_update;
...
static VOID digit_pad_key_event_nfy(DIGIT_PAD_EVENT_TYPE event_type, VOID* pv_param)
{
	...
	DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} event_type:%d, PID:%d, TID:%d\n",__FUNCTION__,__LINE__, event_type, getpid(), MApi_Sys_get_thread_id()));
    ...
    switch(event_type)
    {
    	case DIGIT_PAD_DIGIT_KEY:
            ...
            DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} digit_val:%d\n",__FUNCTION__,__LINE__, digit_val)); );
    		...
    		if (digit_val == '0' || digit_val == '1' || digit_val == '2' ||
				...
				digit_val == '9' )
       		{
                ...
                DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} -1 ui1_input:%d, e_current_button:%d\n",__FUNCTION__,__LINE__, ui1_input, e_current_button));

                if(e_current_button != BUTTON_4)
                {
                    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} -2 ui1_input:%d, e_current_button:%d\n",__FUNCTION__,__LINE__, ui1_input, e_current_button)); );

                    _button_store_info(ui1_input);

                    e_current_button++;
                    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} -3 => e_current_button:%d\n",__FUNCTION__,__LINE__, e_current_button)); );
                ...
                {
                    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} e_current_button == BUTTON_4\n",__FUNCTION__,__LINE__ )); );
                    ...
                    if (_check_new_pwd())
                    {
                        DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} _check_new_pwd() => OK\n", __FUNCTION__,__LINE__ )); );
                    ...
                    {
                        DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} _check_new_pwd() => NG \n", __FUNCTION__,__LINE__ )); );
        ...
        case DIGIT_PAD_DEL_KEY:
            DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} e_current_button:%d\n",__FUNCTION__,__LINE__, e_current_button));
            if(e_current_button != BUTTON_1)
            {
                // Del current char
                ...
                //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} call menu_pm_repaint()\n", __FUNCTION__,__LINE__ )); );
                //menu_pm_repaint();
            }
	        break;
...
/* Fix Coverity Defect Id: 4658660 */
// fix function as other _digit_pad_recv_key_handle
                                          ...
                                          VOID* pv_param2)
{
    ...
    //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} ui4_msg:0x%X\n",__FUNCTION__,__LINE__, ui4_msg)); );

    switch (ui4_msg)
    {
        case WGL_MSG_KEY_DOWN:
        {
            //DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} MSG_KEY_DOWN ui4_keycode:%d\n",__FUNCTION__,__LINE__, ui4_keycode)); );
            ...
            switch (ui4_keycode)
            {
				...
				case BTN_KB_9:
				{
                    DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} WGL_MSG_KEY_DOWN-DIGI, h_digit_handle:0x%X, e_current_button:%d \n",__FUNCTION__,__LINE__, h_digit_handle, e_current_button));

					a_digit_pad_focus_btn_ok(h_digit_handle);
					if(e_current_button != BUTTON_4)
					{
					    DEBUG_MENU_SYS_PIN( DBG_LOG_PRINT(("[MENU][SYS_PIN] {%s,%d} e_current_button:%d, !=BUTTON_4  \n",__FUNCTION__,__LINE__, e_current_button)); );
