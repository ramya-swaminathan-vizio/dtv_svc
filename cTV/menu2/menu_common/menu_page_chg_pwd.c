...
// 2021/03/16:
// MVV-8448: When DTV-CC turn on, NAV will be very busy => menu_pm_repaint() failed!!
// I don't know why need to delay, and why need delay so long!?
// So, turn off delay first, and let customer to verify
#define CHG_PWD_SUCCESS_DELAY   0//5000
...
static INT32 _focus_to_save_btn(VOID)
{
    ...
    DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} \n",__FUNCTION__,__LINE__  )); );

    if( BUTTON_4 != e_current_button || ui1_pwd_stat != CHG_PWD_STAT_CPWD )
    {
        DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} Error: ui1_pwd_stat:%d, e_current_button:%d \n",__FUNCTION__,__LINE__, ui1_pwd_stat, e_current_button ));
...
extern void menu_pm_Set_nav_back_use_repaint_sync(BOOL bUseSyncApi);
...
static INT32 _save_correct_passwd(VOID)
{
  ...
  #if( CHG_PWD_SUCCESS_DELAY )
    {
        // 2021/03/16:
        // MVV-8448: When DTV-CC turn on, NAV will be very busy => menu_pm_repaint() failed!!
        // I don't know why need to delay, and why need delay so long!?
        // So, turn off delay first, and let customer to verify
        DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} delay %d ms\n",__FUNCTION__,__LINE__, CHG_PWD_SUCCESS_DELAY )); );
        /* delay to waiting for completion of repainting editbox. */
        c_thread_delay(CHG_PWD_SUCCESS_DELAY); // Why delay?
        DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} delay %d ms done\n",__FUNCTION__,__LINE__, CHG_PWD_SUCCESS_DELAY  )); );
    }
  #endif
    ...
    // get focus
    {
        ...
        DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} h_CurFocus:0x%X\n",__FUNCTION__,__LINE__, h_CurFocus  )); );
    }


    menu_pm_Set_nav_back_use_repaint_sync(TRUE); // Switch to using menu_pm_repaint_sync()
    DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} call menu_nav_back()\n",__FUNCTION__,__LINE__ )); );
    menu_nav_back();
    menu_pm_Set_nav_back_use_repaint_sync(FALSE); // Switch to using menu_pm_repaint()
     ...
     DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} call menu_pm_refresh()\n",__FUNCTION__,__LINE__ )); );
	 menu_pm_refresh();
...
static INT32 _button_update_ui(BOOL b_repaint)
{
    ...
    DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD] %s(b_repaint:%d) e_current_button:%d\n",__FUNCTION__,__LINE__, b_repaint, e_current_button  )); );

    /*update focus*/
    if(e_current_button < BUTTON_1 || e_current_button > BUTTON_4)
    {
        DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d}Error: Invalid e_current_button:%d\n",__FUNCTION__,__LINE__, e_current_button ));
        return MENUR_INV_ARG;
    }
    ...
    else if( ui1_pwd_stat == CHG_PWD_STAT_CPWD && e_current_button == BUTTON_4 )
    {
        ...
        {
            DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} PW OK\n",__FUNCTION__,__LINE__  ));
        ...
        {
            DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} PW NG\n",__FUNCTION__,__LINE__  ));
    ...
    if(b_repaint == TRUE)
    {
        DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} call menu_pm_repaint()\n", __FUNCTION__,__LINE__ )); );
        menu_pm_repaint();
    }
                        ...
                        )
{
    ...
    {
        DBG_LOG_PRINT(("[MENU][PWD]{%s,%s,%d} Error: Invalid ui1_pwd_stat:%d\n", __FILE__, __FUNCTION__, __LINE__, ui1_pwd_stat ));
        return ;
    }

    // Check e_current_button
    if( (e_current_button == 0)||(e_current_button > BUTTON_LAST_ENTRY) )
    {
        DBG_LOG_PRINT(("[MENU][PWD]{%s,%s,%d} Error: Invalid e_current_button:%d\n", __FILE__, __FUNCTION__, __LINE__, e_current_button ));
        return ;
    }
...
{
    DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} ui1_input:%d\n",__FUNCTION__,__LINE__, ui1_input  )); );
    ...
    DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} ui1_pwd_stat:%d, e_current_button:%d\n",__FUNCTION__,__LINE__, ui1_pwd_stat, e_current_button )); );

    if( ui1_pwd_stat >= CHG_PWD_STAT_NUM )
    {
        DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} Invalid ui1_pwd_stat:%d\n",__FUNCTION__,__LINE__, ui1_pwd_stat ));
        return MENUR_INV_ARG;
    }

    if( e_current_button >= BUTTON_LAST_ENTRY )
    {
        DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} Invalid e_current_button:%d\n",__FUNCTION__,__LINE__, e_current_button ));
        return MENUR_INV_ARG;
    }

    ac_pwd[ui1_pwd_stat][e_current_button] = ui1_input+'0';
    DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} Set ac_pwd[%d][%d] = %c\n",__FUNCTION__,__LINE__, ui1_pwd_stat, e_current_button, ac_pwd[ui1_pwd_stat][e_current_button] ));
...
static INT32 _button_update_info()
{
    ...
    DEBUG_MENU_CHG_PWD( DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} e_current_button:%d\n",__FUNCTION__,__LINE__, e_current_button  )); );
    ...
    for(i1_i = BUTTON_1 ; i1_i < BUTTON_LAST_ENTRY; i1_i++)
    {
...
static BOOL _check_new_pwd(VOID)
{
    ...
    DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%s,%d} \n", __FILE__, __FUNCTION__, __LINE__  ));
    ...
    DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%s,%d} return %d \n", __FILE__, __FUNCTION__, __LINE__, b_ret  ));
                            ...
                            HANDLE_T*   ph_widget)
{
    ...
    SET_RECT_BY_SIZE(&t_rect,
                     ...
                     CHG_PWD_TXT_NPWD_W,
                     CHG_PWD_TXT_NPWD_H);//Y -250 > Y-150
...
{
    t_g_menu_common_page_chg_pwd.pf_menu_page_create =    _on_page_create;
    t_g_menu_common_page_chg_pwd.pf_menu_page_destroy=    _on_page_destroy;
    t_g_menu_common_page_chg_pwd.pf_menu_page_show=       _on_page_show;
    t_g_menu_common_page_chg_pwd.pf_menu_page_hide=       _on_page_hide;
    t_g_menu_common_page_chg_pwd.pf_menu_page_get_focus=  _on_page_get_focus;
    t_g_menu_common_page_chg_pwd.pf_menu_page_lose_focus= _on_page_lose_focus;
    t_g_menu_common_page_chg_pwd.pf_menu_page_update=     _on_page_update;
    ...
    switch(event_type)
    {
        case DIGIT_PAD_DIGIT_KEY:
            ...
            if (digit_val == '0' || digit_val == '1' || digit_val == '2' ||
                ...
                digit_val == '9' )
            {
                DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} digit_val:%c, e_current_button:%d\n",__FUNCTION__,__LINE__, digit_val, e_current_button  ));
    ...
    switch (ui4_msg)
    {
        ...
        case WGL_MSG_KEY_REPEAT:
        {
            switch(ui4_keycode)
            {
                ...
                case BTN_KB_9:
                {
                    DBG_LOG_PRINT(("[MENU][CHG_PWD]{%s,%d} ui4_keycode:0x%X, e_current_button:%d\n",__FUNCTION__,__LINE__, ui4_keycode, e_current_button  ));
    ...
    VOID*  pv_create_data)
{
    ...
    cfg.position.ui4_top = CHG_PWD_TXT_NPWD_Y + 90 ; //Y-10 > Y+90
