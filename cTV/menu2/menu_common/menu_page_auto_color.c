...
/*-----------------------------------------------------------------------------
                        customizable macros
 ----------------------------------------------------------------------------*/
#define COMMON_AUTO_COLOR_TIMEOUT         30000
#define COMMON_AUTO_COLOR_ANIMATION_STEP  4
...
/*-----------------------------------------------------------------------------
                    data declarations
-----------------------------------------------------------------------------*/
MENU_PAGE_FCT_TBL_T  t_g_menu_common_page_auto_color;
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static INT32 _auto_color_create(VOID)
{
    ...
    SIZE_T  z_data_size)
{
    ...
    if(ui4_curr_time - ui4_start_time > COMMON_AUTO_COLOR_TIMEOUT)
    {
        BOOL    b_success = FALSE;
    ...
    else
    {
        ...
        if(COMMON_AUTO_COLOR_ANIMATION_STEP > 0)
        {
            ui1_ani_step = (UINT8)((ui4_curr_time - ui4_start_time)/ 1000 % COMMON_AUTO_COLOR_ANIMATION_STEP);
        }
        else
        {
            ui1_ani_step = 0;
        ...
        c_uc_w2s_strncpy(w2s_msg,
                         MENU_TEXT(MLM_MENU_KEY_VID_AUTO_COLOR_MSG),
                         sizeof(w2s_msg) - COMMON_AUTO_COLOR_ANIMATION_STEP
                         );
                         /* preserve space to store the last 4 char "***\0" */
        z_len = c_uc_w2s_strlen(MENU_TEXT(MLM_MENU_KEY_VID_AUTO_COLOR_MSG));
        for(ui2_i = 0 ;
            ui2_i < COMMON_AUTO_COLOR_ANIMATION_STEP;
            ui2_i ++)
        {
...
#if 0
        a_ui_simple_dialog_set_text(h_simple_dialog, w2s_msg);
#else
    ...
    MENU_PAGE_HINT_T* pt_hint)
{
    ...
    i4_ret = c_timer_start(h_timer, 1000,
                           X_TIMER_FLAG_REPEAT,
                           _timer_notify_fct,
                           NULL);
    MENU_CHK_FAIL(i4_ret);
...
INT32 menu_common_page_auto_color_init(VOID)
{
    INT32 i4_ret;

    t_g_menu_common_page_auto_color.pf_menu_page_create =    _on_page_create;
    t_g_menu_common_page_auto_color.pf_menu_page_destroy=    _on_page_destroy;
    t_g_menu_common_page_auto_color.pf_menu_page_show=       _on_page_show;
    t_g_menu_common_page_auto_color.pf_menu_page_hide=       _on_page_hide;
    t_g_menu_common_page_auto_color.pf_menu_page_get_focus=  _on_page_get_focus;
    t_g_menu_common_page_auto_color.pf_menu_page_lose_focus= _on_page_lose_focus;
    t_g_menu_common_page_auto_color.pf_menu_page_update=     _on_page_update;
...
BOOL menu_common_page_auto_color_is_enabled(VOID)
{
    INT32   i4_ret;
    ISL_REC_T   t_isl_rec;
...
#endif /* COMMON_AUTO_COLOR_ENABLE */
