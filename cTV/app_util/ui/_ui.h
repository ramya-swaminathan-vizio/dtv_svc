...
/* Return on error */
#define UI_CHK_FAIL(_ret)  \
do{ \
if (_ret < 0)   \
{   \
    DBG_ERROR(("<UI> Return fail:%s:%d:%d\r\n", __FILE__, __LINE__, _ret)); \
    return APP_UIR_FAIL;  \
}   \
}while(FALSE)


#define UI_LOG_ON_FAIL(_ret)  \
do{ \
if (_ret < 0)   \
{   \
    DBG_ERROR(("<UI>  fail:%s:%d:%d\r\n", __FILE__, __LINE__, _ret)); \
}   \
}while(FALSE)
...
-----------------------------------------------------------------------------*/
extern VOID    ui_set_dbg_level(UINT16 ui2_dbg_level);
extern UINT16  ui_get_dbg_level(VOID);
