 ...
 * Name:
 *
 * Description:  Customizable screen mode is key handler.
 *
 *
 * Inputs:
 *
 ...
 *
 ----------------------------------------------------------------------------*/
BOOL nav_screen_mode_view_custom_single_list_is_key_handler(
                    UINT32                      ui4_key_code
                    )
{
    switch (ui4_key_code) {
    ...
    case BTN_ASPECT:
    {
    ...
    #if(!NAV_SUPPORT_ASPECT_RATIO) // (MENU_SUPPORT_HOT_KEY_ASPECT_RATIO)
        //if( BTN_ASPECT == ui4_key_code )
        {
            DBG_LOG_PRINT(("[NAV][ASPECT]{%s,%d} Not support KEY_ASPECT !!\n", __FUNCTION__, __LINE__ ));
            return FALSE;
        }
    #endif
        ...
        /* Do not show the Wide mode UI when under OOBE */
