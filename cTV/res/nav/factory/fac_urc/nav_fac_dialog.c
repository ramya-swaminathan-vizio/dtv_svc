...
#ifdef APP_TPV_DFT_URC_WIFI   //ke_vizio_20130912 add
...
static INT32 _nav_fac_dialog_image_init(NAV_FAC_DIALOG_VIEW_T*    pt_nav_fac_dialog)
{
   ...
   i4_ret = c_wgl_img_tpl_create(WGL_IMG_ST_HV,
               &t_g_nav_frame_bk_bmp,
               &pt_nav_fac_dialog->h_ui_simple_dialog_bk0);
    if(WGLR_OK != i4_ret)
    {
        return NAVR_FAIL;
    }
                    ...
                    )
{
    ...
    switch (ui4_key_code)
    {
        ...
        case BTN_FAC_CTC:  //ke_vizio_20130603 modify
		{
			INT16   i2_val = 0;
	   ...
	   case BTN_FAC_ARC:
		...
		case BTN_FAC_LSR:
		{
				...
				if(i2_light_sensor == 0)	//Off
				{
	                ...
	                #ifdef APP_DFT_LSR_TEST //ke_vizio_20131220 add
                    _nav_fac_dialog_hide_timer_stop();
	                _nav_fac_dialog_hide_timer_start(2000);
	                #endif
				}
				else
				{
...
#if 0	// Factory change to disable it   Ben 20140725
 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name : a_nav_fac_dialog_register
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
INT32 a_nav_fac_dialog_register(VOID)
{
    NAV_COMP_T      t_comp;
    INT32           i4_ret;
...
extern VOID nav_fac_urc_key_pre_handler(UINT32 ui4_keystatus, UINT32* pt_ui4_keycode)
{
    ...
    if (ui4_fac_val & APP_CFG_CUST_FAC_RC_ON)
    {
        ...
        if(ui4_keycode == BTN_FAC_S_EFFECT)
...
//            ui4_keycode = BTN_P_EFFECT;
        ...
        if(ui4_keycode == BTN_FAC_SELECT)
            ui4_keycode = BTN_CURSOR_RIGHT;//BTN_SELECT;
