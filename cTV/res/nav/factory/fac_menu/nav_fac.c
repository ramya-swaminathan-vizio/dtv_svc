...
INT32 nav_fac_sce_pat_enable_pat(BOOL b_enable)
{
    ...
    if(b_enable)
    {
        ...
        #if 0  //ke_vizio_20121129  add
        nav_fac_c_rm_set_vid_comp((DRV_CUSTOM_VID_TYPE_T)DRV_CUSTOM_VID_TYPE_SCE_PATTERN,
                                      (VOID *)&t_set_info,
                                      z_size,
                                      FALSE);
        #else
    ...
    if ((c_strcmp (s_active_app, MENU_NAME) == 0))
    {
        ...
        if (pt_fac->ui2_key_num == NAV_FAC_KEY_STACK_NUM)
        {
            if (_nav_fac_check_key_stack())
            {
                ...
                {
                    acfg_SAC_load_default_channel("SZ");
                    b_nav_default_sz_ch_loading = FALSE;
                }
                ...
                {
                    acfg_SAC_load_default_channel("POL");
                    b_nav_default_pol_ch_loading = FALSE;
                }
            }
            else
    ...
    INT32*              pi4_val)
{
    ...
    switch (ui2_idx)
    {
        ...
        #ifdef FAC_INSTALL_ULPK
        case NAV_FAC_MENU_INSTALL_ULPK:/*//sam 0331 for ULPK install in fac menu*/
            i2_val = b_nav_fac_install_ulpk_done;
            break;
        #endif
#if 0	//kyo_20111221			
#ifdef FAC_ULPK_ERASE 
    ...
    INT32              i4_val)
{
    ...
    UINT16       ui2_i;		//kyo_20091002
    PANEL_DATA_T   t_panel_data;
    ...
    switch (ui2_idx)
    {
        ...
        case NAV_FAC_MENU_PANEL_TYPE:
        {
            ...
            for(ui2_i=0; ui2_i < ACFG_CLR_TEMP_NUM; ui2_i++)
            {
                //Re-write 1st part of fac clr. (YPrPb part)
                acfg_set_int16_by_idx(IDX_CLR_FAC_GAIN_R,   ui2_i,  t_panel_data.aai2_clr_gain[2*ACFG_CLR_TEMP_NUM + ui2_i][0]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_GAIN_G,   ui2_i,  t_panel_data.aai2_clr_gain[2*ACFG_CLR_TEMP_NUM + ui2_i][1]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_GAIN_B,   ui2_i,  t_panel_data.aai2_clr_gain[2*ACFG_CLR_TEMP_NUM + ui2_i][2]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_OFFSET_R, ui2_i,  t_panel_data.aai2_clr_offset[2*ACFG_CLR_TEMP_NUM + ui2_i][0]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_OFFSET_G, ui2_i,  t_panel_data.aai2_clr_offset[2*ACFG_CLR_TEMP_NUM + ui2_i][1]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_OFFSET_B, ui2_i,  t_panel_data.aai2_clr_offset[2*ACFG_CLR_TEMP_NUM + ui2_i][2]);

                //Re-write 2st part of fac clr. (VGA part)
                acfg_set_int16_by_idx(IDX_CLR_FAC_GAIN_R,   ui2_i+ACFG_CLR_TEMP_NUM,  t_panel_data.aai2_clr_gain[7*ACFG_CLR_TEMP_NUM + ui2_i][0]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_GAIN_G,   ui2_i+ACFG_CLR_TEMP_NUM,  t_panel_data.aai2_clr_gain[7*ACFG_CLR_TEMP_NUM + ui2_i][1]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_GAIN_B,   ui2_i+ACFG_CLR_TEMP_NUM,  t_panel_data.aai2_clr_gain[7*ACFG_CLR_TEMP_NUM + ui2_i][2]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_OFFSET_R, ui2_i+ACFG_CLR_TEMP_NUM,  t_panel_data.aai2_clr_offset[7*ACFG_CLR_TEMP_NUM + ui2_i][0]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_OFFSET_G, ui2_i+ACFG_CLR_TEMP_NUM,  t_panel_data.aai2_clr_offset[7*ACFG_CLR_TEMP_NUM + ui2_i][1]);
                acfg_set_int16_by_idx(IDX_CLR_FAC_OFFSET_B, ui2_i+ACFG_CLR_TEMP_NUM,  t_panel_data.aai2_clr_offset[7*ACFG_CLR_TEMP_NUM + ui2_i][2]);
            }
        ...
        case NAV_FAC_MENU_CURRENT_SOURCE://Emerson Add for changing the current source 2008-0919
            ...
            b_nav_fac_erase_ulpk_done = FALSE ;/*//sam 110504 for ULPK erase in fac menu*/
#endif			
#if 1	//kyo_20110425 add for MT5395			
...
INT32 nav_get_cur_model_info(VIZIO_MOEDL_INFO_T *pvt_info)
{
    ...
    UINT32      ui4_read_byte;
    CHAR        s_data_r[MODEL_NAME_LENGTH] = {0};
    ...
    if(c_strncmp(s_data_r, s_model_name,MODEL_NAME_LENGTH)!= 0)
    {
        /* Wirte the right panel model */
        UINT32		ui4_write_byte;
	    INT32       i4_ret;
...
{
   	UINT32    ui4_rw_byte = 0;
	UINT8 _au1HDCP[FAC_HDCP_Length] = 
	{
	...
	a_cfg_eep_raw_write(EEP_HDCP_ADDR, _au1HDCP, FAC_HDCP_Length, &ui4_rw_byte);
    return 0;
}
#endif
/*-----------------------------------------------------------------------------
