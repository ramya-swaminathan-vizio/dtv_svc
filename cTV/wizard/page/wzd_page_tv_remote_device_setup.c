...
#include "wizard/page/wzd_page_tv_remote_device_setup.h"
#include "wdk/a_vkb.h"
...
#include "wizard/page/wzd_page_tv_setup.h"
...
static INT32 _wzd_page_tv_remote_device_setup_brand_drop_down_lb_set_text_update(VOID)
{
    ...
    {
        DBG_ERROR( ("[Ln: %d] the brand num value is invalid! get_brand_number = %d\n", __LINE__, pt_this->ui4_brd_num));
        return WZDR_OK;
    }
...
static INT32 _wzd_page_tv_remote_device_setup_model_drop_down_lb_update( VOID )
{
    ...
    if (i4_ret != WZDR_OK)
    {
        DBG_ERROR(("[Ln: %d] a_dev_db_update_model_info fail! i4_ret = %d\n", __LINE__, i4_ret));
        return WZDR_FAIL;
    }     
...
static INT32 _wzd_page_tv_remote_device_setup_model_drop_down_lb_set_text_update(VOID)
{
    ...
    {
        DBG_ERROR( ("[Ln: %d] the model number value is invalid! get_modle_number = %d\n", __LINE__, pt_this->ui4_mdl_num));
        return WZDR_OK;
    }
...
static INT32 _wzd_page_tv_remote_device_setup_set_device_info(VOID)
{
    ...
    {
        DBG_ERROR(("[Ln: %d] [SET DEVICE INFO] sel_src_idx is error!!! pt_this->ui1_sel_src_idx = %d\n", __LINE__, pt_this->ui1_sel_src_idx));                                  
    }
    ...
    {        
        ...
        if (ui4_idx >= pt_this->pt_codeset_list->t_codeset_list1.i4_length)
        {
            if (NULL != pt_this->pt_codeset_list->t_codeset_list2.p_list[ui4_idx - pt_this->pt_codeset_list->t_codeset_list1.i4_length])
            {
                ...
                {
                    DBG_ERROR(("[Ln: %d] a_dev_db_upload_codeset_ex() list2.p_list fail!!! i4_ret = %d\n", __LINE__, i4_ret));
                }
        ...
        else
        {
            if (NULL != pt_this->pt_codeset_list->t_codeset_list1.p_list[ui4_idx])
            {
                ...
                {
                    DBG_ERROR(("[Ln: %d] a_dev_db_upload_codeset_ex()list1.p_list fail!!! i4_ret = %d\n", __LINE__, i4_ret));
                }
                /* set codeset */
...
static VOID _wzd_page_tv_remote_device_setup_do_retry_next_code(VOID)
{
    ...
    if (pt_this->ui1_cur_code_pos < pt_this->ui1_codeset_num)
    {
        ...
        {
            DBG_ERROR(("[Ln: %d] pt_this->ui1_codeset_num = %d is error!!!\n", __LINE__, pt_this->ui1_codeset_num));
        }
    }
    else
    {
        DBG_ERROR(("[Ln: %d] pt_this->ui1_cur_code_pos = %d is error!!!\n", __LINE__, pt_this->ui1_cur_code_pos));
    }
}
...
static VOID _wzd_page_tv_remote_device_setup_dev_db_online_query_hdlr(VOID* pv_tag1, VOID* pv_tag2, VOID* pv_tag3)
{
    ...
    if (DEV_DB_THREAD_QUERY_CODESET == pt_tag_query_info->e_query_type)
    {
        ...
        if (i4_ret != WZDR_OK)
        {
            DBG_ERROR(("[Ln: %d] a_dev_db_get_codeset()fail!!! i4_ret = %d\n", __LINE__, i4_ret));      
            return;
        }
    ...
    if (b_need_to_update_brd_ui == TRUE)    
    {
        ...
        if (i4_ret != WZDR_OK)
        {
            DBG_ERROR(("[Ln: %d] update brand listbox fail!!! i4_ret = %d\n", __LINE__, i4_ret));
            return;
        }
    }
    ...
    if(b_need_to_update_mdl_ui == TRUE)
    {
        ...
        if (i4_ret != WZDR_OK)
        {
            DBG_ERROR(("[Ln: %d] update model listbox fail!!! i4_ret = %d\n", __LINE__, i4_ret));
            return;
        }
    }
}
 ...
 *
 ----------------------------------------------------------------------------*/
static INT32 _wzd_page_tv_remote_device_setup_sel_page_set_losed_foucs_box_txt_initial(VOID)
{
    WZD_TV_REMOTE_DEVICE_SETUP_MEMBER_T* pt_this = &t_g_tv_remote_device_setup;
    WZD_SELECT_PAGE_T*                   pt_select_page = &t_g_device_setup_select_page;
...
static INT32 _wzd_page_tv_remote_device_setup_inp_page_set_foucsed_box_txt_hlt(UINT32 ui4_idx)
{
    ...
    t_img_info.e_type = WGL_IMG_SET_STANDARD;
    t_img_info.u_img_data.t_standard.t_disable = h_g_wzd_img_checkmark_green_large;
    t_img_info.u_img_data.t_standard.t_enable = h_g_wzd_img_checkmark_green_large;
    t_img_info.u_img_data.t_standard.t_highlight = h_g_wzd_img_checkmark_green_large;
    ...
    case WGL_MSG_KEY_DOWN:
        {
            ...
            {
            case BTN_SELECT:   
                _wzd_page_tv_remote_device_setup_sel_page_set_losed_foucs_box_txt_initial();
                c_memset(pt_this->s_type_name, 0, DEV_SETUP_DB_NAME_LEN_MAX + 1);
            ...
            case BTN_CURSOR_DOWN:
                ...
                {
                    _wzd_page_tv_remote_device_setup_sel_page_set_losed_foucs_box_txt_initial();
                    pt_this->ui4_sel_box_idx = pt_this->ui4_sel_box_idx + 5;
                    _wzd_page_tv_remote_device_setup_sel_page_set_losed_foucs_box_txt_hlt();
                    RET_ON_ERR (c_wgl_set_focus(pt_select_page->t_sel_box_btn[pt_this->ui4_sel_box_idx].h_cb_btn_frm, WGL_NO_AUTO_REPAINT));   
...
static INT32 _wzd_page_tv_remote_device_setup_set_choose_box_buttons_content (VOID)
{
	...
	if((e_dev_type_category != DEV_DB_DEV_TYPE_CTGY_ALL_DEV)||(c_strncmp(pt_this->crn_s639_lang, pt_this->pre_s639_lang, ISO_639_LANG_LEN) != 0))
	{
	    ...
	    if (i4_ret != 0)
	    {
	        DBG_ERROR(("[Ln: %d] a_dev_db_update_dev_type fail! i4_ret = %d\n", __LINE__, i4_ret));
	        return WZDR_FAIL;
	    }
    ...
    if (i4_ret != 0)
    {
        DBG_ERROR(("[Ln: %d] a_dev_db_get_dev_type_ex fail! i4_ret = %d\n", __LINE__, i4_ret));
        return WZDR_FAIL;
    }       
    
    if (pt_this->ui4_type_num < WZD_CB_BTN_COUNT) 
    {
        DBG_ERROR(("[Ln: %d]  pui4_dev_type_num = %d\n",  __LINE__, pt_this->ui4_type_num)); 
        return WZDR_FAIL;
    }
...
{ 
    ...
    if (ui1_rec_num >= 3)
    {
        ...
        {
           DBG_ERROR(("[Ln: %d] ui1_input_id[%d] get show idx error!!!\n", __LINE__, ui1_input_id, ui1_show_idx));
        }
    }
...
static INT32 _wzd_page_tv_remote_device_setup_get_device_paired_info (VOID)
{
    ...
    for (ui1_input_id = 1; ui1_input_id < ui1_rec_num; ui1_input_id++)
    {
...
/*----------------------------------------------------------------------------
 * Name: _wzd_page_tv_remote_device_setup_choose_page_select_box_mark_show
 *
 * Description: 
 *
 * Inputs:  -
 ...
 * Any other values            Failed.
 ----------------------------------------------------------------------------*/
static INT32 _wzd_page_tv_remote_device_setup_choose_page_select_box_mark_show (VOID)
{
    WZD_TV_REMOTE_DEVICE_SETUP_MEMBER_T* pt_this        = &t_g_tv_remote_device_setup;
    WZD_SELECT_PAGE_T*                   pt_select_page = &t_g_device_setup_select_page;
    UINT8                                ui1_show_idx;  
    ...
    for (ui1_type_id = WZD_CB_BTN_01; ui1_type_id < WZD_CB_BTN_COUNT; ui1_type_id++)
    {
        RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[ui1_type_id]).h_cb_icon_1, WGL_SW_HIDE));
    }
    ...
    for (ui1_show_idx = 0; ui1_show_idx <= ui1_rec_num - 2; ui1_show_idx++)
    {
        for (ui1_type_id = DEVICE_TYPE_SELECT_BLURAY_PLAYER; ui1_type_id <= DEVICE_TYPE_SELECT_VCR; ui1_type_id++)
        {
            if (pt_this->b_is_paired[ui1_show_idx][ui1_type_id] == TRUE)
            {
                RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[ui1_type_id]).h_cb_icon_1, WGL_SW_NORMAL));
            }
        }        
    }

    if (pt_this->b_is_aduio_paired == TRUE)
    {
        RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[DEVICE_TYPE_SELECT_AUDIO_RECIEVER]).h_cb_icon_1, WGL_SW_NORMAL));
    }
    else 
    {
        RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[DEVICE_TYPE_SELECT_AUDIO_RECIEVER]).h_cb_icon_1, WGL_SW_HIDE));
    }

    if (pt_this->b_is_soundbar_paired == TRUE)
    {
        RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[DEVICE_TYPE_SELECT_SOUND_BAR]).h_cb_icon_1, WGL_SW_NORMAL));
    }
    else 
    {
        RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[DEVICE_TYPE_SELECT_SOUND_BAR]).h_cb_icon_1, WGL_SW_HIDE));
    }
	for(ui1_show_idx=0;ui1_show_idx<WZD_CB_BTN_COUNT;ui1_show_idx++)
	{
		if(pt_this->b_is_type_sel[ui1_show_idx])
		{
			RET_ON_ERR (c_wgl_set_visibility ((pt_select_page->t_sel_box_btn[ui1_show_idx]).h_cb_icon_1, WGL_SW_NORMAL));
			wzd_view_main_frm_repaint();
		}
	}
...
/*----------------------------------------------------------------------------
 * Name: _wzd_page_tv_remote_device_setup_input_page_select_box_mark_show
 *
 * Description: 
 *
 * Inputs:  -
 ...
 * Any other values            Failed.
 ----------------------------------------------------------------------------*/
static INT32 _wzd_page_tv_remote_device_setup_input_page_select_box_mark_show (VOID)
{
    WZD_TV_REMOTE_DEVICE_SETUP_MEMBER_T* pt_this      = &t_g_tv_remote_device_setup;
    ...
    for (ui1_show_idx = 0; ui1_show_idx < ui1_rec_num; ui1_show_idx++)
    {
        h_sb_icon = wzd_view_sel_box_get_icon_1_handle(ui1_show_idx);
        RET_ON_ERR (c_wgl_set_visibility (h_sb_icon, WGL_SW_HIDE));  
    }
    ...
    for (ui1_show_idx = 0; ui1_show_idx < ui1_rec_num; ui1_show_idx++)
    {
        for (ui1_type_id = DEVICE_TYPE_SELECT_BLURAY_PLAYER; ui1_type_id <= DEVICE_TYPE_SELECT_VCR; ui1_type_id++)
        {
            ...
            if (pt_this->b_is_paired[ui1_show_idx][ui1_type_id] == TRUE)
            {
                RET_ON_ERR (c_wgl_set_visibility (h_sb_icon, WGL_SW_NORMAL));
            } 
        }
    }
	...
	for(ui1_show_idx=0;ui1_show_idx<WZD_CB_BTN_COUNT;ui1_show_idx++)
	{
		if(pt_this->b_is_input_sel[ui1_show_idx])
		{
			h_sb_icon = wzd_view_sel_box_get_icon_1_handle(ui1_show_idx);
			RET_ON_ERR (c_wgl_set_visibility (h_sb_icon, WGL_SW_NORMAL));
		}
...
static INT32 _wzd_page_tv_remote_device_setup_set_sel_box_theme (UINT8 ui1_rec_num)
{
    ...
    {       
        ...
        /* move position */
        SET_RECT_BY_SIZE (& t_rect,
                          WZD_SEL_BOX_BTN_ICON_2_X - 10 * WZD_UI_RATIO_W,
                          WZD_SEL_BOX_BTN_ICON_2_Y + 40 * WZD_UI_RATIO_W,
...
static INT32 _wzd_page_tv_remote_device_setup_set_sel_box_content(VOID)
{
    ...
    { 
        ...
        if (i4_ret != ISLR_OK) {
            DBG_ERROR(("[Ln: %d]  ui2_idx = %d, ui1_rec_num = %d, i4_ret = %d!!!!\n",  __LINE__, ui2_src_idx, ui1_rec_num, i4_ret)); 
            return WZDR_FAIL;
        }
...
{    
    WZD_TV_REMOTE_DEVICE_SETUP_MEMBER_T* pt_this = &t_g_tv_remote_device_setup; 
    //UTF16_T      w2s_dev_view_name[DEV_DB_NAME_LEN_MAX + 1] = {0};
    ...
    if (ui4_msg == VKB_MSG_DLG_KEY_INPUT)
    {
        ...
        { 
            ...
            else
            {
                ...
                if (i4_ret != WZDR_OK)
                {
                    DBG_ERROR(("[Ln: %d] Model_drop_down_lb_update FAIL! i4_ret = %d\n", __LINE__, i4_ret)); 
                    RET_ON_ERR(c_wgl_get_visibility(pt_model_page->h_model_lb_frm, &b_is_visible)); 
                    if ( b_is_visible == TRUE )
                    {
                        RET_ON_ERR (c_wgl_set_visibility (pt_model_page->h_model_lb_frm, WGL_SW_HIDE_RECURSIVE));
        ...
        {   
            ...
            if (h_focus == pt_model_page->h_model_eb)
            {
                ...
                else 
                {
                    ...
                    if (i4_ret != WZDR_OK)
                    {
                        DBG_ERROR(("[Ln: %d] [MODEL_BTN_CURSOR_LEFT] Model_drop_down_lb_update FAIL! i4_ret = %d\n", __LINE__, i4_ret)); 
                        RET_ON_ERR(c_wgl_get_visibility(pt_model_page->h_model_lb_frm, &b_is_visible)); 
                        if ( b_is_visible == TRUE )
                        {
                           RET_ON_ERR (c_wgl_set_visibility (pt_model_page->h_model_lb_frm, WGL_SW_HIDE_RECURSIVE));
    ...
    if (ui4_msg == VKB_MSG_DLG_KEY_INPUT)
    {
        ...
        {  
            ...
            else
            {
                ...
                if (i4_ret != WZDR_OK)
                {
                    DBG_ERROR(("[Ln: %d] Brand_drop_down_lb_update FAIL! i4_ret = %d\n", __LINE__, i4_ret)); 
                    RET_ON_ERR(c_wgl_get_visibility(pt_brand_page->h_brand_lb_frm, &b_is_visible)); 
                    if ( b_is_visible == TRUE )
                    {
                        RET_ON_ERR (c_wgl_set_visibility (pt_brand_page->h_brand_lb_frm, WGL_SW_HIDE_RECURSIVE));
...
static INT32 _wzd_page_tv_remote_device_setup_code_text_connect (UTF16_T* code_w2s_prefix, UTF16_T* code_w2s)
{
    ...
    {
       DBG_ERROR(("[Ln: %d] brand is null or brand and model are all null!\n", __LINE__));
    }
...
static INT32 _wzd_page_tv_remote_device_setup_confirm_skip_full_dlg_text_connect (UTF16_T* dlg_w2s)
{
    ...
    ISL_REC_T      t_isl_rec = {0};
    INT32          i4_ret = WZDR_OK;
	CHAR           s_rf_name[DEV_SETUP_DB_NAME_LEN_MAX + 1] = "Coaxial";
	UINT8		   ui1_rec_num = 0;
	...
	else
	{
	    ...
	    if (i4_ret != ISLR_OK) {
	        DBG_ERROR(("[Ln: %d] pt_this->ui1_sel_src_idx = %d, i4_ret = %d!!!!\n",  __LINE__, pt_this->ui1_sel_src_idx, i4_ret)); 
	        return WZDR_FAIL;
	    }
	    c_uc_w2s_to_ps(a_isl_get_display_name(&t_isl_rec), s_src_name, DEV_SETUP_DB_NAME_LEN_MAX);
...
static INT32 _wzd_page_tv_remote_device_setup_select_show(BOOL b_repaint)
{
    ...
    RET_ON_ERR (_wzd_page_tv_remote_device_setup_choose_page_select_box_mark_show());
	
    RET_ON_ERR (c_wgl_set_focus(pt_select_page->t_sel_box_btn[WZD_CB_BTN_01].h_cb_btn_frm, WGL_NO_AUTO_REPAINT));
    RET_ON_ERR (_wzd_page_tv_remote_device_setup_sel_page_set_losed_foucs_box_txt_hlt());
    RET_ON_ERR (_wzd_page_tv_remote_device_setup_choose_box_btns_set_all_unfloat());
    RET_ON_ERR (c_wgl_float(pt_select_page->t_sel_box_btn[WZD_CB_BTN_01].h_cb_btn_frm, TRUE, WGL_NO_AUTO_REPAINT));
...
static INT32 _wzd_page_tv_remote_device_setup_input_show(BOOL b_repaint)
{
    ...
    RET_ON_ERR (_wzd_page_tv_remote_device_setup_input_page_select_box_mark_show());
    RET_ON_ERR (wzd_view_sel_box_set_focus (WZD_SEL_BOX_BTN_01, FALSE));
    RET_ON_ERR (wzd_view_footer_set_visibility (TRUE));
...
static INT32 _wzd_page_tv_remote_device_setup_update_by_state(BOOL b_repaint)
{
    ...
    if (DEVICE_SETUP_CHOOSE_DEVICE == pt_this->e_state)
    {
        ...
        pt_this->ui4_sel_box_idx = WZD_CB_BTN_01;
...
static INT32 _wzd_page_tv_remote_device_setup_on_show (UINT32  ui4_page_id)
{
    ...
    if (FALSE == pt_this->b_is_notify_reg)
    {
        ...
        if (WZDR_OK != i4_ret)
        {
            DBG_ERROR(("[Ln: %d][nav_dev_db] a_dev_db_register() i4_ret = %d\n", __LINE__, i4_ret));
            return i4_ret;
        }
    }
                ...
                case BTN_SELECT:
                    {
                        ...
                        else if( DEVICE_SETUP_SELECT_BRAND == pt_this->e_state)
                        {
                            if (WZD_FOOTER_BTN_01 == (WZD_FOOTER_BTN_INDEX_T)pv_param2) /* next button okey */ 
                            {
                                ...
                                if (pt_this->b_is_db_locked)
                                {
                                    ...
                                    if (i4_ret != WZDR_OK)
                                    {
                                        DBG_ERROR(("[Ln: %d] [BRAND_BTN_NEXT]_wzd_page_tv_remote_device_setup_update_codeset() is Fail!. i4_ret = %d\n", __LINE__, i4_ret));
                                ...
                                else
                                {
                                	if((pt_this->ui4_type_sel_idx == DEVICE_TYPE_SELECT_SOUND_BAR)&&(c_strcmp(pt_this->s_brd_name, "VIZIO")==0))
                            		{
		                                ...
		                                if (i4_ret != WZDR_OK)
		                                {
		                                    DBG_ERROR(("[Ln: %d] _wzd_page_tv_remote_device_setup_update_codeset_with_brand() is Fail!, i4_ret = %d\n", __LINE__, i4_ret));
		                                    pt_this->b_is_codeset_querying = FALSE;
		                                    _wzd_page_tv_remote_device_setup_update_codeset_exciption_handle();
#ifdef APP_UEI_SPINNER_SUPPORT
											/*hide spinner after get codeset for confirm remote conection page*/
                        ...
                        else if( DEVICE_SETUP_SELECT_MODEL == pt_this->e_state )
                        {
                            ...
                            { 
                                ...
                                if (i4_ret != WZDR_OK)
                                {
                                    DBG_ERROR(("[Ln: %d] _wzd_page_tv_remote_device_setup_update_codeset()is Fail!, i4_ret = %d\n", __LINE__, i4_ret));
                                    _wzd_page_tv_remote_device_setup_update_codeset_exciption_handle();
                                }
                            }
                ...
                case BTN_CURSOR_UP:
                    {
                        if( DEVICE_SETUP_CHOOSE_DEVICE == pt_this->e_state )
                        {
                            pt_this->ui4_sel_box_idx = WZD_CB_BTN_01;
                            _wzd_page_tv_remote_device_setup_sel_page_set_losed_foucs_box_txt_hlt();
                            c_wgl_set_focus(pt_select_page->t_sel_box_btn[WZD_CB_BTN_01].h_cb_btn_frm, WGL_NO_AUTO_REPAINT);
