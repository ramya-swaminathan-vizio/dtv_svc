...
static INT32 _wzd_page_tv_name_input_set_device_info(VOID)
{
    ...
    if (i4_ret != WZDR_OK)
    {
        DBG_ERROR(("[Ln: %d] a_cfg_custom_set_inp_name(). i4_ret = %d\n", __LINE__,i4_ret));
        return WZDR_FAIL;      
    }
        ...
        case WZD_SEL_BOX_MSG_KEY_INPUT:
            switch((UINT32)pv_param1)
            {
                case BTN_SELECT:
					...
					/*should not set device info when select RF*/
...
{ 
    ...
    UINT8        ui1_comp_offset[10] = {0,   //DEV_AVC_COMP_VIDEO,           
                                        0,   //DEV_AVC_S_VIDEO,              
                                        0,   //DEV_AVC_Y_PB_PR,              
                                        0,   //DEV_AVC_VGA,                  
                                        0,   //DEV_AVC_SCART,                
    ...
    for(ui1_idx = 0; ui1_idx < ui1_rec_num; ui1_idx++)
    {
        ...
        else if (t_isl_rec.t_avc_info.e_video_type == DEV_AVC_COMP_VIDEO
    ...
    if (ui1_rec_num >= 3)
    {
        ...
        {
                                        /* VGA      COMP/AV     TV */
            ui1_show_idx = ui1_input_id-ui1_vga_num-ui1_comp_num-1;
        }
        else if (DEV_AVC_COMP_VIDEO == t_isl_rec.t_avc_info.e_video_type 
            || DEV_AVC_COMBI == t_isl_rec.t_avc_info.e_video_type
            || DEV_AVC_Y_PB_PR == t_isl_rec.t_avc_info.e_video_type)                  /* COMP */
        {
            ui1_show_idx = ui1_hdmi_num+ui1_input_id-1 \
              -ui1_comp_offset[t_isl_rec.t_avc_info.e_video_type-DEV_AVC_COMP_VIDEO];
        }
        else if ( DEV_AVC_VGA == t_isl_rec.t_avc_info.e_video_type )                /* RGB */
        {
            ui1_show_idx = 0xFF;
        }
        else
...
static INT32 _wzd_page_tv_name_input_set_sel_box_content(VOID)
{
    ...
    a_isl_get_num_rec(&ui1_rec_num);
    for (ui2_input_idx = 1; ui2_input_idx < ui1_rec_num; ui2_input_idx++)
    { 
        c_memset(&t_isl_rec, 0, sizeof(ISL_REC_T));
        ...
        if (t_isl_rec.e_src_type == INP_SRC_TYPE_AV && DEV_AVC_HDMI == t_isl_rec.t_avc_info.e_video_type)  
        {
            ...
            || DEV_AVC_COMBI == t_isl_rec.t_avc_info.e_video_type
            || DEV_AVC_Y_PB_PR == t_isl_rec.t_avc_info.e_video_type))         
        {
...
/*----------------------------------------------------------------------------
 * Name: _wzd_page_tv_name_input_page_select_box_mark_show
 *
 * Description: 
 *
 * Inputs:  -
 ...
 * Any other values            Failed.
 ----------------------------------------------------------------------------*/
static INT32 _wzd_page_tv_name_input_page_select_box_mark_show (VOID)
{
    WZD_TV_DEVICE_NAME_INPUT_MEMBER_T*   pt_this = &t_g_tv_name_input;
    ...
    for (ui1_show_idx = 0; ui1_show_idx < ui1_rec_num; ui1_show_idx++)
    {
        ...
        {
            DBG_INFO(("[Ln: %d] [DEVICE LIST] input_src_named[input(%d)][type(%d)] = %d.\n", __LINE__, 
                        ui1_show_idx, 
                        ui1_type_id, 
                        pt_this->b_is_named[ui1_show_idx][ui1_type_id]));

            h_sb_icon = wzd_view_sel_box_get_icon_1_handle(ui1_show_idx);
             
            if (pt_this->b_is_named[ui1_show_idx][ui1_type_id] == TRUE)
            {
...
static INT32 _wzd_page_tv_name_input_select_input_show(VOID)
{
    ...
    if (pt_this->e_name_input_enter_reason == NAME_INPUT_ENTER_REASON_TV_SETUP)
    {
    	if(pt_this->ui4_type_sel_idx == DEVICE_TYPE_SELECT_CABLE_BOX)
    	{
          ...
          /* Set description */
          RET_ON_ERR (_wzd_page_tv_name_input_set_page_text (pt_input_page->h_input_desc_txt, WZD_TEXT(MLM_WZD_KEY_DEVICE_SETUP_SEL_INPUT_MSG_CABLE)));
    	}
    	else if(pt_this->ui4_type_sel_idx == DEVICE_TYPE_SELECT_SATELLITE)
    	{
            ...
            /* Set description */
            RET_ON_ERR (_wzd_page_tv_name_input_set_page_text (pt_input_page->h_input_desc_txt, WZD_TEXT(MLM_WZD_KEY_DEVICE_SETUP_SEL_INPUT_MSG_SAT)));
    	}
    }
    else
    {
    ...
    RET_ON_ERR (_wzd_page_tv_name_input_page_select_box_mark_show());
