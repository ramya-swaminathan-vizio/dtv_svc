 ...
 *
 *---------------------------------------------------------------------------*/
#if (!defined(APP_DVBT_SUPPORT) && !defined(APP_ISDB_SUPPORT) && !defined(APP_TWN_SUPPORT))
...
static VOID _reset_scanning_flag (HANDLE_T   h_builder)
{
    ...
    else if (h_builder == t_g_util.h_atsc_air)
    {
        t_g_util.b_atsc_air_scanning = FALSE;
    }
    else if (h_builder == t_g_util.h_atsc_cable)
    {
        t_g_util.b_atsc_cable_scanning = FALSE;
    }
    else
    {
        /* do nothing */
...
static INT32 _wzd_util_sb_init (VOID)
{
    ...
    if (t_g_util.h_atsc_air == NULL_HANDLE)
    {
        ...
        RET_ON_ERR (c_sb_open_builder(WZD_UTIL_ATSC_AIR_ENG_NAME, 
                                      & t_sb_atsc_air_data, 
                                      NULL, _sb_nfy_fct, & t_g_util.h_atsc_air, & e_cond));
    }

    /* Open SVL Builder of the ATSC CABLE */
    if (t_g_util.h_atsc_cable == NULL_HANDLE)
    {
        ...
        RET_ON_ERR (c_sb_open_builder(WZD_UTIL_ATSC_CABLE_ENG_NAME, 
                                      ...
                                      NULL, _sb_nfy_fct, & t_g_util.h_atsc_cable, & e_cond));
    }
...
static INT32 _wzd_util_update_sb_snk_name(
    HANDLE_T    h_sb)
{
    INT32   i4_ret;
...
INT32 wzd_util_scan_start (UINT16   ui2_scanning_type)
{
    ...
    if (ui2_scanning_type == WZD_SB_SCANNING_AIR)
    {
        ...
        RPC_ADD_REF_DESC(RPC_DEFAULT_ID, & t_ntsc_scan_data, RPC_DESC_SB_NTSC_SCAN_DATA_T, NULL);
        ...
        DBG_INFO(("<WZD> scan start: DTV ATSC AIR Svl Builder.\r\n"));
        c_memset(& t_atsc_air_scan_data, 0, sizeof(WZD_UTIL_ATSC_AIR_SCAN_DATA_T));
        t_atsc_air_scan_data.e_scan_type   = WZD_UTIL_ATSC_AIR_SCAN_TYPE;   
        t_atsc_air_scan_data.ui4_scan_info = 0;

        RPC_ADD_REF_DESC(RPC_DEFAULT_ID, & t_atsc_air_scan_data, RPC_DESC_SB_ATSC_SCAN_DATA_T, NULL);
        i4_rc = c_sb_start_scan(t_g_util.h_atsc_air, 
                                & t_atsc_air_scan_data, & t_scan_info_dtv, _scan_nfy_fct, & e_cond);

        t_g_util.b_atsc_air_scanning = (i4_rc == SBR_OK) ? TRUE : FALSE;

        /* check the scan status */
        if (t_g_util.b_ntsc_air_scanning && t_g_util.b_atsc_air_scanning)
        {
            b_successful = TRUE;
        }
    }
    else /* (ui2_scanning_type == WZD_SB_SCANNING_CABLE) */
    {
        ...
        c_memset(& t_ntsc_scan_data, 0, sizeof(WZD_UTIL_NTSC_SCAN_DATA_T));
        t_ntsc_scan_data.ui1_freq_plan = WZD_UTIL_NTSC_CABLE_FREQ_PLAN;
        t_ntsc_scan_data.e_scan_type   = WZD_UTIL_NTSC_SCAN_TYPE;
        t_ntsc_scan_data.ui2_start_idx = 0; 
        t_ntsc_scan_data.ui2_end_idx   = 0;
        ...
        t_atsc_cable_scan_data.e_scan_type   = WZD_UTIL_ATSC_CABLE_SCAN_TYPE;
        t_atsc_cable_scan_data.ui4_scan_info = 0; 
        ...
        i4_rc = c_sb_start_scan(t_g_util.h_atsc_cable, 
                                & t_atsc_cable_scan_data, & t_scan_info_dtv, _scan_nfy_fct, & e_cond);

        t_g_util.b_atsc_cable_scanning = (i4_rc == SBR_OK) ? TRUE : FALSE;

        /* check the scan status */
        if (t_g_util.b_ntsc_cable_scanning && t_g_util.b_atsc_cable_scanning)
        {
            b_successful = TRUE;
        }
    }
...
INT32 wzd_util_scan_cancel (UINT16   ui2_scanning_type)
{
    ...
    if (ui2_scanning_type == WZD_SB_SCANNING_AIR)
    {
        ...
        if (t_g_util.b_atsc_air_scanning)
        {
            DBG_INFO(("<WZD> scan cancel: DTV ATSC AIR Svl Builder.\r\n"));
            c_sb_cancel_scan(t_g_util.h_atsc_air);
            t_g_util.b_atsc_air_scanning = FALSE;
        }
    }
    else /* (ui2_scanning_type == WZD_SB_SCANNING_CABLE) */
    {
        ...
        if (t_g_util.b_atsc_cable_scanning)
        {
            DBG_INFO(("<WZD> scan cancel: DTV ATSC CABLE Svl Builder.\r\n"));
            c_sb_cancel_scan(t_g_util.h_atsc_cable);
            t_g_util.b_atsc_cable_scanning = FALSE;
        }
    }
...
INT32 wzd_util_init (VOID)
{
    ...
    t_g_util.h_atsc_air   = NULL_HANDLE;
    t_g_util.h_atsc_cable = NULL_HANDLE;
