                                 ...
                                 VOID*    pv_param2)
{
    ...
    switch( ui4_msg )
    {
        ...
        case WZD_DLG_MSG_KEY_INPUT:
            DBG_LOG_PRINT(( "[WZD][DLG CLI Test] WZD_DLG_MSG_KEY_INPUT received, keycode =%d\n", (UINT32)pv_param1 ));
...
static INT32 _wzd_app_init_fct (const CHAR*    ps_name,
                                HANDLE_T       h_app)
{
    DBG_INFO (("initializing Wizard (application handle = %d)\n\r", h_app));
...
static INT32 _wzd_app_resume_fct (HANDLE_T    h_app)
{
    ...
    DBG_INFO (("resume Wizard (handle = %d)\n\r", h_app));
...
#if 0 
    /*set backlight control to off in wzd_custom_resume*/
    ...
    /*set light sensor to off in wzd_custom_resume*/
 ...
 /*Set smart dimming off in wzd_custom_resume*/
    ...
    {
	    if(a_wzd_is_resume_from_menu_setup() == TRUE)   //ke_vizio_20140103
	    {   
...
static INT32 _wzd_app_pause_fct (HANDLE_T    h_app)
{
    ...
    DBG_INFO (("pause Wizard (handle = %d)\n\r", h_app));
    ...
    /* re-trigger country code change nfy */
    ...
    switch (ui4_type)
    {
        ...
        case WZD_MSG_WGL_NFY: /* widget notifications */
        {
            ...
            if (t_g_wzd.ui1_page_idx == pt_wgl_msg->ui1_cur_pg_idx)
            {
	            ...
	            {
	                DBG_ERROR (("ERR: dispatch event to wgl failed = %d\n\r", i4_rc));
	            }
            }
#ifdef APP_WZD_AUTO_PAUSE
  ...
  case WZD_MSG_WGL_SETI_LST_ELEM_HLT:
        ...
        default: /* unknown messages */
        {
            ...
            case AM_BRDCST_MSG_PRE_POWER_OFF:
                ...
                DBG_LOG_PRINT(("[WZD] a_tv_stop after receive AM_BRDCST_MSG_POWER_OFF.\n"));
                break;
...
INT32 wzd_chg_inp_src_to_hdmi(VOID)
{
    ...
    DBG_API(("[WZD] change input source to HDMI!\n"));
...
#endif
    DBG_LOG_PRINT(("[WZD][_wzd_app_power_seq_cb] (1)b_resume = %d\n", b_resume));
    if (b_resume == FALSE)
    {
        b_resume = a_wzd_is_resume ();
        DBG_LOG_PRINT(("[WZD][_wzd_app_power_seq_cb] (2)b_resume = %d\n", b_resume));
    }
...
static VOID _wzd_nfy_timer (HANDLE_T    h_timer,
                            VOID*       pv_tag)
{
    DBG_INFO (("auto-pause timer expired\n\r"));
                               ...
                               BOOL  b_repaint)
{
    ...
    DBG_INFO (("update page information (idx = %d)\n\r", ui1_page_idx));

    /* Reset the background for closing the small video display window 
	 *if small video had been displayed.*/
	...
	//RET_ON_ERR( wzd_page_nw_setup_sel_box_hide());

#ifdef APP_2K12_RETAIL_DEMO_MODE
    /* hide choose mode page */
    wzd_page_choose_mode_on_hide();
#endif
    ...
    switch (pt_page->pt_page_data->t_page_type)
    {
        ...
        default:
        {
            DBG_INFO (("Unknown view type of the template layout\n\r"));
        }
        break;
    }
...
static INT32 _wzd_scan_start (VOID)
{
    ...
    if (!wzd_is_scan_active ())
    {
        ...
        // start scan, set the crnt and last channel id INVALID
        ...
        DBG_INFO (("Broadcast the scan avtive message. [START]\r\n"));
...
static INT32 _wzd_scan_progress (SB_NFY_MSG_T*      pt_sb_nfy)
{
        ...
        wzd_is_scan_active ())
    {
        ...
        DBG_INFO (("scan progress. [ATV CH: %d] [DTV CH: %d] [PROGRESS: %d]\n\r",
...
/* DTV00139009	When support DVB-C, after scan complete, should goto the last page directly.  <Brooke  090811>  */
    ...
    {
        c_dbg_ctrl_stmt("call wzd_get_nwl_info return fail!\n");
    }
#endif
	if(t_g_wzd.b_g_is_rf_scan)
	{
    ...
    /* Broadcast the scan avctive message */
    DBG_INFO (("Broadcast the scan avtive message. [STOP]\r\n"));
...
static INT32 _wzd_set_status (VOID)
{
	...
	DBG_INFO (("set wizard status [state = %d] [page = %d]\n\r", t_g_wzd.ui1_state, t_g_wzd.ui1_page_idx));
...
static INT32 _wzd_get_status (VOID)
{
    ...
    DBG_INFO (("get wizard status [state = %d] [page = %d]\n\r", t_g_wzd.ui1_state, t_g_wzd.ui1_page_idx));
...
static INT32 _wzd_store_svl_and_config (VOID)
{
    ...
    {
        DBG_INFO(("Begin storing config\n\r"));
        if (a_cfg_store() != APP_CFGR_OK)
        {
            DBG_INFO(("Can't store config.\n\r"));
        }
        else
        {
            DBG_INFO(("End storing config\n\r"));
        }
    }
    ...
    {
        DBG_INFO(("Begin storing SVL, TSL, NWL, LOL.\n\r"));
        if (a_cfg_ch_lst_store() != APP_CFGR_OK)
        {
            DBG_INFO(("Can't store SVL, TSL, NWL, LOL.\n\r"));
        }
        else
        {
            DBG_INFO(("End storing SVL, TSL, NWL, LOL.\n\r"));
        }
    }
    t_g_wzd.b_svl_dirty = FALSE;
                        ...
                        SIZE_T         z_msg_len)
{
    ...
    if (i4_rc != AEER_OK)
    {
        DBG_ERROR (("ERR: send message = %d failed = %d\n\r", ui4_type, i4_rc));
    }
...
BOOL a_wzd_is_resume (VOID)
{
    ...
    if (a_cfg_custom_get_wifi_remote_support())
    {
        ...
        i4_rc = c_pcl_get_wakeup_reason (& e_wake_up_reason,
                                 NULL);
        if (i4_rc != PCLR_OK)
        {
            DBG_ERROR (("ERR: c_pcl_get_wakeup_reason failed = %d\n\r", i4_rc));
        }
    	else
    	{
    		DBG_LOG_PRINT(("[WZD] e_wake_up_reason = %d\n", e_wake_up_reason));
    	}
    }
    ...
    if (a_cfg_get_wzd_status (& ui2_status) == APP_CFGR_OK)
    {
        ...
        DBG_LOG_PRINT(("[WZD][a_wzd_is_resume] ui2_status = %d\n", ui2_status));

        if (ui2_real_status == APP_CFG_WZD_STATUS_INIT ||
            ui2_real_status  & WZD_STATE_START_SETUP)
        {
            b_resume = TRUE;
            DBG_LOG_PRINT(("[WZD][a_wzd_is_resume] b_resume = %d\n", b_resume));
        }
		else if (e_wake_up_reason == PCL_WAKE_UP_REASON_CUSTOM_2)
		{
		    if (a_cfg_custom_get_wifi_remote_support())
		    {
                b_resume = TRUE;
                #ifdef APP_PORTING
    			a_cfg_set_wzd_status (APP_CFG_WZD_STATUS_WIFI_UNPAIR);
                DBG_LOG_PRINT(("[WZD][a_wzd_is_resume] APP_CFG_WZD_STATUS_WIFI_UNPAIR\n"));
                #endif
            }
		}
    }
...
{
    DBG_INFO (("add %d-th page list entry\n\r", t_g_wzd.ui1_page_num));
...
static INT32 wzd_get_nwl_info(VOID)
{
    ...
    if ((c_nwl_get_num_rec(h_nwl_hdl, &ui2_num_rec, &ui4_ver_id)!=NWLR_OK)||
        (ui2_num_rec == 0))
    {
        DBG_LOG_PRINT(("NWL record is NULL!\n"));
        return WZDR_OK;
    }
    i4_pos=0;
    ui2_rec_id = 1;
    ...
    DBG_LOG_PRINT(("Candidate Favorite Network List number: %d\n", t_g_wzd.ui2_nwl_num));
    DBG_LOG_PRINT(("Candidate Favorite Network List:\n"));
    for(ui2_i = 0; ui2_i < t_g_wzd.ui2_nwl_num;ui2_i++)
    {
        DBG_LOG_PRINT(("num%d: %d\n", ui2_i, t_g_wzd.aui2_nwl_candidate_id[ui2_i]));
    }
                    ...
                    )
{
    ...
    if(i4_ret != AEER_OK)
    {
        DBG_LOG_PRINT(("wzd c_app_send_msg(WZD_REQ_CTX_SWITCH_EVENT) failed. i4_ret = %d\n.", i4_ret));
    }
...
INT32 wzd_get_subpage_id( UINT8* pui1_idx )
{
    ...
    DBG_LOG_PRINT(( "[WZD] Subpage ID = %d\n", *pui1_idx));
...
INT32 wzd_select_crnt_or_first_channel(BOOL b_skip_fake)
{
    ...
    if(i4_ret != TVR_OK)
    {
        ...
        if (i4_ret != TVR_OK)
        {
            DBG_ERROR (("Stop service failed = %d\n\r", i4_ret));
        }
...
BOOL a_wzd_is_resume_from_wifi_unpairing (VOID)
{
    ...
    {
    	DBG_LOG_PRINT(("a_wzd_is_resume_from_wifi_unpairing return true\n"));
        return TRUE;
    }
	DBG_LOG_PRINT(("a_wzd_is_resume_from_wifi_unpairing return false\n"));
