...
typedef enum
{
    ...
    NAV_FAC_DIALOG_ID_PIC_BRIGHT,
    NAV_FAC_DIALOG_ID_PIC_CALIBRATED,
    NAV_FAC_DIALOG_ID_PIC_CALIBRATED_DARK,
    NAV_FAC_DIALOG_ID_PIC_GAME,
    NAV_FAC_DIALOG_ID_PIC_SPORT,
#if 1	//kyo_20120202
...
static BOOL b_TPV_wifi_connect_status = FALSE;
#endif
...
extern INT32 a_sleep_timer_start(UINT32 ui4_time);	//<TPV>James 20120321:Add auto shut-down at URC RST key

#ifdef APP_TPV_DFT_URC_WIFI
...
static INT32 _nav_fac_dialog_image_init(NAV_FAC_DIALOG_VIEW_T*    pt_nav_fac_dialog)
{
    ...
    i4_ret = TPV_fac_img_create_urc_bk_img_tpl(&pt_nav_fac_dialog->h_ui_simple_dialog_bk0);
    if(WGLR_OK != i4_ret)
    {
        return NAVR_FAIL;
    }
                                          ...
                                          GL_POINT_T*         pt_begin_offset)
{
    ...
    if(pt_rect == NULL)
    {
        SET_RECT_BY_SIZE(&t_rect,
                         NAV_FAC_DIALOG_DEF_CONTENT_FRAME_X + t_begin_offset.i4_x,
                         NAV_FAC_DIALOG_DEF_CONTENT_FRAME_Y + t_begin_offset.i4_y,
                         NAV_FAC_DIALOG_DEF_CONTENT_FRAME_W,
                                ...
                                HANDLE_T*                nav_fac_dialog)              /* Out */
{
    ...
    do
    {
        ...
        *nav_fac_dialog = pt_nav_fac_dialog_show->h_frm_simple_dialog;
        ...
        /* Done */
        i4_ret = _nav_fac_dialog_set_attach_data(pt_nav_fac_dialog_show->h_frm_simple_dialog,
                                          pt_nav_fac_dialog_show);
        if(i4_result != APP_UIR_OK)
        {
            i4_ret = i4_result;
                    ...
                    )
{
    ...
    TPV_LOG_ON_FAIL(c_timer_create(&t_g_nav_fac_dialog.h_hide_timer));

    #ifdef APP_DFT_LSR_TEST //ke_vizio_20131220 add
    TPV_LOG_ON_FAIL(c_timer_create(&t_g_nav_fac_dialog.h_LSR_timer));
    #endif
...
static INT32 _nav_fac_dialog_deinit(VOID)
{
    ...
    if (c_handle_valid(pt_this->h_hide_timer)) {
        TPV_LOG_ON_FAIL(c_timer_delete(pt_this->h_hide_timer));
    }
    ...
    if (c_handle_valid(pt_this->h_LSR_timer)) {
        TPV_LOG_ON_FAIL(c_timer_delete(pt_this->h_LSR_timer));
    }
                    ...
                    )
{
    ...
    switch (ui4_key_code)
    {
		...
		case BTN_FAC_FAC:
             DBG_LOG_PRINT(("\n--------Enter Factory MODE--------\n"));
             nav_grab_activation(NAV_COMP_ID_FAC);
        ...
        case BTN_FAC_VOL_L: // Denny 2008/12/09 add
             DBG_LOG_PRINT(("\n--------Sound Left--------\n"));
            #if 0 //ke_vizio_20160902 sync MT5580 //Ben 20151221
            a_cfg_av_set(CFG_MAKE_ID(APP_CFG_GRPID_AUDIO, APP_CFG_RECID_AUD_BALANCE), -50);
            a_cfg_av_update(CFG_MAKE_ID(APP_CFG_GRPID_AUDIO, APP_CFG_RECID_AUD_BALANCE));
        ...
        case BTN_FAC_EDID_WP:
            {
                ...
                a_cfg_cust_fac_set_backlight_lifetime(0);
                _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_WP,
                                                      L"Reset TV working hours",
                                                      NULL);
                #endif
            }
            break;
        ...
        case BTN_FAC_RST:
			{
				...
				i4_ret = system("sync;sync;sync");		//mind 20160913
#endif
    			...
    			_nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SET_TO_FM,
    																	   pw2s_string,
    																	  NULL);
    			a_cfg_set_bgm_wake_disable(FALSE);
                a_amb_enable_power_key(TRUE);  // Ben 20190327, fix TV can't shut-down issue
    			a_sleep_timer_start(3);	//<TPV>James 20120321:Add auto shut-down at URC RST key
        	}
            break;
        ...
        case BTN_FAC_CTC:
		{
			...
			else // if (i2_val == 2) //normal
			{
			    DBG_LOG_PRINT(("\n--------Color Temp to Normal--------\n"));
				pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_NORMAL);
		...
		case BTN_FAC_ANTENNA:
        {
            ...
            if(i2_val == 0)
            {
                c_uc_w2s_strncat( ws2_append_txt, MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_WARM), //Warm
            ...
            {
                c_uc_w2s_strncat( ws2_append_txt, MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_NORMAL), //Normal
                                  ui4_len - c_uc_w2s_strlen(ws2_append_txt));
            }
            else
            {
	...
	case BTN_FAC_AVL_OFF:
       ...
       case BTN_FAC_SET_TO_FM:
        ...
        case BTN_FAC_P_EFFECT: //ke_vizio_20130603 modify //Denny 2011/03/17 add
            {
                ...
                case 0:
                    ...
                    pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_ITEM_VIVID);
                    _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_PIC_VIVID, pw2s_string, NULL);
                    break;
                case 1:
                    DBG_LOG_PRINT(("\n--------Picture Mode to Bright--------\n"));
                    pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_ITEM_BRIGHT);
                    _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_PIC_BRIGHT, pw2s_string, NULL);
                    break;
                case 2:
                    DBG_LOG_PRINT(("\n--------Picture Mode to Calibrated--------\n"));
                    pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_ITEM_CALIBRATED);
                    _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_PIC_CALIBRATED, pw2s_string, NULL);
                    break;
                case 3:
                    DBG_LOG_PRINT(("\n--------Picture Mode to Calibrated Dark--------\n"));
                    pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_ITEM_CALIBRATED_DARK);
                    _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_PIC_CALIBRATED_DARK, pw2s_string, NULL);
                    break;
                case 4:
                    ...
                    break;
                case 5:
                    DBG_LOG_PRINT(("\n--------Picture Mode to Sport--------\n"));
                    pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_ITEM_SPORT);
                    _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_PIC_SPORT, pw2s_string, NULL);
                    break;
                default:
                    DBG_LOG_PRINT(("\n--------Picture Mode to VIVID--------\n"));
                    pw2s_string = MLM_FAC_TEXT(nav_get_mlm_lang_id(), MLM_FAC_KEY_MSG_ITEM_VIVID);
		...
		case BTN_FAC_LSR:
		{
                ...
                UINT32 ui4_wirtten;
                ui1_status = 0x00;
                a_cfg_eep_raw_write(EEP_FACTORY_CHECK_FLAG + 1, &ui1_status, 1, &ui4_wirtten);
            #endif
            ...
            {
                i4_ret = tuner_setup_view_reset_scan();
            ...
            #endif
			case BTN_FAC_RJ45:
			case BTN_FAC_RS232:           //ke_vizio_20130912 mark
			//case BTN_FAC_RESERVE:      // Larry 20210304, Using Reserve2 key to change Digital Audio Out to Auto or PCM (TF1020VIZUSMTK01-6485) 
			    DBG_LOG_PRINT(("\n--------Not Support--------\n"));
    ...
    case BTN_FAC_RESERVE1:  //ke_vizio_20170523
    {
        ...
        DBG_LOG_PRINT(("%s EEP_WIFI_status: %d\n", __FUNCTION__, TPV_fac_get_WIFI_status()));

        pw2s_string = (TRUE == TPV_fac_get_WIFI_status() ? L"WIFI status : 01": L"WIFI Status : 00");
	...
	case BTN_FAC_RESERVE: // Larry 20210304, Using Reserve2 key to change Digital Audio Out to Auto or PCM (TF1020VIZUSMTK01-6485) 
	{
		UINT16 ui2_Digital_Audio_Out_func;		
		a_cfg_custom_get_digital_audio(&ui2_Digital_Audio_Out_func);
		UINT16 value = (ui2_Digital_Audio_Out_func==0) ?1:0;  
		a_cfg_custom_set_digital_audio(value);		
		a_cfg_set_spdif_type(value);
        a_cfg_update_spdif_type();
		if (value == 1)
		{
			_nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_ARC_ON,
                                                  L"Digital Audio Out is PCM",
                                                  NULL);
			DBG_LOG_PRINT(("\n--------TPV factory Digital Audio Out is PCM, value is %d--------\n", value));
		}
		else
		{
			_nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_ARC_OFF,
                                                  L"Digital Audio Out is Auto",
                                                  NULL);
			DBG_LOG_PRINT(("\n--------TPV factory Digital Audio Out is Auto, value is %d--------\n", value));
		}
 ...
 *---------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------
 * Name : a_TPV_fac_dialog_register
 * Description
 * Input arguments
 * Output arguments
 * Returns  -
 *---------------------------------------------------------------------------*/
INT32 a_TPV_fac_dialog_register(VOID)
{
    NAV_COMP_T      t_comp;
    INT32           i4_ret;
...
extern INT32 nav_fac_keypad_test(UINT32 ui4_keystatus,UINT32 * pt_ui4_keycode)
{
    ...
    a_cfg_cust_fac_get(&ui4_fac_val);
    a_cfg_custom_get_model_name(s_name);

    if (is_one_button_model_platform() == FALSE)
    {
        //if ((ui4_fac_val & APP_CFG_CUST_FAC_RC_ON)&&(nav_get_key_source() == KEY_SRC_FRONT_PANEL))   // fac rc on
        if (((ui4_fac_val & APP_CFG_CUST_FAC_RC_ON)&&(nav_get_key_source() == KEY_SRC_FRONT_PANEL))|| (a_cfg_cust_fac_get_urc_flag() == 1))   // fac rc on
        {
            if(ui4_keystatus == WGL_MSG_KEY_DOWN)
            {
                switch(ui4_keycode)
                {
                    case BTN_PRG_UP:     /* Ch + */
                        c_sprintf(popmes,"CH +");
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_CH_UP,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_CH_INC); //Denny 2014/02/17 add
                        c_mem_free(pw2s_string);
                        break;
                    ...
                    case BTN_PRG_DOWN:   /* Ch - */
                        ...
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_CH_DOWN,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_CH_DEC); //Denny 2014/02/17 add
                        c_mem_free(pw2s_string);
                        break;

                    case BTN_VOL_UP:     /* Vol + */
                        c_sprintf(popmes,"VOL +");
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_VOL_UP,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_VOL_INC); //Denny 2014/02/17 add
                        c_mem_free(pw2s_string);
                        break;
                    ...
                    case BTN_VOL_DOWN:   /* Vol - */
                        ...
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_VOL_DOWN,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_VOL_DEC); //Denny 2014/02/17 add
                        c_mem_free(pw2s_string);
                        break;

                    case BTN_INPUT_SRC:       /* Input Source */
                        c_sprintf(popmes,"SOURCE");
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SRC,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_INPUT); //Denny 2014/02/17 add
                        c_mem_free(pw2s_string);
                        break;

                    case BTN_FAC_KEYPAD_POWER:  //BTN_CUSTOM_10:        /* Power */
                        c_sprintf(popmes,"POWER");
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SRC,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_POWER); //Denny 2014/02/17 add
                        c_mem_free(pw2s_string);
                    #ifdef TPV_2K19_DFM // Ben 20181221
                        UINT8 ui1_status;
                        UINT32 ui4_wirtten;
                        ui1_status = 0x00;
                        a_cfg_eep_raw_write(EEP_FACTORY_CHECK_FLAG + 4, &ui1_status, 1, &ui4_wirtten);
                    #endif

                        break;

                    case BTN_FAC_KEYPAD_MENU: //BTN_CUSTOM_11:     /* Menu*/
                        c_sprintf(popmes,"MENU");
                        pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                        c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                        _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SRC,pw2s_string,NULL);
                        a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_MENU); //Denny 2014/02/17 add
                        c_mem_alloc(pw2s_string);
                        break;
    ...
    if ((c_strstr(s_name,"V556-J01")!= NULL)||
        (c_strstr(s_name,"V586-J01")!= NULL)||
        (c_strstr(s_name,"M55Q6-J01")!= NULL)||
        (c_strstr(s_name,"M55Q6-J01")!= NULL)||
        (c_strstr(s_name,"M50Q7-J01")!= NULL)||
        (c_strstr(s_name,"M55Q7-J01")!= NULL)||
        (c_strstr(s_name,"M58Q7-J01")!= NULL)||
        (c_strstr(s_name,"M65Q7-J01")!= NULL)||
        (c_strstr(s_name,"M50Q6-J01")!= NULL)||
        (c_strstr(s_name,"V556x-J01")!= NULL)||
        (c_strstr(s_name,"V586x-J01")!= NULL)||
        (c_strstr(s_name,"-K01")!= NULL)
        )
    {
            if ((ui4_fac_val & APP_CFG_CUST_FAC_RC_ON)|| (a_cfg_cust_fac_get_urc_flag() == 1))   // fac rc on
            {
                if((ui4_keystatus == WGL_MSG_KEY_DOWN) && (ui4_keycode == BTN_INPUT_SRC))
                {
                    c_sprintf(popmes,"POWR");
                    pw2s_string = (UTF16_T*) c_mem_alloc(10 * sizeof(UTF16_T));
                    c_uc_ps_to_w2s(popmes, pw2s_string, sizeof(popmes));
                    _nav_fac_dialog_prompt_msg_dialog(NAV_FAC_DIALOG_ID_SRC,pw2s_string,NULL);
                    a_cfg_cust_fac_set_keypad_status_read(ACFG_FAC_KEYPAD_INPUT); //Denny 2014/02/17 add
...
VOID TPV_custom_rcu_key_pre_handler(
                    NAV_MODEL_T*                pt_this,
                    NAV_EVN_T*                  pt_event
                    )
{
...
BOOL TPV_custom_system_key_cb(UINT32     ui4_evt_code)
{
    BOOL               b_process = TRUE;
    ...
    if(ui4_fac_val & APP_CFG_CUST_FAC_RC_ON)
    {
        ...
        if(ui4_evt_code == IOM_GEN_EVT_CODE(KEY_SRC_FRONT_PANEL, BTN_POWER))
        {
            t_event.ui4_msg_id = NAV_KEY_EVENT;
            t_event.ui4_data1  = WGL_MSG_KEY_DOWN;
...
INT32 TPV_FACTORY_WIFI_AUTODETECT(VOID)
{
    ...
    CHAR a_ssid[NET_802_11_MAX_LEN_SSID] = "iptvwifi_test";
    CHAR a_password[128+1]= "0000000000";

    UINT8 ui1_odm_index = 0;

    TPV_fac_set_WIFI_status(FALSE);  // Reset the connect status

    a_cfg_custom_get_odm_index(&ui1_odm_index);

    if(ui1_odm_index != 2){
        DBG_LOG_PRINT(("TPV_FACTORY_WIFI_AUTODETECT ODM ID : %d\n", ui1_odm_index));
...
)
{
    ...
    switch (e_nfy_id)
    {
        case NW_NFY_ID_WLAN_START_IND:  // WIFI driver ready and start
        case NW_NFY_ID_DHCP_SUCCESS_DHCPv4: //After WIFI connected
        {
            ...
            if(ui1_status == 0xAA)
            {
                ...
                {
                    DBG_LOG_PRINT(("%s Line: %d, TPV_FACTORY_WIFI_AUTODETECT\n", __FUNCTION__, __LINE__));
                    TPV_FACTORY_WIFI_AUTODETECT();
                }
                else
                {
                    TPV_fac_set_WIFI_status(TRUE);
                }
            }
            break;
        }
 ...
 *
 ----------------------------------------------------------------------------*/
VOID TPV_fac_set_WIFI_status(BOOL b_connect)
{
    b_TPV_wifi_connect_status = b_connect;
}
/*----------------------------------------------------------------------------
 * Name: _auto_detect_notify_callback
 *
 ...
 *
 ----------------------------------------------------------------------------*/
BOOL TPV_fac_get_WIFI_status(VOID)
{
    return b_TPV_wifi_connect_status;
}

#endif /* end of APP_TPV_DFT_WIFI_AUTO_DETECTION */
