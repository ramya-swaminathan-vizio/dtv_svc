...
//ke_vizio_20180626
typedef struct _NAV_FAC_F_ICON_VIEW_T
{
    /* Widgets */
    HANDLE_T        h_icon_frm;   /* the icon frame  */
    WGL_HIMG_TPL_T  h_ui_F_icon;  /* F icon image */
} NAV_FAC_F_ICON_VIEW_T;
...
static NAV_FAC_F_ICON_VIEW_T t_g_nav_F_icon = {0};  //ke_vizio_20180626
BOOL b_TPV_facotry_parser_icon_show = FALSE; 
/*-----------------------------------------------------------------------------
...
//ke_vizio_20180626
static INT32 _TPV_Factory_F_icon_create(HANDLE_T h_canvas,INT32 i4_frame_x_offset,INT32 i4_frame_y_offset);
/*-----------------------------------------------------------------------------
...
static INT32 _nav_fac_urc_dialog_image_init(NAV_FAC_URC_DIALOG_VIEW_T*    pt_nav_fac_dialog)
{
    ...
    i4_ret = TPV_fac_img_create_T_img_tpl(&pt_nav_fac_dialog->h_ui_simple_dialog_bk0);
    if(WGLR_OK != i4_ret)
    {
        return NAVR_FAIL;
    }
                                ...
                                HANDLE_T*                nav_fac_dialog)              /* Out */
{
    ...
    {
        /* Content Frame */
        i4_result =  _nav_fac_urc_dialog_create_frm(pt_nav_fac_dialog_show,
        ...
        /* txtinfo */
        i4_result =  _nav_fac_urc_dialog_create_txtinfo(pt_nav_fac_dialog_show,
                                                     pt_nav_fac_dialog_init->pt_rect,
...
//    c_timer_create(&t_g_nav_fac_urc_dialog.h_hide_timer);
    ...
    //ke_vizio_20180626 : create F icon
    c_memset(&t_g_nav_F_icon, 0, sizeof(NAV_FAC_F_ICON_VIEW_T));
    _TPV_Factory_F_icon_create(pt_ctx->t_gui_res.h_canvas,
                    ...
                    )
{
    ...
    switch (pt_ui_msg->e_id)
    {
        case NAV_UI_MSG_POWER_OFF:
        {
            ...
            //ke_vizio_20180626
            if(b_TPV_facotry_parser_icon_show == TRUE)
            {
                i4_ret = c_wgl_set_visibility(t_g_nav_F_icon.h_icon_frm, WGL_SW_HIDE);
                if(i4_ret != APP_UIR_OK)
                {
                    DBG_LOG_PRINT(("%s Line: %d\n", __FUNCTION__, __LINE__));
                    return i4_ret;
                }
                i4_ret = c_wgl_repaint(t_g_nav_F_icon.h_icon_frm, NULL, TRUE);
                if(i4_ret != APP_UIR_OK)
                {
                    DBG_LOG_PRINT(("%s Line: %d\n", __FUNCTION__, __LINE__));
                    return i4_ret;
                }
                b_TPV_facotry_parser_icon_show = FALSE;
            }
            break;
        }
 ...
 * Returns  -
 *---------------------------------------------------------------------------*/
INT32 a_TPV_fac_urc_dialog_register(VOID)
{
    NAV_COMP_T      t_comp;
    INT32           i4_ret;
 ...
 * Returns  -
 *---------------------------------------------------------------------------*/
static INT32 _TPV_Factory_F_icon_create(HANDLE_T h_canvas,
                                        INT32 i4_frame_x_offset,
                                        INT32 i4_frame_y_offset)
{
    INT32 i4_ret = 0;
    NAV_FAC_F_ICON_VIEW_T* pt_F_icon = &t_g_nav_F_icon;
    GL_RECT_T         t_rect;
    UINT32            ui4_style = 0;
    WGL_IMG_INFO_T    t_img_info;
    //UINT32 ui4_width = 0;
    ...
    i4_ret = TPV_fac_img_create_F_img_tpl(&pt_F_icon->h_ui_F_icon);
    if(i4_ret !=0 )
    {
        return i4_ret;
    }

    //i4_ret = c_wgl_get_canvas_size(h_canvas, &ui4_width, &ui4_height);
    //DBG_LOG_PRINT((" %s ui4_width: %d\n", __FUNCTION__, ui4_width));
    //DBG_LOG_PRINT((" %s ui4_height: %d\n", __FUNCTION__, ui4_height));
    
    SET_RECT_BY_SIZE(&t_rect,
                         NAV_FAC_DIALOG_URC_FRAME_X + 45,
                         NAV_FAC_DIALOG_URC_FRAME_Y,
                         NAV_FAC_DIALOG_URC_FRAME_W,
                         NAV_FAC_DIALOG_URC_FRAME_H);
    ...
    i4_ret = c_wgl_create_widget(h_canvas,
                                 ...
                                 (VOID*)ui4_style,
                                 NULL,
                                 &pt_F_icon->h_icon_frm);
    if (i4_ret < 0)
    {
        DBG_LOG_PRINT(("%s Line: %d c_wgl_create_widget fail\n", __FUNCTION__, __LINE__));
        return APP_UIR_WIDGET_CREATE_FAIL;
    }

    t_img_info.e_type = WGL_IMG_SET_STANDARD;
    t_img_info.u_img_data.t_standard.t_enable    = pt_F_icon->h_ui_F_icon;
    t_img_info.u_img_data.t_standard.t_highlight = pt_F_icon->h_ui_F_icon;
    t_img_info.u_img_data.t_standard.t_disable   = pt_F_icon->h_ui_F_icon;
    i4_ret = c_wgl_do_cmd(pt_F_icon->h_icon_frm,
                          WGL_CMD_GL_SET_IMAGE,
                          WGL_PACK(WGL_IMG_BK),
                          WGL_PACK(&t_img_info));
    if(WGLR_OK != i4_ret)
    {
        DBG_LOG_PRINT(("%s Line: %d WGL_CMD_GL_SET_IMAGE fail\n", __FUNCTION__, __LINE__));
        return NAVR_FAIL;
    }

    i4_ret = c_wgl_set_visibility(pt_F_icon->h_icon_frm, WGL_SW_HIDE);
    if(i4_ret != APP_UIR_OK)
    {
        DBG_LOG_PRINT(("%s Line: %d c_wgl_set_visibility fail\n", __FUNCTION__, __LINE__));
        return i4_ret;
    }
 ...
 * Returns  -
 *---------------------------------------------------------------------------*/
VOID a_nav_TPV_Factory_show_icon(VOID* param0, VOID* param1, VOID *param2)
{
    INT32   i4_ret = 0;
    NAV_FAC_F_ICON_VIEW_T* pt_F_icon = &t_g_nav_F_icon;
    ...
    if(b_TPV_facotry_parser_icon_show == TRUE)
    {   
        i4_ret = c_wgl_set_visibility(pt_F_icon->h_icon_frm, WGL_SW_RECURSIVE);
        if(i4_ret != APP_UIR_OK)
        {
            return;
        }
        i4_ret = c_wgl_repaint(pt_F_icon->h_icon_frm, NULL, TRUE);
        if(i4_ret != APP_UIR_OK)
        {
            return;
        }
 ...
 * Returns  -
 *---------------------------------------------------------------------------*/
VOID a_Factory_to_nav_show_icon(BOOL b_show)
{
    b_TPV_facotry_parser_icon_show = b_show;
    nav_request_context_switch(a_nav_TPV_Factory_show_icon,
                               NULL,
                               NULL,
                               NULL);
}
