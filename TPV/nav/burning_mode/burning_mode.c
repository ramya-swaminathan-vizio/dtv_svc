 ...
 *---------------------------------------------------------------------------*/
    ...
    #ifndef TPV_2K19_BURNING_MODE
    #define BURNING_MODE_TIMER_DELAY         (3000)  /* in ms */
    #else
        #if defined(TPV_BURNING_MODE_4K)
        #define BURNING_MODE_TIMER_DELAY         (2000)  /* in ms */
        #elif defined(TPV_BURNING_MODE_VCOM)
        #define BURNING_MODE_TIMER_DELAY         (2000)  /* in ms */
        #else
        #define BURNING_MODE_TIMER_DELAY         (3000)
        #endif
    #endif
#endif
                    ...
                    )
{
    ...
    #ifdef TPV_2K19_BURNING_MODE
    i4_ret = burning_mode_ptgn_deinit_plane();
    if (i4_ret != NAVR_OK)
    {
        return NAVR_FAIL;
    }
                    ...
                    )
{
    ...
    #if defined(TPV_BURNING_MODE_VCOM)
    if(a_cfg_cust_oled_support() == FALSE)
    {
                    ...
                    )
{
    ...
    if (! pt_this->b_stop) /* Timer is running. */
    {
        ...
        #ifdef TPV_2K19_BURNING_MODE
        //DBG_LOG_PRINT(("\n[TPV_FAC] %s pt_this->b_active: %d\n", __FUNCTION__, pt_this->b_active));
        #ifndef TPV_BURNING_MODE_VCOM
                    ...
                    )
{
    ...
    _burning_mode_stop_timer (pt_this);
        ...
        if(a_cfg_cust_oled_support() == TRUE)
        {
            ...
            a_scrn_svr_set_enable(FALSE);
            a_c4tv_apron_enable_cast_shell(FALSE);
            menu_blank_oled_scrn_timer_stop();  // Temp solution to stop blank screen.
        }
        else
        {
            burning_mode_ptgn_vcom_pattern_stop();
    ...
    if (! pt_this->b_stop) /* Timer is running. */
    {
            ...
            (e_active_comp == NAV_COMP_ID_LAST_VALID_ENTRY))
        {
            ...
            #if defined(APP_C4TV_SUPPORT)&& defined(APP_VTRL_INP_SRC_SUPPORT) && defined(ISL_VTRL_INP_SRC_CAST)
            {
                ...
                {      
                    DBG_LOG_PRINT(("\n[BURNING_MODE] current source is DEV_VTRL_CAST, stop burning\n"));
                    _burning_mode_stop_burning();
            ...
            if(a_cfg_cust_oled_support() == FALSE){
                #if defined(TPV_BURNING_MODE_4K)
                burning_mode_ptgn_pattern_run();
                #elif defined(TPV_BURNING_MODE_VCOM)
                burning_mode_ptgn_vcom_pattern_run();
                #else
                burning_mode_ptgn_pattern_run_old();
                #endif
            }
            else{
                //burning_mode_ptgn_oled_pattern_run();
                burning_mode_ptgn_LG_oled_pattern_run();
            }
            #endif
