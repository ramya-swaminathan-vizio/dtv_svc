...
# $SWAuthor: Iolo Tsai $
# $MD5HEX: 470b30bae75011d2365627dd409da91f $
#
# Description:
...
include $(TARGET_ROOT)/linux_mak/target.mak
...
ifeq "$(THUMB)" "y"
CC_FLAG += -mthumb -mthumb-interwork
CC_FLAG += -mlong-calls
CC_FLAG += -mcallee-super-interworking  -mcaller-super-interworking
endif
endif

#CC_FLAG_R := $(shell if [ -e ./NONE_CHECK_WARN ]; then echo $(CC_FLAG) | sed -r 's/diag_error/diag_warning/g; s/Werror/Wall/g; '; else echo $(CC_FLAG); fi)

ifndef ARM_FEEDBACK_FILE
	export ARM_FEEDBACK_FILE = $(THIS_ROOT)/LINK_FEEDBACK.txt
endif
CC_FLAG += -fvisibility=hidden
...
CPP_FLAG_R := $(subst -std=gnu99,-std=gnu++11,$(CC_FLAG_R))

CPP_FLAG_R += -Wno-literal-suffix

#
# Set the actual compiler include path and define
#
ifeq "$(findstring $(CUSTOM_ROOT),$(CC_INC))" ""
CC_INC := -I$(CUSTOM_ROOT) $(CC_INC)
endif

ABS_INCLUDE = -I$(C_INC_DIR) -I$(X_INC_DIR) $(TARGET_INC) $(CC_INC)
ABS_DEFINES = $(sort $(DEFINES))

#
# Set Redirect Tool Path
#
REDIRECT = $(TOOL_ROOT)/source/public/pbuild/redirect.sh
...
#
.PHONY : all custom lib depend c_inc $(SUB_COMPONENTS) $(OPTIONAL_SUB_COMPONENTS)


all custom lib : $(SUB_COMPONENTS) $(OPTIONAL_SUB_COMPONENTS) $(OBJ) $(LIBS)
...
#
$(SUB_COMPONENTS) :
	@$(MAKE) $(MAKE_FLAG) -C $(call adjust_custom_comp_path,$@) $(MAKECMDGOALS) ENTER_OPT_SUB_COMP=FALSE


$(OPTIONAL_SUB_COMPONENTS) :
	@$(MAKE) $(MAKE_FLAG) -C $(call adjust_custom_comp_path,$@) $(MAKECMDGOALS) ENTER_OPT_SUB_COMP=TRUE
...
define rule_cpp_o_c
	@echo -e "  CPP\t$@"
	@echo -E '$(CXX) $(CC_DEP_GEN_FLAG)$(strip $(OBJ_DIR)/$(@:.o=.d)) -c $(CPP_FLAG_R) $(call inc_path_cnvt,$(ABS_INCLUDE)) $(ABS_DEFINES) $< -o $(OBJ_DIR)/$@' > $(OBJ_DIR)/$(@:.o=.o.cmd)
	$(QUIET)$(CXX) $(CC_DEP_GEN_FLAG)$(strip $(OBJ_DIR)/$(@:.o=.d)) -c $(CPP_FLAG_R) $(call inc_path_cnvt,$(ABS_INCLUDE)) $(ABS_DEFINES) $< -o $(OBJ_DIR)/$@ 2>&1 | tee $(OBJ_DIR)/$@.log
endef
...
#
$(LIBS) : $(OBJ)
	$(call if_dir_not_exist_fct,$(call path_cnvt,$(CUSTOM_LIB_DIR)), \
	    $(MKDIR) $(MKDIR_FLAG) $(call path_cnvt,$(CUSTOM_LIB_DIR));)
